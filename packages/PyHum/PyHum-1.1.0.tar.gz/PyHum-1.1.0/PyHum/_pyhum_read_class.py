...
class humread:
   ...
   def __init__(self, humfile, sonpath, cs2cs_args, c, draft, doplot, t, f, bedpick, flip_lr):
      ...
      try:
         ...
         savemat(os.path.expanduser("~")+base+'.mat', mdict={'dat': dat, 'data_port': data_port, 'data_star': data_star, 'data_dwnlow': data_dwnlow, 'data_dwnhi': data_dwnhi},oned_as='row')
      ...
      try:
         ...
         kml.save(os.path.expanduser("~")+base+"trackline.kml")
      ...
      try:
         ...
         savemat(os.path.expanduser("~")+base+'meta.mat', metadat ,oned_as='row')
      ...
      try:
         ...
         heading = np.squeeze(loadmat(os.path.expanduser("~")+base+'meta.mat')['heading'])
      ...
      try:
         ...
         f = open(os.path.expanduser("~")+base+'rawdat.csv', 'wt')
         writer = csv.writer(f)
         writer.writerow( ('longitude', 'latitude', 'easting', 'northing', 'depth (m)', 'distance (m)', 'heading (deg.)' ) )
         for i in range(0, len(lon)):
            writer.writerow(( float(lon[i]),float(lat[i]),float(es[i]),float(ns[i]),float(dep_m[i]),float(dist_m[i]), float(heading[i]) ))
