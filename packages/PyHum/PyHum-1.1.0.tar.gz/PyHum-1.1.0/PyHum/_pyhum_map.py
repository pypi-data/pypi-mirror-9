...
def domap(humfile, sonpath, cs2cs_args, imagery):
      ...
      try:
         ...
         esi = np.squeeze(loadmat(os.path.expanduser("~")+base+'meta.mat')['e']) #+ 395
         nsi = np.squeeze(loadmat(os.path.expanduser("~")+base+'meta.mat')['n']) #- 58
         pix_m = np.squeeze(loadmat(os.path.expanduser("~")+base+'meta.mat')['pix_m'])
         bearing = np.squeeze(loadmat(os.path.expanduser("~")+base+'meta.mat')['heading'])
         port_la = np.asarray(np.squeeze(loadmat(os.path.expanduser("~")+base+'port_la.mat')['port_mg_la']),'float16')
         star_la = np.asarray(np.squeeze(loadmat(os.path.expanduser("~")+base+'star_la.mat')['star_mg_la']),'float16')
      ...
      try:
         ...
         outfile = os.path.expanduser("~")+'x_y_ss_raw.asc' 
         with open(outfile, 'w') as f:
            np.savetxt(f, np.hstack((humutils.ascol(X.flatten()), humutils.ascol(Y.flatten()), humutils.ascol(merge.flatten()))), delimiter=' ', fmt="%8.6f %8.6f %8.6f")
...
#plt.show()
      ...
      try:
         ...
         kml.save(os.path.expanduser("~")+base+"GroundOverlay.kml")
