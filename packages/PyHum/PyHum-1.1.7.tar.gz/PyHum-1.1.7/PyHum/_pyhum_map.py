...
#      try:
#         d = Parallel(n_jobs = min(cpu_count(),len(star_fp)), verbose=0)(delayed(make_map)(esi[shape_port[-1]*p:shape_port[-1]*(p+1)], nsi[shape_port[-1]*p:shape_port[-1]*(p+1)], theta[shape_port[-1]*p:shape_port[-1]*(p+1)], dist_tvg[shape_port[-1]*p:shape_port[-1]*(p+1)], port_fp[p], star_fp[p], pix_m, res, cs2cs_args, sonpath, trans, dogrid, p) for p in xrange(len(star_fp)))
#      except:
#         print "memory error: trying serial"
#         d = Parallel(n_jobs = 1, verbose=0)(delayed(make_map)(esi[shape_port[-1]*p:shape_port[-1]*(p+1)], nsi[shape_port[-1]*p:shape_port[-1]*(p+1)], theta[shape_port[-1]*p:shape_port[-1]*(p+1)], dist_tvg[shape_port[-1]*p:shape_port[-1]*(p+1)], port_fp[p], star_fp[p], pix_m, res, cs2cs_args, sonpath, trans, dogrid, p) for p in xrange(len(star_fp)))
...
## =========================================================
#def make_map(e, n, t, d, dat_port, dat_star, pix_m, res, cs2cs_args, sonpath, trans, dogrid, p):
#   
...
#      X.append(xx)
#      Y.append(yy) 
#      R.append(d[k] / (np.sqrt( (e[k]-xx)**2 + (n[k]-yy)**2 )) )  #range to point  
