...
* a `version.py` created by some earlier build step
...
Versioneer works by adding a special `version.py` file into your source
tree, where your `__init__.py` can import it. This `version.py` knows how to
dynamically ask the VCS tool for version information at import time. However,
when you use "setup.py build" or "setup.py sdist", `version.py` in the new
copy is replaced by a small static file that contains just the generated
version data.

`version.py` also contains `$Revision$` markers, and the installation
process marks `version.py` to have this marker rewritten with a tag name
...
* `versionfile_source`:
  ...
  be written. This is usually a `version.py` next to your project's main
  `__init__.py` file, so it can be imported at runtime. If your project uses
  `src/myproject/__init__.py`, this should be `src/myproject/version.py`.
  ...
  therefore never import `version.py`), since "setup.py sdist" -based trees
  still need somewhere to record the pre-calculated version strings. Anywhere
  in the source tree should do. If there is a `__init__.py` next to your
  `version.py`, the `setup.py versioneer` command (described below) will
...
*  `versionfile_build`:
  ...
  then you will probably have `versionfile_build='myproject/version.py'` and
  `versionfile_source='src/myproject/version.py'`.

  If this is set to None, then `setup.py build` will not attempt to rewrite
  any `version.py` in the built tree. If your project does not have any
...
* 2: add the following lines to the top of your `setup.py`, with the
  configuration values you decided earlier:
        ...
        import versioneer
        versioneer.VCS = 'git'
        versioneer.versionfile_source = 'src/myproject/version.py'
        versioneer.versionfile_build = 'myproject/version.py'
...
* 4: now run `setup.py versioneer`, which will create `version.py`, and will
  modify your `__init__.py` (if one exists next to `version.py`) to define
  `__version__` (by calling a function from `version.py`). It will also
  modify your `MANIFEST.in` to include both `versioneer.py` and the generated
  `version.py` in sdist tarballs.
...
* re-run `setup.py versioneer` to replace `SRC/version.py`
...
public domain. The `version.py` that it creates is also in the public
domain.
...
# these strings are filled in when 'setup.py versioneer' creates version.py
...
def git_get_keywords(versionfile_abs):
    # the code embedded in version.py can just fetch the value of these
    # keywords. When used from setup.py, we don't want to import version.py,
...
def git_versions_from_vcs(tag_prefix, root, verbose=False):
    ...
    # version.py hasn't already been rewritten with a short version string,
...
def get_versions(default={"version": "unknown", "full": ""}, verbose=False):
    # I am in version.py, which lives at ROOT/VERSIONFILE_SOURCE. If we have
...
def get_versions(default=DEFAULT, verbose=False):
    ...
    # extract version from first of version.py, VCS command (e.g. 'git
...
class cmd_build(_build):
    def run(self):
        ...
        _build.run(self)
        # now locate version.py in the new build/ directory and replace it
...
class cmd_sdist(_sdist):
    ...
    def make_release_tree(self, base_dir, files):
        ...
        # now locate version.py in the new base_dir directory (remembering
...
class cmd_update_files(Command):
    description = "install/upgrade Versioneer files: __init__.py SRC/version.py"
    user_options = []
    boolean_options = []
    def initialize_options(self):
        pass
    ...
    def run(self):
        ...
        # (PKG/version.py, used by runtime code) are in MANIFEST.in, so
        ...
        # .gitattributes to mark version.py for export-time keyword
        # substitution.
