...
class Command(NoArgsCommand):
    """
    Command object for importing a feed blog
    into Zinnia via Google's gdata API.
    """
    help = 'Import a feed blog into Zinnia.'

    option_list = NoArgsCommand.option_list + (
        make_option('--feed-username', dest='feed_username', default='',
                    help='The username to login to feed with'),
        make_option('--category-title', dest='category_title', default='',
                    help='The Zinnia category to import feed posts to'),
        make_option('--feed-blog-id', dest='feed_blog_id', default='',
                    help='The id of the feed blog to import'),
        make_option('--feed-limit', dest='feed_limit', default=25,
    ...
    def handle_noargs(self, **options):
        ...
        self.feed_username = options.get('feed_username')
        self.feed_blog_id = options.get('feed_blog_id')
        self.feed_limit = int(options.get('feed_limit'))
        ...
        self.write_out(self.style.TITLE(
            'Starting migration from feed to Zinnia %s\n' % __version__))

        if not self.feed_username:
            self.feed_username = input('feed username: ')
            if not self.feed_username:
                raise CommandError('Invalid feed username')

        self.feed_password = getpass('feed password: ')
        try:
            self.feed_manager = feedManager(self.feed_username,
                                                  self.feed_password)
        except gdata_service.BadAuthentication:
            raise CommandError('Incorrect feed username or password')
    ...
    def select_blog_id(self):
        ...
        blogs_list = [blog for blog in self.feed_manager.get_blogs()]
        while True:
            i = 0
            blogs = {}
        ...
        self.feed_blog_id = get_blog_id(blog)
    ...
    def import_posts(self):
        ...
        for post in self.feed_manager.get_posts(self.feed_blog_id,
                                                   self.feed_limit):
            creation_date = convert_feed_timestamp(post.published.text)
            ...
            try:
                ...
                entry.last_update = convert_feed_timestamp(
                    post.updated.text)
                entry.save()
    ...
    def import_comments(self, entry, post):
        blog_id = self.feed_blog_id
        post_id = get_post_id(post)
        comments = self.feed_manager.get_comments(blog_id, post_id)
        entry_content_type = ContentType.objects.get_for_model(Entry)

        for comment in comments:
            submit_date = convert_feed_timestamp(comment.published.text)
...
def convert_feed_timestamp(timestamp):
...
class feedManager(object):
    ...
    def __init__(self, username, password):
        ...
        self.service.server = 'www.feed.com'
