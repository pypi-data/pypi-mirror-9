...
class AlignedInterval(object):
    """docstring for AlignedInterval
        stores number of aligned reads and total alignment 
        length covered on the contigs by the alignments"""
    def __init__(self, c1,c2):
        super(AlignedInterval, self).__init__()
        self.ctg1 = c1 
        self.ctg2 = c2
        self.alignments[c1] = {}
        self.alignments[c2] = {}

        self.alignments[c1]['n'] = 0
        self.alignments[c2]['n'] = 0

        self.alignments[c1]['a'] = 0
        self.alignments[c2]['a'] = 0


    def add_alignment(self,ctg,start,stop):
        self.alignments[ctg] 
    def get_alignment_coverage(self,ctg1,ctg2):
        return self.alignments[c1]['a'], self.alignments[c1]['n'], self.alignments[c2]['a'], self.alignments[c2]['n']
...
def PE(Contigs, Scaffolds, Information, C_dict, param, small_contigs, small_scaffolds, bam_file):
    ...
    print 'done with initailizations of all structures!'
    # coverage 
    r_len = float(param.read_len)
    cyy=0
    for read in bam_object.aligned_reads():
        contig = bam_object.bam_file.getrname(read.rname)
        cont_aligned_len[contig][0] += read.alen
        # if read.qlen/float(r_len) < 0.9 and not read.mate_is_unmapped:
        #     if read.rname != read.mrnm :
        #         print read.qlen, read.pos, read.rlen, cont_aligned_len[contig][1]
        #         cyy += 1


    print 'done with coverage comp!'
    print cyy, 'reads partly mapped. avg read_len:', r_len
    sys.exit()
    # link information
    for read1,read2 in bam_object.unique_reads_on_different_references():
        if read1.qlen/r_len < 0.98 or read2.qlen/r_len < 0.98 or read1.qlen < 25 or read2.qlen < 25:
            continue
        ...
        if contig1 in Contigs and contig2 in Contigs and Contigs[contig2].scaffold != Contigs[contig1].scaffold:
            ...
            if not param.no_score:
                is_dupl = CreateEdge(cont_obj1, cont_obj2, scaf_obj1, scaf_obj2, G, param, read1, read2, counter, contig1, contig2)
            else:
                is_dupl = CreateEdge(cont_obj1, cont_obj2, scaf_obj1, scaf_obj2, G_prime, param, read1, read2, counter, contig1, contig2, save_obs=False)

            if param.extend_paths and not is_dupl and not param.no_score:
                counter.prev_obs1 = -1
                counter.prev_obs2 = -1
                CreateEdge(cont_obj1, cont_obj2, scaf_obj1, scaf_obj2, G_prime, param, read1, read2, counter, contig1, contig2, save_obs=False)
...
def CreateEdge(cont_obj1, cont_obj2, scaf_obj1, scaf_obj2, G, param, read1, read2, counter, contig1, contig2, save_obs=True):
    counter.count += 1
