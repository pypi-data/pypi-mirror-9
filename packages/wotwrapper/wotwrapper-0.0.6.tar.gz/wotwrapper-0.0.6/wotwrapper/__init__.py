...
def connect(path, name, objectToWrap, sendDataFunction, delay):
	def onMessage(message, send):
		...
		try:
			...
			return
		if msg[0] == name and hasattr(objectToWrap, msg[1]):
			try:
				methodToCall = getattr(objectToWrap, msg[1])
				result = methodToCall(*msg[2:])
				resp = msg[0:2]
				if type(result) is list:
					resp += result
				else: resp.append(result)
				send(resp)
			except:
	...
	time.sleep(2)
	while True:
		result = [name, sendDataFunction.__func__.__name__, sendDataFunction()]
		if type(result[2]) is list:
