...
class Product(Base):
    ...
    uuid = uuid_column()
    upc = sa.Column(GPCType(), index=True)
    department_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('departments.uuid'))
    subdepartment_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('subdepartments.uuid'))
    category_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('categories.uuid'))
    family_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('families.uuid'))

    report_code_uuid = sa.Column(sa.String(length=32), nullable=True, doc=u"""\
UUID of the product's report code, if any.
""")

    brand_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('brands.uuid'))
...
""")
    ...
    regular_price_uuid = sa.Column(
        sa.String(length=32),
        sa.ForeignKey('product_prices.uuid',
                   use_alter=True,
                   name='products_regular_price_uuid_fkey'))

    current_price_uuid = sa.Column(
        sa.String(length=32),
        sa.ForeignKey('product_prices.uuid',
                   use_alter=True,
...
class ProductCode(Base):
    ...
    uuid = uuid_column()
    product_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('products.uuid'), nullable=False)
...
class ProductCost(Base):
    ...
    uuid = uuid_column()
    product_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('products.uuid'), nullable=False)
    vendor_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('vendors.uuid'), nullable=False)
