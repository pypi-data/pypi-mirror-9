...
class ImportSubcommand(Subcommand):
    ...
    def run(self, args):
        from sqlalchemy_continuum import get_versioning_manager, remove_versioning

        log.info("begin {0} for data model(s): {1}".format(
                self.name, ', '.join(args.models or ["ALL"])))

        if args.no_versioning:
            log.info("disabling versioning")
            manager = get_versioning_manager(model.Product) # any ol' model
