...
class CustomerGroupAssignment(Base):
    ...
    uuid = uuid_column()
    customer_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('customers.uuid'), nullable=False)
    group_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('customer_groups.uuid'), nullable=False)
...
class CustomerPerson(Base):
    ...
    uuid = uuid_column()
    customer_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('customers.uuid'), nullable=False)
    person_uuid = sa.Column(sa.String(length=32), sa.ForeignKey('people.uuid'), nullable=False)
    ordinal = sa.Column(sa.Integer(), nullable=False)
