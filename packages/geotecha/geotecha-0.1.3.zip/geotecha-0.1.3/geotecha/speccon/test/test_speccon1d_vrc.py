...
def test_schiffman_and_stein_1970():
    ...
    dTv = dTvc= 1 / H**2 * kvref / mvref
    neig = 60

    n=6
    mv = PolyLine(z1, z2, mv/mvref, mv/mvref)

    kv=kvc= PolyLine(z1, z2, kv/kvref, kv/kvref)
...
class test_omega_phase(unittest.TestCase):
    ...
    def test_surcharge(self):
        ...
        a = Speccon1dVRC(self.reader %
            {'drn': drn, 'use_actual': True, 'load_to_test': load_to_test})
        b = Speccon1dVRC(self.reader %
...
def test_zhuandyin2012_drn0_kv_linear_mv_const():
    ...
    kv = kvc=PolyLine([0,1], [1,2])
    mv = PolyLine([0,1], [1,1])

    dTv = dTvc=kvref/mvref/H**2
...
#            plt.figure()
#            plt.plot(por, z,'b-*', label='expected')
#            plt.plot(a.pors, z, 'r-+', label='calculated')
#            plt.legend()
#
#
#            plt.figure()
...
def test_tang_and_onitsuka_vert_and_radial_well_resistance():
    ...
    dTvc = 0.0003613006824568446 #dTv=kwref/mvref/gamw/H**2
    dThc= 1e12
...
#            plt.show()
            ...
            assert_allclose(a.avp, avp, atol=0.2,
                            err_msg = ("Fail. test_tang_and_onitsuka_vert_and_radial, avp, "
                                "implementation='%s', dT=%s" % (impl, dT)))
            assert_allclose(a.pors, por, atol=0.2,
                            err_msg = ("Fail. test_tang_and_onitsuka_vert_and_radial, por, "
                                "implementation='%s', dT=%s" % (impl, dT)))
            assert_allclose(a.set, settle, atol=0.2,
                            err_msg = ("Fail. test_tang_and_onitsuka_vert_and_radial, settle, "
                                "implementation='%s', dT=%s" % (impl, dT)))
...
kvc = PolyLine([0, 0.5, 0.5, 1.0], [1, 1, 0,0])
mv = PolyLine([0,1], [0.5,0.5])
kh = khc = PolyLine([0,1], [1,1])
kv = PolyLine([0,1], [5,5])
...
def test_luetal2010():
    """
    test against lu et al 2010. load is constant with depth

    """

    t = np.array([0.1, 0.8, 2.0])

    z = np.array([0, 3.0, 6.0, 9.0, 10.0])/10

    por = np.array(
      [[ 0.        ,  0.        ,  0.        ],
       [ 0.79767965,  0.18858878,  0.02875992],
       [ 0.87548262,  0.31227618,  0.05113408],
       [ 0.90569845,  0.36732228,  0.0623297 ],
       [ 0.90760761,  0.37098625,  0.06309929]])

    pors = np.array(
      [[ 0.        ,  0.        ,  0.        ],
       [ 0.80276198,  0.18945113,  0.02887799],
       [ 0.87888332,  0.31356551,  0.0513433 ],
       [ 0.90841281,  0.36875931,  0.06258414],
       [ 0.91027792,  0.37243242,  0.06335682]])

    porc = np.array(
      [[ 0.        ,  0.        ,  0.        ],
       [ 0.39109374,  0.11960056,  0.01931412],
       [ 0.60342626,  0.20913012,  0.03439659],
       [ 0.68854978,  0.25235955,  0.04197469],
       [ 0.69398298,  0.25529224,  0.04249651]])

    avp = np.array([[ 0.78694202,  0.24695875,  0.0402363 ]])

    settle = np.array([[ 0.06163463,  0.21784408,  0.27764593]])
...
#
#            plt.figure()
#            plt.plot(pors, z,'b-*', label='expected')
#            plt.plot(a.pors, z, 'r-+', label='calculated')
#            plt.legend()
#
#            plt.figure()
#            plt.plot(porc, z,'b-*', label='expected')
#            plt.plot(a.porc, z, 'r-+', label='calculated')
#            plt.legend()
#
#            plt.figure()
#            plt.plot(t,settle[0],'b-*', label='expected')
...
#            plt.show()
            ...
            assert_allclose(a.avp, avp, atol=1e-5,
                            err_msg = ("Fail. luetal2010, avp, "
                                "implementation='%s', dT=%s" % (impl, dT)))
            assert_allclose(a.por, por, atol=1e-5,
                            err_msg = ("Fail. luetal2010, por, "
                                "implementation='%s', dT=%s" % (impl, dT)))
            assert_allclose(a.pors, pors, atol=1e-5,
                            err_msg = ("Fail. luetal2010, por, "
                                "implementation='%s', dT=%s" % (impl, dT)))
            assert_allclose(a.porc, porc, atol=1e-5,
                            err_msg = ("Fail. luetal2010, por, "
                                "implementation='%s', dT=%s" % (impl, dT)))
            assert_allclose(a.set, settle, atol=1e-5,
                            err_msg = ("Fail. luetal2010, settle, "
                                "implementation='%s', dT=%s" % (impl, dT)))

def test_luetal2010_linear_depth_load():
    """
    test against lu et al 2010. load is linear with depth
    ...
    por = np.array(
      [[ 0.        ,  0.        ,  0.        ],
       [ 0.52853487,  0.09413123,  0.01311388],
       [ 0.41705324,  0.14377135,  0.0232476 ],
       [ 0.28573816,  0.16009769,  0.02827924],
       [ 0.26598743,  0.1610165 ,  0.02862395]])

    pors = np.array(
      [[ 0.        ,  0.        ,  0.        ],
       [ 0.53252015,  0.09460952,  0.013168  ],
       [ 0.41875919,  0.1443755 ,  0.0233428 ],
       [ 0.28571473,  0.16068201,  0.02839443],
       [ 0.26572287,  0.16159755,  0.02874049]])


    porc = np.array(
      [[ 0.        ,  0.        ,  0.        ],
       [ 0.20971236,  0.05586808,  0.00878429],
       [ 0.28057743,  0.09543921,  0.0156313 ],
       [ 0.28761234,  0.11335233,  0.01906416],
       [ 0.28715249,  0.11453272,  0.01930032]])


    avp = np.array([[ 0.40487972,  0.11402353,  0.01829196]])

    settle = np.array([[ 0.04198122,  0.12612176,  0.15381554]])
