...
{%- if SSL_CERT and SSL_KEY %}
    ...
    listen 443 ssl;
    ssl_certificate {{ DIRECTORY }}/{{ SSL_CERT }};
    ssl_certificate_key {{ DIRECTORY }}/{{ SSL_KEY }};
...
def get_deploy_config(directory):
    ...
    if not DEPLOY_CONFIG:
        yaml_file = directory + "/" + CONFIG_FILE
    ...
    def deploy_web(self):
        if "web" in self.config:
            for site in self.config["web"]:
                ...
                allow_www = True if "allow_www" in nginx and nginx["allow_www"] is True else False
...
def cmd():
    try:
        ...
        if arg.git_push_deploy:
            ...
            _print("> Set Git Push Deploy")
            git.update_post_receive_hook(repo, True)

        if arg.git_no_push_deploy:
            repo = arg.git_push_deploy
