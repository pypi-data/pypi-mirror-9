...
VIRTUALENV = None
VERBOSE = False
VIRTUALENV_DIRECTORY = "%s/.virtualenvs" % PY_USER
...
{% if MAINTENANCE["ACTIVE"] %}
    ...
    {% set maintenance_page = "/etc/deployapp_maintenance.html" %}
    {% if MAINTENANCE["PAGE"] %}
...
def nginx_reload():
    run("service nginx reload; service php-fpm reload")
...
class App(object):
    ...
    def deploy_web(self, undeploy=False, _maintenance_on=False):
        """
        ...
        if "web" in self.config:
            for site in self.config["web"]:
                ...
                if "maintenance" in site:
                    maintenance.update(site["maintenance"])
                elif _maintenance_on:
                    maintenance.update({"active": True})
                ...
                if application:
                    ...
                    if (not maintenance["active"]) \
                            or (maintenance["active"] and maintenance["allow_ips"]):
                ...
                with open(nginx_config_file, "wb") as f:
                    context = dict(NAME=name,
                                   ...
                                   MAINTENANCE={
                                       ...
                                       "ALLOW_IPS": maintenance.get("allow_ips", [])}
                                   )
...
def cmd():
    try:
        ...
        parser.add_argument("-m", "--maintenance", help="Values: on|off - To set the site on maintence. ie [--maintenance on]")
...
<body>
    ...
    maintenance_page = "/var/deployapp_maintenance.html"
    install_pkgs = ["nginx", "php-fpm"]
    upstarts = ["nginx", "php-fpm", "supervisord"]

    if _dist == "RHEL":
        install_pkgs.extend(['groupinstall "Development Tools"', "python-devel"])
    elif _dist == "DEBIAN":
        install_pkgs.extend(["clamav*", "python-dev"])
