...
class Repositorio(object):
    ...
    def _obter(self, **filtros):
        ...
        try:
            ...
            raise InstanciaNaoExiste(u'Não foi encontrada instância de {} com {}.'.format(self.classe_model.__name__, ', '.join(['{}={}'.format(chave, filtros[chave]) for chave in filtros])))
        except self.classe_model.MultipleObjectsReturned:
            raise MaisDeUmaInstanciaEncontrada(u'Mais de uma instância de {} foi retornada com {}.'.format(self.classe_model.__name__, ', '.join(['{}={}'.format(chave, filtros[chave]) for chave in filtros])))
