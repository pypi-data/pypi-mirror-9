...
class ResultadoDeApi(object):
    ...
    def resposta(cls, conteudo, nome_api, versao, status=200, excecao=None):
        ...
        if type(conteudo) is not dict:
            conteudo = {'conteudo': conteudo}
        metadados = {
            'api': nome_api,
            'versao': versao,
            'resultado': cls._resultados[status],
        }
        conteudo = conteudo
        if excecao:
            ...
            conteudo.update({'excecao': {'mensagem': str(excecao), 'stack_trace': stack_trace}})
        resultado = {
            'metadados': metadados,
            cls._resultados[status]: conteudo
        }
