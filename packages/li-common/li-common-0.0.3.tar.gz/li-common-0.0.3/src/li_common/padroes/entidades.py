...
from li_common.padroes.serializacao import Formatador
...
class Entidade(object):
    ...
    def _membro_nao_eh_serializavel(self, membro):
        """
        ...
        :rtype: bool
        """
        atributo, valor = membro
        if atributo == 'formatador':
            return True
        if atributo.startswith("_"):
            return True
        if inspect.ismethod(valor):
            return True
        return False
    ...
    def _extrai_membros(self):
        ...
        :rtype: list
        """
        for membro in inspect.getmembers(self):
            if self._membro_nao_eh_serializavel(membro):
                continue
            atributo, valor = membro
            yield atributo, valor

    def _processa_valor_de_atributo(self, valor, metodo, formato=None, **argumentos_do_metodo):
        """
        ...
        :type formato: str
        :param argumentos_do_metodo: kwargs a ser passado para o método de serialização.
        :return: Retorna o valor do atributo processado.
        :rtype: object
        """
        if issubclass(valor.__class__, Entidade):
            return getattr(valor, metodo)(**argumentos_do_metodo)
        if formato:
    ...
    def _membro_to_dict(self, atributo, valor):
        ...
        :rtype: dict
        """
        valor = self._processa_valor_de_atributo(valor, 'to_dict')
        if type(valor) is list and valor:
            valor = [self._processa_valor_de_atributo(valor, 'to_dict') for valor in valor]
    ...
    def _monta_parametros(self, chave):
        ...
        for parametro in self._parametros_para_serializacao[chave]:
            parametros.append('{}="{}"'.format(parametro, self._parametros_para_serializacao[chave][parametro]))
    ...
    def _membro_to_xml(self, atributo, valor):
        ...
        :rtype: str
        """
        valor = self._processa_valor_de_atributo(valor, 'to_xml')
        if type(valor) is list and valor:
            lista = []
            for valor_na_lista in valor:
                lista.append(self._processa_valor_de_atributo(valor_na_lista, 'to_xml', formato="<item>{}</item>", raiz=True))
            valor = "".join(lista)
