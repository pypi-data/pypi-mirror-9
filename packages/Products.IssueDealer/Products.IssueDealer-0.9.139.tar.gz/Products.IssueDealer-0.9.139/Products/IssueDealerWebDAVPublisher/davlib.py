...
if __name__ == '__main__' and None:
  headers = {'AUTHORIZATION': 'Basic %s' % \
             string.replace(base64.encodestring("%s:%s" % ('username', 'password')), "\012", "")}
  connection = DAV('localhost', port=8080)
  response = connection.mkcol('/test', extra_hdrs=headers)
  print response.msg
  #connection.close()
  #connection = DAV('localhost', port=8080)
  response = connection.put('/test/test', 'This is a test', extra_hdrs=headers)
  print response.msg
  connection.close()
