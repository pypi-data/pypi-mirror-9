...
##
"""Tests for student payments.

"""

from zope.interface import verify
from waeup.kofa.testing import FunctionalLayer, FunctionalTestCase
from waeup.kofa.students.payments import (
    StudentOnlinePayment, StudentOnlinePaymentFactory, Payer)
...
class StudentOnlinePaymentFactoryTest(FunctionalTestCase):

    layer = FunctionalLayer

    def setUp(self):
        super(StudentOnlinePaymentFactoryTest, self).setUp()
        self.factory = StudentOnlinePaymentFactory()
        return

    def tearDown(self):
        super(StudentOnlinePaymentFactoryTest, self).tearDown()
        return

    def test_factory(self):
        obj = self.factory()
        assert isinstance(obj, StudentOnlinePayment)

    def test_getInterfaces(self):
        implemented_by = self.factory.getInterfaces()
        assert implemented_by.isOrExtends(IOnlinePayment)
        assert implemented_by.isOrExtends(IStudentNavigation)
