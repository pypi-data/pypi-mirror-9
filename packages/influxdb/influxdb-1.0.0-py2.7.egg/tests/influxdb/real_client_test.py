...
class InfluxDbInstance(object):
    ...
    def __init__(self, conf_template):
        ...
        self.influxd_temp_dir_base = tempfile.mkdtemp()
        # "temp_dir_base" will be used for conf file and logs,
        # while "temp_dir_influxdb" is for the databases files/dirs :
        self.influxd_temp_dir_influxdb = tempfile.mkdtemp(
            dir=self.influxd_temp_dir_base)
        # we need some "free" ports :
        self.influxd_broker_port = get_free_port()
        self.influxd_admin_port = get_free_port()
        # as it's UDP we can reuse the same port than the broker:
        self.influxd_udp_port = get_free_port()

        self.logs_file = os.path.join(
            self.influxd_temp_dir_base, 'logs.txt')

        with open(conf_template) as fh:
            conf = fh.read().format(
                broker_port=self.influxd_broker_port,
                admin_port=self.influxd_admin_port,
                udp_port=self.influxd_udp_port,
                broker_raft_dir=os.path.join(
                    self.influxd_temp_dir_influxdb, 'raft'),
                broker_node_dir=os.path.join(
                    self.influxd_temp_dir_influxdb, 'db'),
                influxdb_cluster_dir=os.path.join(
                    self.influxd_temp_dir_influxdb, 'state'),
                influxdb_logfile=self.logs_file
            )

        conf_file = os.path.join(self.influxd_temp_dir_base, 'influxdb.conf')
        with open(conf_file, "w") as fh:
            fh.write(conf)

        # now start the server instance:
        proc = self.influxdb_proc = subprocess.Popen(
            [influxdb_bin_path, '-config', conf_file],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        print("Started influxdb bin in %r with ports %s and %s.." % (
              self.influxd_temp_dir_base,
              self.influxd_admin_port, self.influxd_broker_port),
              file=sys.stderr)
        ...
        try:
            while time.time() < timeout:
                if (is_port_open(self.influxd_broker_port)
                        and is_port_open(self.influxd_admin_port)):
                    break
                time.sleep(0.5)
            ...
            data['reason'] = str(err)
            raise RuntimeError("%(reason)s. RC=%(rc)s\n"
                               "stdout=%(out)r\nstderr=%(err)r\nlogs=%(logs)r"
                               % data)

    def get_logs_and_output(self):
        proc = self.influxdb_proc
    ...
    def close(self, remove_tree=True):
        self.influxdb_proc.terminate()
        self.influxdb_proc.wait()
        if remove_tree:
            shutil.rmtree(self.influxd_temp_dir_base)


class TestClientWithNoServer(unittest.TestCase):
    """ Tests which don't require a server instance running """
    ...
    def test_write_points_bad_precision(self):
        ...
        with self.assertRaisesRegexp(
            ValueError,
            "Invalid time precision is given. \(use 'n', 'u', 'ms', 's', 'm'"
            " or 'h'\)"
        ):
            cli.write_points(
                dummy_point,
...
class InfluxDbClientTestWithServerInstanceMixin(object):
    ...
    def setUp(self):
        ...
        self.cli = InfluxDBClient('localhost',
                                  self.influxd_inst.influxd_broker_port,
                                  'root', '', database='db')
...
class UdpTests(InfluxDbClientTestWithServerInstanceMixin,
    ...
    def test_write_points_udp(self):
        ...
        cli = InfluxDBClient(
            'localhost', self.influxd_inst.influxd_broker_port,
            'dont', 'care',
            database='db',
            use_udp=True, udp_port=self.influxd_inst.influxd_udp_port
        )
