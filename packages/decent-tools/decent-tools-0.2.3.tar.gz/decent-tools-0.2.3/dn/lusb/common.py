...
def print_motes():
    ...
    if not args.c:
        ...
        maxes = [max([len(s) for s in col + [title]]
                     ) for title, col in zip(titles, cols)]
        title_line = [title.center(m) for title, m in zip(titles, maxes)]

        print(' '.join(title_line))
        print(' '.join(['-' * m for m in maxes]))
        for row in zip(*cols):
            row_line = [item.ljust(m) for item, m in zip(row, maxes)]
            print(' '.join(row_line))
    else:
        for row in zip(*cols):
            print(','.join(row))
