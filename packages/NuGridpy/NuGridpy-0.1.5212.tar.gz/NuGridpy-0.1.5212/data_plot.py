...
class DataPlot():
    ...
    def plot_isoratios(self,xiso,yiso,fign=1,spec=None,deltax=True,deltay=True,logx=False,logy=False,
                        ...
                        plt_sparse=1,plt_symb='o',plt_col='k',plt_lt='-',plt_lw=1.,plt_massrange=False,plt_show=True,
                        figsave=False):
        '''
            ...
            if errbar:
                if xdataerr != None or ydataerr != None:
                    pl.errorbar(xdata[i],ydata[i],xerr=xdataerr[i],yerr=ydataerr[i],marker=style[i][0],
                                color=style[i][1],linestyle='',lw=2,markevery=plt_sparse)
            pl.plot(xdata[i],ydata[i],style[i][0],c=style[i][1],mfc=style[i][2],ms=float(style[i][3]),
                    mew=float(style[i][4]),label=style[i][5],markevery=plt_sparse,linewidth=plt_lw)
...
###############     print labelsx[bbb]
    ...
    def elemental_abund(self,cycle,zrange=[1,15],ylim=[-12,0],title_items=None,
                        ref=0,solar_filename='',label='',show_names=True,
                        colour='',**kwargs):
        ''' 
        ...
        if plotType=='PPN':
            ...
            pl.plot(z_el[zmin_ind:zmax_ind],np.log10(el_abu),'o-')
            j=0        # add labels
            for z in z_el[zmin_ind:zmax_ind]:
    ...
    def abu_profile(self,ixaxis='mass',isos=None,ifig=None,fname=None,logy=False):
        ...
        for i in range(len(risos)):
            if logy:
                ...
                y = abunds[i]
            pl.plot(x,y,u.linestyle(i)[0],markevery=u.linestyle(i)[1]*10,
                    label=names[i],markeredgecolor='None')
