...
class RepositoryNotificationTC(HgHelperTC):
    ...
    def test_repo_nosy_list(self):
        ...
        self.hgrepo.commit('refresh', user='Cornelius', date='2012-01-01 00:00 +0300')
        self.refresh()
        self.set_description('new patch notification')
        self.assertEqual(len(MAILBOX), 1)
        yield self._check_new_patch_notification, MAILBOX[0]
    ...
    def _check_new_patch_notification(self, mail):
        self.assertEqual(mail.recipients, ['zephir@monke.ys'])
        self.assertEqual(mail.subject,
                         '[mercurial repository #1084] new patch: refresh')
        self.assertMultiLineEqual(
            mail.message.get_payload(decode=True),
            '\n'.join([
                'url: http://testing.fr/cubicweb/patch/1088',
                '',
                '# HG changeset patch',
                '# User Cornelius',
                '# Date 1325365200 -10800',
                '#      Sun Jan 01 00:00:00 2012 +0300',
                '# Node ID bc2d1a04e9b830e21c16e0e9ad47e046bc912c29',
...
class PatchNotificationTC(HGRCMixin, CubicWebTC):
    ...
    def test_task_notification(self):
        with self.new_access('Victor').client_cnx() as cnx:
            ...
            task = cnx.create_entity('Task', title=u'todo', reverse_has_activity=patch)
            cnx.commit()
            assert len(MAILBOX) == 1, MAILBOX
            self.assertEmailEqual(
                MAILBOX[0],
                '[Jungle] patch task: Add more elephants children - todo',
                'victor@ratax.es',
                u'\n'.join([u'', u'todo\n\n\n',
                            u'url: {}'.format(patch.absolute_url())]))
            MAILBOX[:] = []
            point = cnx.create_entity('InsertionPoint', lid=123, point_of=rev)
            task = cnx.create_entity('Task', title=u'needs work!', reverse_has_activity=point)
            cnx.commit()
            self.assertEmailEqual(
                MAILBOX[0],
                '[Jungle] patch task: Add more elephants children - needs work!',
                'victor@ratax.es',
                u'\n'.join([u'', u'needs work!\n\n\n',
                            u'url: {}#navcontentbottom{}'.format(
                                rev.absolute_url(), point.eid)]))
