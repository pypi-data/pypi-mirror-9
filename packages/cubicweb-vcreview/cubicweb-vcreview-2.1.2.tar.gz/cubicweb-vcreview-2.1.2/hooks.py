...
def rewind_state(patch):
    """Restore the state of an 'outdated' patch (if possible) to the state it
    was prior to the 'obsolete' transition.
    """
    wfable = patch.cw_adapt_to('IWorkflowable')
    trinfo = wfable.latest_trinfo()
    oldstate = trinfo.from_state[0]
    for tr in wfable.possible_transitions():
        if tr.destination_state[0] == oldstate:
            wfable.fire_transition(tr.name)
            break
...
class PatchLifeCheckOp(hook.DataOperationMixIn, hook.SingleLastOperation):
    ...
    def compare_with_ancestor(self, rev, patch_state):
        ...
        """
        # XXX Why do we need this guard?
        if patch_state in ('in-progress', 'pending-review', 'reviewed', 'outdated'):
