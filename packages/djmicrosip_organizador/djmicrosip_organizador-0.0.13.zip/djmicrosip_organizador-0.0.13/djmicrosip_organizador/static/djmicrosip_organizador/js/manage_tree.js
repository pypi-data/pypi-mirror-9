...
function cargar_tree(create){
	...
	$.ajax({
		...
		success: function(data){
			...
			{
				$parent = $("#demo2").parent();
				$parent.parent().find(".modal-header").html('<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button><h3>Selecciona una carpeta</h3>');
				$parent.html("<div id='demo2'></div>");
				cargar_arbol(data.data,$("#demo2"));
				$parent.parent().find(".modal-header").append("<br><button type='button' class='btn btn-default' data-dismiss='modal'>Cancelar</button><a href='#' class='btn btn-default' id='enviar' data-dismiss='modal'>Mover</a>");
				$("#enviar").on('click',move_articles);
