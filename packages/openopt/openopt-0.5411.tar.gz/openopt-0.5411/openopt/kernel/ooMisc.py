...
def Len(arg):
    ...
    if type(arg) in [int, float]:
        return 1
    elif arg is None or (isinstance(arg, (list, tuple)) and len(arg) == 0) \
    or (isinstance(arg, ndarray) and arg.size==1 and arg == array(None, dtype=object)):
        return 0
    else:
        return len(arg)
...
def LinConst2WholeRepr(p):
    ...
    """
    if p.A is None and p.Aeq is None:
        return
