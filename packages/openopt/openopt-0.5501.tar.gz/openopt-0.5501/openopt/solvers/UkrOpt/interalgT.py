...
splitDomainForDiscreteVariable
...
def adjustDiscreteVarBounds(y, e, p):
    ...
    for i in p._discreteVarsNumList:
        ...
        if 0:
            ...
            #new
#            from interalgMisc import hasPoint
#            P = np.array([-1.02000003,  1.0618039 , -0.5       , -0.5       ,  8.99999843,
#                8.8752538 ])
#            print '-----'
#            assert hasPoint(y, e, P)
#            if hasPoint(y, e, P): print '+'
            y[:, i], e[:, i] = splitDomainForDiscreteVariable(y[:, i], e[:, i], v)
#            print hasPoint(y, e, P)
#            if not hasPoint(y, e, P):
#                print p.iter
#                assert 0
