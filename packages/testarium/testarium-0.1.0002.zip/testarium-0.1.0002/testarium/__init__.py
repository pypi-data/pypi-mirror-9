...
experiment = experiment.Experiment(testarium)

def run(args): 
	if args.config_path: config_path = args.config_path # use user config
...
def print_commits(commits, args):
	...
	for commit in reversed(commits):
		if best == commit and len(commits) > 1: log('COLOR.GREEN', commit)
		else: log(commit)
		
		if args.print_config: log_lines(commit.config.__str__(args.config_keys)); log_simple('') # print config
		if args.print_diff: print_diff(commit, prev_commit, 'this', 'prev', args.config_keys); log_simple('') # print incremental diff
		prev_commit = commit
...
def mail(args):
	...
	try: log('SMTP server:', c['mail.smtp.server'] +':'+ str(c['mail.smtp.port']))
	except: log('COLOR.RED', 'No SMTP server [or error occurred]')

	try: 
		if c['mail.proxy.server']: log('HTTP proxy:', c['mail.proxy.server']+':'+ str(c['mail.proxy.port']))
		else: raise 'No proxy server'
	except: log('No proxy [or error occurred]')
		
	autotime = c['mail.autoreport.time']
	if autotime>0: log('Autoreports: send report if experiment duration >', autotime, 'sec')
	else: log('No autoreports [or error occurred]')
...
def main():
	...
	# run 
	parser_run.add_argument('config_path',	default='', nargs='?',				help='config path to use in the commit; using default branch config if option is empty')
	parser_run.add_argument('-c',			default='', dest='comment',			help='add comment to the commit')
	parser_run.add_argument('-p',			default='', dest='newParams',		help='json dictionary, used instead of parameters in current config')
	parser_run.add_argument('--mail',		default=False, dest='mail', action='store_true', help='send report to mail')
	
	# branch
	parser_branch.add_argument('name',			default='', nargs='?',			help='change branch to branche "name" (or create if need)')
	parser_branch.add_argument('config_path',	default='', nargs='?',			help='set default branch config (using when change or create branch)')
	parser_branch.add_argument('--cfg',			default='', dest='cfg_path',	help='set default config for current the branch')
	
	# delete
	parser_delete.add_argument('name',		default='', 	nargs='?',			help="name of commit. Use 'best' for the best scored commit. 0 is last, -1 is first commit")
	parser_delete.add_argument('--branch',	default='',		dest='branch',		help='name of branch, stay it empty to use active branch')
	
	# diff
	parser_differ.add_argument('nameA',		default='best', nargs='?',			help="name of commit to diff to. Use 'best' for the best scored commit. 0 is last, -1 is first commit")
	parser_differ.add_argument('nameB',		default='',		nargs='?',			help="name of commit to diff with. Use 'best' for the best scored commit. 0 is last, -1 is first commit")
	parser_differ.add_argument('--branch',	default='',		dest='branch',		help='name of common branch to operate, stay it empty to use active branch')
	...
	parser_differ.add_argument('-k', 		default=[],		dest='config_keys', nargs='+',	help='config keys to use for check difference')
	
	# log
	parser_log.add_argument('name',		default='', 	nargs='?',										help="name of commit to display. Use 'best' for the best scored commit. 0 is last, -1 is first commit")
	parser_log.add_argument('-i',		default=False,	dest='print_diff',	action='store_true',		help='print incremental difference between the commit configs')
	...
	parser_log.add_argument('--branch',	default='',		dest='branch',									help='name of branch to be shown, stay it empty to use active branch')
	...
	parser_where.add_argument('-i',		default=False,	dest='print_diff',	 action='store_true',	help='print incremental difference between the commit configs')
	...
	parser_mail.add_argument('--smtp',		default=None, 	dest='smtp', nargs=2,		help='"smtp_server smtp_port", by default here is gmail smtp (TLS)')
	parser_mail.add_argument('--proxy',		default=None, 	dest='proxy',nargs=2,		help='proxy (http) for sending email, eg.: "192.168.10.1 8080"')
	parser_mail.add_argument('--auto',		default=-1,		dest='time',				help='send autoreports if experiment duration is higher specified time (in seconds), set -1 to disable it')
	parser_mail.add_argument('--test',		default=False,	dest='test', action='store_true',	help='send testing email')
	parser_mail.add_argument('--reset',		default=False,	dest='reset', action='store_true',	help='reset mail settings (account, password and others)')
	
	
	if len(sys.argv) == 1: args = parser.parse_args(['run'])
