...
class Experiment:
	...
	def Search(self, config, comment, newParams, useTry):
		...
		if max_variants == 1:
			...
			beginTime = time.time()
			newParams = { key:(newParams[key][0] if hasattr(newParams[key], '__iter__') else newParams[key]) for key in newParams}
		...
		# perform search 
			...
			newParams = { key:(newParams[key] if hasattr(newParams[key], '__iter__') else [ newParams[key] ]) for key in newParams}
			...
			if best_commit: log('COLOR.GREEN', best_commit)
			else: log('No best commit')
			...
			autotime = try_get(self.testarium.config, 'mail.autoreport.time', -1)
			if duration>autotime and autotime>=0: self.sendmail = True
			text = makehtml(commits2html('Best experiment', [(best_commit,True)]) + commits2html('All the experiment reports', results))
	...
	def Run(self, config, comment, newParams, useTry):
		...
		if self.user_score is None: 
			log('COLOR.RED', 'Error: User Score function is not described. Use decorator @Experiment.set_score to set it')
			return None, False
