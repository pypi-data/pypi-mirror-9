...
def UrlGraph(string):
	if string[0:6] == 'graph:': return 'storage/'+string[6:]
	else: return ''
	
# check and return graph url
def UrlFile(string):
	if string[0:5] == 'file:': return 'storage/'+string[5:]
	else: return ''

# time to str
def strtime(t): 
	if t<10: return '0' + str(t)
	else: return str(t)
...
def log(*msg):
	...
	sys.stdout.write('\n')
	if colored: sys.stdout.write(colorama.Fore.RESET + colorama.Back.RESET + colorama.Style.RESET_ALL)
...
def commits2html(header, results):
	...
	for c in results:
		...
		cols, out = c[0].Print()
		body += '<tr style="'+('background:#EEE;' if count%2==0 else '') +  ('color:#A00' if not out else '')  + '">'
		if not out: out = [c[0].name, 'failed commit', c[0].desc['params']]
		else: trueCols = cols
