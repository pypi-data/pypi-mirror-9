...
class CloudCIXAuthMethod(AuthMethod):
    ...
    def get_auth_data(self, session, auth, headers, **kwargs):
        ...
        if self.idMember:
            auth_data['idMember'] = six.text_type(self.idMember)
        return 'cloudcix_auth', auth_data
...
class CloudCIXAuth(Auth):
    ...
    def get_auth_ref(self, session, **kwargs):
        ...
        try:
            ...
            try:
                resp = e.response.json()['error']['identity']['cloudcix_auth']
    ...
    def select_account(self, idMember):
        assert six.text_type(idMember) in [m['idMember'] for m in self.members]
        self.idMember = str(idMember)
        self._auth_method.token_id = self.token_id
        self._auth_method.idMember = six.text_type(idMember)
