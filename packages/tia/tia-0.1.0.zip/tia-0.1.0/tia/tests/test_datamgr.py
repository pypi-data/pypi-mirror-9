...
class MockDataManager(object):
    ...
    def get_attributes(self, sids, flds, **overrides):
        sids = [sids] if isinstance(sids, basestring) else sids
        flds = [flds] if isinstance(flds, basestring) else flds
        self.access_cnt += 1
...
class TestDataManager(unittest.TestCase):
    ...
    def test_cache_sub(self):
        ...
        for i, (sid, fld) in enumerate([('SID1', 'FLDA'), ('SID2', 'FLDB'), ('SID3', 'FLDC')]):
            ...
            self.assertEquals(i+1, self.dm.access_cnt)
        ...
        self.assertEquals(6, self.dm.access_cnt)
