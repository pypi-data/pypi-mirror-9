...
class DBInterface(object):
    ...
    def get_map_fields(self, tablename):
        ...
        return (
            fmap[tablename] if isinstance(tablename, basestring) else
                list(OrderedDict(
                (fieldname, None)
    ...
    def get_tablespecs(cls, tablename):
        return (
            cls.db_structure[tablename] if isinstance(tablename, basestring) else
            list(OrderedDict(
                (fieldspec, None)
                for table in tablename
    ...
    def get_tuple(cls, tablename, untyped=False):
        if isinstance(tablename, unicode):
            tablename = str(tablename)  # need this because unicode class names aren't allowed
        tuple_name = tablename if isinstance(tablename, str) else str('_'.join(tablename))
        if untyped:
    ...
    def _get_tablespec(self, tablename, include_nomatch):
        return (
            tablename if isinstance(tablename, basestring) else
