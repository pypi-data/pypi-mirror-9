...
#if PY_VERSION_HEX > 0x03000000
static struct PyModuleDef _camodule = {
	PyModuleDef_HEAD_INIT,
	"_ca",
	"A python module for EPICS Channel Access. a la KEK,Japan.",
	-1,
	CA_Methods,
	NULL,
	NULL,
...
PyMODINIT_FUNC PyInit__ca(void){ return _init_ca(); } 
#endif
 		     ...
 		     PyThreadState *tstate, purgebleT purgeble)
{
  ...
  if(DEBUG)
    PyErr_WarnEx(NULL, "_ca_frame:create frame", 2);
...
#if PY_VERSION_HEX > 0x03000000
    m = PyModule_Create(&_camodule);
#else
    m = Py_InitModule("_ca", CA_Methods);
#endif
    if (m == NULL){
      PyErr_WarnEx(NULL, "_ca_init:failed to initialise module", 2);
      return NULL;
    }
...
#if defined(PYCA_HG_RELEASE)
    obj=PyString_FromString(PYCA_HG_RELEASE);
#else
    obj=PyString_FromString("unknown");
#endif
    PyDict_SetItemString(d, "release", obj);
    PyDict_SetItemString(d, "hg_release", obj);
    Py_DECREF(obj);                            
  ...
  if (status == ECA_NORMAL){
	...
	if (type == -1){
	  type = dbf_type_to_DBR_TIME(ca_field_type(ch_id));
        ...
        /*Version EPICS_VERSION_STRING EPICS_VERSION EPICS_REVISION */
...
#define MoveDBRtoValue(DBRTYPE, VP, PYTYPE, FORMAT) \
     {\
       ...
       register unsigned long count=cval.count;\
...
#endif
	      PyErr_SetString(CaError, "Exception in ca_get_callback.\n");
	...
	try{
	  if ((pframe->pfunc != Py_None)){ 
	    if (pframe->tstate) {
	      ...
	      oldtstate=NULL;
	      PyErr_WarnEx(NULL, "Empty thread state to recover in exec-callback\n",1);
		...
		result=NULL;
	      ...
	      result=NULL;
	      //PyErr_WarnEx(NULL, "exception while pend_event \n",1);
	      PyErr_SetString(CaError, "Exception in pend_event\n");
	      //errlogPrintf("exception while pend_event \n");
	    }
#if defined(DEBUG) && DEBUG
	  ...
	  PyErr_SetString(CaError, "Exception in get/put_callback.\n");
	  //PyErr_WarnEx(NULL, "get/put callback un-captured exeption.\n",1);
...
# if (PY_MAJOR_VERSION == 2 && PY_MINOR_VERSION > 6) || (PY_MAJOR_VERSION == 3 && PY_MINOR_VERSION > 1) || (PY_MAJOR_VERSION > 3)
    rval=PyCapsule_New(pframe, NULL, NULL);
#   else
...
# if (PY_MAJOR_VERSION == 2 && PY_MINOR_VERSION > 6) || (PY_MAJOR_VERSION == 3 && PY_MINOR_VERSION > 1) || (PY_MAJOR_VERSION > 3)
  pframe = (_ca_frame *) PyCapsule_GetPointer(obj,NULL);
# else
...
static PyObject *Py_ca_channelInfo(PyObject *self, PyObject *args){
  ...
  //PySys_WriteStderr("get channel info\n");
  //PyErr_WarnEx(NULL,"get channel info",2);
  Py_BEGIN_ALLOW_THREADS{
    ENTER_CA{
      pyca_field_type=ca_field_type(ch_id);
      ...
      ca_get_host_name(ch_id, host_name_buf, blen);
      //errlogPrintf("channel info unpdated : %d\n",pyca_state);
    }LEAVE_CA;
  }Py_END_ALLOW_THREADS;
  //PyErr_WarnEx(NULL,"got channel info",2);
