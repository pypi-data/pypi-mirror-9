...
# Signals
mkthumb = Signal(providing_args=[])
mediasync = Signal(providing_args=[])


def mkthumb_handler(sender, **kwargs):
    """signal intercept for mkthumb"""
    call_command('mkthumb')
mkthumb.connect(mkthumb_handler)


def mediasync_handler(sender, **kwargs):
    """signal intercept for mediasync"""
    call_command('mediasync', storage='file', source=conf.MEDIASYNCDIR)
mediasync.connect(mediasync_handler)
