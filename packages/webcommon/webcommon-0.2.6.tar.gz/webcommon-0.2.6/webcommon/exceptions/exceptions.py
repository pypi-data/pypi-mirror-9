...
class InternalException(AppResourceException):
    ...
    problem and whether user should try it once more after some time or it
    is simply hopeless.
    """
...
class NotImplementedException(AppResourceException):
    """
    Raised when someone is trying to use a feature, which is meant to be
    implemented in future.

    Status code must be 501 to indicate Not Implemented.
    Message should be AFAP (as funny as possible). If you are not able to
    generate funny message ask your smarter colleague to do it and go tell
    CEO you are leaving the company, you worthless piece of shit. We don't
    like people like you around.
    """

    def __init__(self, message, status_code=501, data=None):
        """
        Creates NotImplementedException
        ...
        Args:
            ...
            status_code - HTTP status code that can be returned from resource
        """
        super(NotImplementedException, self).__init__(message, status_code, data)

def exception_handler(f, *args, **kwargs):
    """
    Handling function for requests.

    Note:
        If it only taken from handle_common_exceptions wrapper
        to be able to test it

    Args:
        f: function to be execute
    ...
    try:
        ...
        AppLogger.log_exception_as_warning()
        return json_failed(
        ...
        AppLogger.log_exception_as_error()
        return json_failed(
        ...
        return json_failed(
            'Operation timed out. It seems that backend server is too busy right now. Please try it again later.',
            status_code=503,
            )
...
def handle_common_exceptions(f):
    ...
    def decorated_function(*args, **kwargs):
        return exception_handler(f, *args, **kwargs)
