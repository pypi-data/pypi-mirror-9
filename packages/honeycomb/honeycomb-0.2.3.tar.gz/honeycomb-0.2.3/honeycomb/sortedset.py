...
class SortedSet(HoneyBean, collections.MutableSet):
    ...
    def _zrank(self, elem):
        return self.redis.zrank(self.key, self._pickle(elem))
    ...
    def chunk(self, limit, offset, last=False, withscore=True, interval=None):
        return self._chunk(limit, offset, withscore, last=last, pipe=self.redis, interval=interval)
    ...
    # 增加score 可选的闭开区间的参数.
    def _chunk(self, limit, offset, withscore, last=False, pipe=None, interval=None):
        redis = pipe if pipe is not None else self.redis
        if interval and isinstance(interval, (tuple, list)) \
                and len(interval) == 2:
            from_score, to_score = interval
