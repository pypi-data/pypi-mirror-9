...
def getcollector():
    ...
    if g_collector is None:
        ...
        a("double_types",
          "(PUBLIC|ENTITY|DOCTYPE|LINKTYPE|NOTATION|SYSTEM|DELEGATE)")
        a("double",
...
class XMLCatalog(Catalog):
    ...
    def parse(self):
        ...
        self.parent_map = dict((c, p)
                               for p in self.tree.getiterator() for c in p)
    ...
    def _handle_rewritesystem(self, node):
        self.rewritesystem[node.attrib.get("systemIdStartString")] = node.attrib.get(
            "rewritePrefix")
    ...
    def _handle_rewriteuri(self, node):
        self.rewriteuri[node.attrib.get("uriStartString")] = node.attrib.get(
            "rewritePrefix")
