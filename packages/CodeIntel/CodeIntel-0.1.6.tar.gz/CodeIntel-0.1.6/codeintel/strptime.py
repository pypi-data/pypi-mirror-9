...
class TimeObj:
    ...
    def gregToJulian(self):
        ...
        # To make result be same as what strftime would give.
    ...
    def dayWeek(self):
        ...
        m = self.month + 12 * a - 2
        day_week = (
            self.day + y + (y / 4) - (y / 100) + (y / 400) + ((31 * m) / 12)) % 7
        if day_week == 0:
            day_week = 6
        else:
            day_week = day_week - 1
    ...
    def FillInInfo(self):
        ...
        if (self.month == None or self.day == None) and self.year and self.julian_date:
            ...
            # Year tossed out since needed to do calculation in the first
            # place.
            self.month = gregorian[1]
...
class StrpObj(TimeObj):
    ...
    def DictAssembly(self, locale_setting):
        ...
        DIRECTIVEDict = {'%d': r'(?P<d>[0-3]\d)',  # Day of the month [01,31].
                         ...
                         # Week number of the year with Sunday as first day of
                         # the week [00,53].
                         '%U': r'(?P<U>[0-5]\d)',
                         '%w': r'(?P<w>[0-6])',  # Weekday [0(Sunday),6].
                         # Week number of the year with Monday as the first day
                         # of the week [00,53].
                         '%W': r'(?P<W>[0-5]\d)',
                         ...
                         # exists).
                         '%Z': r'(?P<Z>(\D+ Time)|([\S\D]{3,3}))',
                         # Literal "%".  Doesn't matter that group name is not
                         # same as directive since literal % will be ignored in
                         # the end.
                         '%%': r'(?P<percent>%)'
                         }
        ...
        if locale_setting == ENGLISH:
            ...
            DIRECTIVEDict[
                '%c'] = r'(?P<m>\d\d)/(?P<d>\d\d)/(?P<y>\d\d) (?P<H>\d\d):(?P<M>\d\d):(?P<S>\d\d)'
    ...
    def convert(self, string, format, locale_setting):
        ...
        DIRECTIVEDict, AM_PM, MONTHDict, DAYDict = self.DictAssembly(
            locale_setting)
