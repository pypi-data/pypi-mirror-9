...
def check_insert_rails_env(path, blob_scope):
            ...
            if role_parts[1] in ("controllers", "models"):
                if role_parts[1] == "controllers":
                    if role_parts[2] != "application.rb":
                        blob_scope.insert(
                            0, Element("import", module="./application", symbol='*'))
                    # For loading models
                    apath = abspath(path)
                    add_models = True
                ...
                apath = abspath(path)
                migration_dir = join(
                    dirname(dirname(dirname(apath))), "db", "migrate")
                ...
                for migration_file in migration_files:
                    ...
                    blob_class.insert(
                        idx, Element("import", module=base_part, symbol=modelName))
    elif (len(role_parts) > 2
    ...
    if add_models:
        ...
        for model_file in model_files:
            ...
            blob_scope.insert(
                idx, Element("import", module=base_part, symbol='*'))
