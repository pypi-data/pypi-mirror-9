...
class RubyCommonClassifier:
    ...
    def quote_patterns_cb_aux(self, tok):
        ...
        if tval[0] == '"':
            ...
            # Fallback
            return self._quote_patterns[ScintillaConstants.SCE_RB_DEFAULT]
...
class Parser:
            ...
            if len(toks_to_return) < 3:
                ...
                if self._at_end_expression(tok3):
                    ...
                    if self._at_end_expression(tok4):
                        var_citdl = ''.join([tok['text']
                                             for tok in tok_list[:-2]])
        ...
        if builtin_type:
            ...
            if self._at_end_expression(tok2):
                if self.classifier.is_number(tok):
                    ...
                    type1 = "String"
                update_collection(
                    collectionA[-1], var_name, start_line, builtin_type)
                toks = [tok2]
            else:
    ...
    def _finish_list(self, collectionA, var_name, start_line, orig_tok, end_op, class_name):
        ...
        update_collection(
            collectionA[-1], var_name, start_line, class_name_final)
        return skipped_toks
        ...
        while 1:
            ...
            if style == self.classifier.style_word:
                if text in ["module", "class", "def"]:
                    ...
                    new_node = node_class(
                        nm_token[0], tok['start_line'], nm_token[0] == "initialize")
                    if is_class_method:
                        ...
                        update_collection(
                            new_node.local_vars, 'self', tok['start_line'], curr_node.name)
                    ...
                    if new_name and existing_name:
                        update_collection(self.containers[
                                          VAR_KIND_ALIAS][-1], new_name, new_tok['start_line'], existing_name)
                ...
                if text == "require" or text == "load":
                    ...
                    nm_token = self.get_fully_qualified_name()
                    if nm_token[0]:
                        curr_node.includes.append(
                            Name_LineNum(nm_token[0], nm_token[1]))
                #@@@@ elif text == "base":
                    ...
                    if text in ['attr', 'attr_reader', 'attr_writer', 'attr_accessor']:
                        ...
                        self.parse_assignment(
                            self.containers[VAR_KIND_LOCAL], text, tok['start_line'])
            ...
            # Check for assignment to some kind of variable
                ...
                if narrow_style != VAR_KIND_UNKNOWN:
                    self.parse_assignment(
                        self.containers[narrow_style], tok['text'], tok['start_line'])
            # end if WORD block
