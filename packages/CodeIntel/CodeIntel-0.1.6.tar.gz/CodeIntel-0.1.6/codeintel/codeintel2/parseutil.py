...
# don't consider this a token that is to be considered a part of the
# grammar, like '\n'
SKIPTOK = 0x01
MAPTOK = 0x02  # use the token associated with the pattern when it matches
# execute the function associated with the pattern when it matches
EXECFN = 0x04
# if you match a single character and want its ascii value to be the token
USETXT = 0x08
...
class recollector:
    ...
    def add(self, name, reg, mods=None):
        ...
        if mods:
            ...
            self.res[name] = re.compile(self.regs[name], mods)
        else:
            # check that it is valid
            self.res[name] = re.compile(self.regs[name])
