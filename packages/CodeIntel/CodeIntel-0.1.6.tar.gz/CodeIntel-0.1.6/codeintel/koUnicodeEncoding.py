...
def autoDetectEncoding(buffer,
    ...
    if len(buffer) > 2:
        ...
        if tryModeline:
            ...
            if match:
                ...
                # instead.  So if we fail on that kind of name, lets try the
                # alternate
                if decodedBuffer:
    ...
    # since what we want or our default both failed, if we have an ascii
    # buffer, return it
    if asciiBuffer:
...
def autoDetectXMLEncoding(buffer):
    ...
    if bom:
        ...
        # Without a BOM or xml decl, there is no basis for a guess
        return (None, None, '')
