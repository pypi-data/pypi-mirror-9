...
class GoLexer(Lexer):
    ...
    def __init__(self):
        ...
        self._keyword_lists = [
            SilverCity.WordList(
                ' '.join(sorted(GoLangInfo.reserved_keywords))),
...
class GoLangIntel(CitadelLangIntel,
    ...
    def codeintel_type_from_completion_data(self, completion_entry):
        ...
        """
        completion_type = self.completion_name_mapping.get(
            completion_entry['class']) or completion_entry['class']
    ...
    def lookup_defn(self, buf, trg, ctlr):
        ...
        if godef_path is None:
            godef_path = 'godef'
        cmd = [godef_path, '-i=true', '-t=true', '-f=%s' %
               buf.path, '-o=%s' % trg.pos]
        ...
        d = Definition("Go", path,
                       ...
                       name=name,
                       line=line,
                       ilk='function' if typeDesc.startswith(
                           'func') else typeDesc,
                       citdl=None,
                       signature=typeDesc,
                       doc='\n'.join(lines[1:]),
    ...
    def invoke_gocode(self, buf, trg, ctlr):
        ...
        ctlr.start(buf, trg)
        # remove PANIC entries if present
        completion_data = [x for x in completion_data if x['class'] != 'PANIC']
        if not completion_data:
            # Only contained PANIC entries.
            ctlr.error(
                "no valid response from gocode: check gocode is running")
            ctlr.done("error")
            return

        if trg.type == "object-members":
            ctlr.set_cplns([(self.codeintel_type_from_completion_data(
                entry), entry['name']) for entry in completion_data])
            ctlr.done("success")
...
class GoCILEDriver(CILEDriver):
    ...
    def compile_gooutline(self, buf):
        ...
        if self._gooutline_executable_and_error is None or go_exe != self._gooutline_executable_and_error[0]:
            self._gooutline_executable_and_error = (
                go_exe, None, "Unknown Error")
