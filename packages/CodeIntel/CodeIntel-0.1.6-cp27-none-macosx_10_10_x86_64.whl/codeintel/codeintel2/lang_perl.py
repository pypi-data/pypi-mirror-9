    ...
    def trg_from_pos(self, buf, pos, implicit=True):
        ...
        if last_ch == ' ':
            ...
            text = accessor.text_range(
                max(0, last_pos - LIMIT), last_pos)  # working text
            if DEBUG:
            ...
            while i >= 0:  # parse out the preceding identifier
                if not isident(text[i]):
                    ...
                    while j >= 0 and text[j] in WHITESPACE:
                        j -= 1
                    if j >= 0:
                        preceding_ch = text[j]
                    else:
            ...
            LIMIT = 50
            text = accessor.text_range(
                max(0, last_pos - 1 - LIMIT), last_pos - 1)  # working text
            if DEBUG:
                print "  working text: %r" % text
            i = len(text) - 1
            while i >= 0 and text[i] in WHITESPACE:  # parse off whitespace
            ...
            # skip to whitespace
            while i > 0 and text[i - 1] not in WHITESPACE:
                i -= 1
            prefix = text[i:pos - 2]
