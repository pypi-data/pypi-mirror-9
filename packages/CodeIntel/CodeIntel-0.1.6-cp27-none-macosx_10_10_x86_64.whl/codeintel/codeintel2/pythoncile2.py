...
# should be in codeintel/lib/lib2to3 (read "to play along" above)
import lib2to3
            ...
            if args.type == token.RPAR:  # No args, perhaps a @staticmethod?
                ...
                for idx, arg in enumerate(args.children):
                    if arg.type == token.COMMA:
                        idx += 1
                        break
                # XXX safe? What about `def method(self,)`?
                args.children[idx].prefix = ''
                a = ''.join(arg.prefix + str(arg)
                            for arg in args.children[idx:])
            if name == "__init__":
...
def _traverse(nodes, _offset=None):
    ...
    for node in itertools.islice(nodes, _offset, len(nodes)):
        if node.type in _traverse_scope_types:
            ...
            # `simple_stmt` for one-liner, `suite` otherwise
