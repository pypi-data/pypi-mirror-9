...
class AgeCalc:
    ...
    def age(self):
        ag = current_yy - self.yy
        if self.mm > current_mm:
            ag -= 1
        elif self.mm < current_mm:
            pass
        else:
            if self.dd < current_dd:
                pass
            elif self.dd > current_dd:
                ag -= 1
            else:
                pass
        return ag
    ...
    def age_months(self):
        am = current_yy - self.yy
        am *= 12
        am = am + current_mm - self.mm
        return am
    ...
    def next_birthday(self):
        dnb = datetime(current_yy, self.mm, self.dd)
        dnb = dnb - datetime.now()
        dnb = dnb.days + 1
        if str(dnb)[0] == "-":
            dnb = datetime(current_yy + 1, self.mm, self.dd)
    ...
    def last_birthday(self):
        dlb = datetime.now()
        dlb = dlb - datetime(current_yy, self.mm, self.dd)
        dlb = dlb.days
        if str(dlb)[0] == "-":
            dlb = datetime.now()
            dlb = dlb - datetime(current_yy - 1, self.mm, self.dd)
            dlb = dlb.days
        return dlb
