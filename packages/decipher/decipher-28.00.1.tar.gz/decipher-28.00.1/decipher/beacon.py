...
class BeaconAPI(object):
    ...
    def _requestAuthHeaders(self):
        if self.key.startswith('session '):
            ...
            return {"x-apikey" : 'session %s' % formkey, "Cookie" : 'BEACON_LOGIN="%s"' % rest}
        else:
            return {"x-apikey" : self.key}

    def do(self, action, name, args, async=False):
        "Perform action"
        if args.pop('meta', None):
            return (dict(
        ...
        if action == 'get':
            ...
            body = kw['data'] = json.dumps(args, indent=1)
        ...
        try:
            r = self.session.request(action, url, verify=True, headers=headers, **kw)
