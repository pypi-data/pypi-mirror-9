...
def make_python_function(in_ast,func_name,targetname, special_ids=[{}],cif_dic=None,cat_meth=False, 
                         func_def = False, have_sn=True,loopable={},debug=None,depends=False):
    """Convert in_ast to python code"""
    if debug is not None:
        print "incoming AST:"
        print `in_ast`
    func_text,withtable,dependencies,cur_row = start_traverse(in_ast,target_id=targetname,cat_meth=cat_meth,loopable=loopable,debug=debug,func=func_def,cif_dic=cif_dic)
    if debug is not None:
        print 'Start========'
        print func_text
        print 'End=========='
    ...
    if len(withtable) > 0 or cur_row:  # a loop method
            ...
            preamble +="    def drel_func(cifdata,%s):\n" % dummy_vars
            # the actual function gets inserted here
            end_body=      "\n    if packet_no < 0:   #map\n"
            if cur_row and len(actual_names) > 1:    #i.e. have real names from category
                end_body +="        __row_id = range(len(%s))\n" % actual_names[0]
            elif cur_row and len(actual_names)==1:   #so no actual names available
                
                end_body += "       cat_names = ciffile.dictionary.names_in_cat('%s')\n" % getcatname(targetname)[0]
                end_body += "       have_name = [a for a in cat_names if ciffile.has_key(a)]\n"
                end_body += "       if len(have_name)>0:\n"
                end_body += "           full_length = len(ciffile[have_name[0]])\n"
                end_body += "           __row_id = range(full_length)\n"
                end_body += "       else:\n"
                end_body += "           return []\n"
            end_body+=      "       return map(lambda %s: drel_func(ciffile,%s),%s)\n" % (dummy_vars,dummy_vars,map_names)
            end_body+=     "    else:\n"
            end_body+=     "        return drel_func(ciffile,%s)\n" % one_packet_each
...
def start_traverse(in_node,target_id=None,loopable={},cat_meth=False,debug=None, func=False,
                  cif_dic=None):
  ...
  if target_id is not None and not cat_meth and not func:
      ...
      print "Category method %s may not work"
  ...
  def traverse_ast(in_node,debug=debug):
    ...
    if node_type == "ARGLIST":
        ...
        if len(in_node)==1:  #empty list
           return "StarFile.StarList([])"
        if special_info["rhs"] == True:
            outstring = "StarFile.StarList(["
        else:
            outstring = ""
        ...
        if newid:
            ...
            get_loop = "ciffile.GetKeyedSemanticPacket(%s,'%s')" % (traverse_ast(in_node[2]),newid[0])
        ...
        if newid:
            ...
            # unlooped category, we put the full name back.
                ...
                if not special_info["withtable"].has_key(true_name):  #new
            ...
            if special_info.get("target_id","").lower() == fullname:
                ...
                outstring = "ciffile['%s']" % fullname
        else: # default to Python attribute access
            # check for packet variables
            if special_info["packet_vars"].has_key(primary):
...
# everything up to the final number
        ...
        outstring = "__pycitems = " + repr(catnames)
        ...
        if var_info[2] == True:
            outstring += "\n" + "__loop_range%d = range(len(ciffile[__pycitems[0]]))" % loop_num
        else:
...
#            try:
                ...
                if not isinstance(next_bit,basestring):
...
#                outstring += "raise SyntaxError, %s" % `one_statement`
  ...
  if not special_info.get("have_drel_target",False):
      print 'WARNING: no assignment to __dreltarget in %s (this is OK for category methods)' % `target_id`
      print result
