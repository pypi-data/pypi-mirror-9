...
class Iface:
  ...
  def execute(self, token, stmt, workspace, opts):
    """
    Parameters:
     - token
     - stmt
  ...
  def admin_execute(self, token, query_stmt, workspace, opts):
    """
    Parameters:
     - token
     - query_stmt
...
class Client(Iface):
  ...
  def execute(self, token, stmt, workspace, opts):
    ...
    Parameters:
     ...
     - opts
    """
    self.send_execute(token, stmt, workspace, opts)
    return self.recv_execute()

  def send_execute(self, token, stmt, workspace, opts):
  ...
  def admin_execute(self, token, query_stmt, workspace, opts):
    ...
    Parameters:
     ...
     - opts
    """
    self.send_admin_execute(token, query_stmt, workspace, opts)
    return self.recv_admin_execute()

  def send_admin_execute(self, token, query_stmt, workspace, opts):
    self._oprot.writeMessageBegin('admin_execute', TMessageType.CALL, self._seqid)
    args = admin_execute_args()
    args.token = token
    args.query_stmt = query_stmt
    args.workspace = workspace
    args.opts = opts
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
  ...
  def recv_admin_execute(self):
    ...
    if result.arr is not None:
      raise result.arr
    raise TApplicationException(TApplicationException.MISSING_RESULT, "admin_execute failed: unknown result");
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["admin_execute"] = Processor.process_admin_execute
  ...
  def process_execute(self, seqid, iprot, oprot):
    ...
    result = execute_result()
    try:
      result.success = self._handler.execute(args.token, args.stmt, args.workspace, args.opts)
    except exc.ttypes.ServiceError, err:
      result.err = err
    except exc.ttypes.AuthError, arr:
      result.arr = arr
  ...
  def process_admin_execute(self, seqid, iprot, oprot):
    args = admin_execute_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = admin_execute_result()
    try:
      result.success = self._handler.admin_execute(args.token, args.query_stmt, args.workspace, args.opts)
    except exc.ttypes.ServiceError, err:
      result.err = err
    except exc.ttypes.AuthError, arr:
      result.arr = arr
    oprot.writeMessageBegin("admin_execute", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class admin_execute_args:
  ...
  thrift_spec = (
    None, # 0
    (1, TType.STRING, 'token', None, None, ), # 1
    (2, TType.STRING, 'query_stmt', None, None, ), # 2
    (3, TType.STRING, 'workspace', None, None, ), # 3
    (4, TType.STRING, 'opts', None, None, ), # 4
  )

  def __init__(self, token=None, query_stmt=None, workspace=None, opts=None,):
    self.token = token
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('admin_execute_args')
    if self.token is not None:
      oprot.writeFieldBegin('token', TType.STRING, 1)
      oprot.writeString(self.token)
      oprot.writeFieldEnd()
    if self.query_stmt is not None:
      oprot.writeFieldBegin('query_stmt', TType.STRING, 2)
...
class admin_execute_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('admin_execute_result')
    if self.success is not None:
      oprot.writeFieldBegin('success', TType.STRING, 0)
      oprot.writeString(self.success)
      oprot.writeFieldEnd()
