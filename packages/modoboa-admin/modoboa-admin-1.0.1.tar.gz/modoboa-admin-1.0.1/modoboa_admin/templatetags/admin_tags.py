    ...
    if user.has_perm("modoboa_admin.add_domain"):
        entries += [
            {"name": "newdomain",
             ...
             "url": reverse("modoboa_admin:domain_add")},
        ]
        ...
        entries += [
            {"name": "import",
             ...
             "url": reverse("modoboa_admin:domain_import"),
             "modal": True,
             "modalcb": "admin.importform_cb"},
            {"name": "export",
             "label": _("Export"),
             "img": "fa fa-share-alt",
             "url": reverse("modoboa_admin:domain_export"),
             "modal": True,
...
def identities_menu(user, selection=None):
    ...
    entries = [
        ...
        {"name": "newaccount",
         ...
         "url": reverse("modoboa_admin:account_add")},
        {"name": "newalias",
         "label": _("Add alias"),
         "img": "fa fa-plus",
         "modal": True,
         "modalcb": "admin.aliasform_cb",
         "url": reverse("modoboa_admin:alias_add")},
        {"name": "newforward",
         "label": _("Add forward"),
         "img": "fa fa-plus",
         "modal": True,
         "modalcb": "admin.aliasform_cb",
         "url": reverse("modoboa_admin:forward_add")},
        {"name": "newdlist",
         "label": _("Add distribution list"),
         "img": "fa fa-plus",
         "modal": True,
         "modalcb": "admin.aliasform_cb",
         "url": reverse("modoboa_admin:dlist_add")},
        {"name": "import",
         "label": _("Import"),
         "img": "fa fa-folder-open",
         "url": reverse("modoboa_admin:identity_import"),
         "modal": True,
         "modalcb": "admin.importform_cb"},
        {"name": "export",
         "label": _("Export"),
         "img": "fa fa-share-alt",
         "url": reverse("modoboa_admin:identity_export"),
         "modal": True,
         "modalcb": "admin.exportform_cb"
         }
    ]
...
def identity_actions(user, ident):
    ...
    if name == "User":
        ...
        actions += [
            {"name": "delaccount",
             "url": reverse("modoboa_admin:account_delete", args=[objid]),
             "img": "fa fa-trash",
...
def domain_modify_link(domain):
    ...
    if domain.__class__.__name__ == "Domain":
        linkdef["url"] = reverse("modoboa_admin:domain_change", args=[domain.id])
...
def identity_modify_link(identity, active_tab='default'):
    ...
    if identity.__class__.__name__ == "User":
        linkdef["url"] = reverse("modoboa_admin:account_change", args=[identity.id])
        linkdef["url"] += "?active_tab=%s" % active_tab
        linkdef["modalcb"] = "admin.editaccount_cb"
    else:
        linkdef["url"] = reverse("modoboa_admin:alias_change", args=[identity.id])
...
def domadmin_actions(daid, domid):
    actions = [{
        ...
        "url": "{0}?domid={1}&daid={2}".format(
            reverse("modoboa_admin:permission_remove"), domid, daid),
        "img": "fa fa-trash",
