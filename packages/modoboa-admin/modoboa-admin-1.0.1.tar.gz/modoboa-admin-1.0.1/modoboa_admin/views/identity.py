...
@user_passes_test(
    ...
    u.has_perm("modoboa_admin.add_alias")
)
def _identities(request):
    ...
    if page is None:
        ...
        context["headers"] = _render_to_string(
            request, "modoboa_admin/identity_headers.html", {})
        context["rows"] = _render_to_string(
            request, "modoboa_admin/identities_table.html", {
...
@login_required
@permission_required("modoboa_admin.add_mailbox")
def list_quotas(request):
    ...
    context = {
        "headers": _render_to_string(
            request, "modoboa_admin/quota_headers.html", {}
        )
    }
    if page is None:
        context["length"] = 0
        ...
        context["rows"] = _render_to_string(
            request, "modoboa_admin/quotas.html", {
                "mboxes": page
            }
        )
        context["pages"] = [page.number]
...
@ensure_csrf_cookie
def identities(request, tplname="modoboa_admin/identities.html"):
...
@login_required
@permission_required("modoboa_admin.add_domain")
