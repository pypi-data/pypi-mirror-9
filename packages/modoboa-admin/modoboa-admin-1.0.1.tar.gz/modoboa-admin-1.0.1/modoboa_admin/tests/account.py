...
class PermissionsTestCase(ModoTestCase):
    ...
    def test_domain_admins(self):
        self.ajax_post(
            reverse("modoboa_admin:account_change", args=[self.user.id]),
            self.values
        )
    ...
    def test_self_modif(self):
        ...
        self.ajax_post(
            reverse("modoboa_admin:account_change", args=[admin.id]),
            values
        )
    ...
    def test_domadmin_access(self):
        ...
        self.assertEqual(
            self.clt.login(username="admin@test.com", password="toto"),
            True)
        response = self.clt.get(reverse("modoboa_admin:domain_list"))
    ...
    def test_domainadmin_deletes_superadmin(self):
        ...
        self.clt.login(username="admin@test.com", password="toto")
        self.ajax_post(
            reverse("modoboa_admin:account_delete", args=[account.id]), {}, 403
        )
