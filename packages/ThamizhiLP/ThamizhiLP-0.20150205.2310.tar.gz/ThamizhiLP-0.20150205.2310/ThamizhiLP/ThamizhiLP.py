...
def CheckEndingLetter(word):
    ...
    uyir_rest=["ா","ி","ீ","ு","ூ","ெ","ே","ை","ொ","ோ"] #107 Ak not yet considered
...
def ValidateTamilWord(word):
	if CheckOreluthuOrumozhi(word)==1:
		print("A valid Oreluthu orumozhi")
	else:
		if CheckStartingLetter(word)==1:
			if CheckEndingLetter(word)==1:
				if CheckMeimmayakkam(word)==0:
					print ("A valid Tamil word")
				else :
					print(CheckMeimmayakkam(word))
			else:
				print(CheckEndingLetter(word))
		else :
			print(CheckStartingLetter(word))
