...
def SaveAsTXT(lst, fname):
	...
	with open(fname, "w") as myfile:
		for dict in lst:
			myfile.write(formatListItemText(dict))	
...
def SaveAsCSV(lst, fname):  
	...
	for dict in lst:
		op.write(formatListItemCSV(dict))
...
def SaveAsXML(lst, fname):
	def formatListItemXML(itm):
		...
		def StripHexChars(txt):  # The project is based on a theory about “intelligence” as a whole
			txt2 = txt.replace('\0x93', '"') #.replace('“','"').replace('”','"')   # bug here - still has trouble with quotes
