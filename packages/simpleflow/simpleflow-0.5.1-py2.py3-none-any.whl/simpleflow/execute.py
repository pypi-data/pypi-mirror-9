    ...
    def wrap_callable(func):
        ...
        def execute(*args, **kwargs):
            ...
            try:
                ...
                print err.output
                excline = err.output.rsplit('\n', 2)[1]
                try:
                    excname, excmsg = excline.split(':', 1)
                except ValueError:
                    excname = excline
                    excmsg = ''

                exception = make_callable(excname.strip())
                raise exception(excmsg.strip())
...
def make_callable(funcname):
    ...
    try:
        callable = getattr(module, object_name)
...
if __name__ == '__main__':
    ...
    callable = make_callable(funcname)
