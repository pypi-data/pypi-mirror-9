...
class CCUFlag(object):
	...
	def signalWithSource(self, source):
		with self._lock:
			self._source=source
			self.value=1

	def signal(self):
		return self.signalWithSource(None)
...
class CCUVirtualChannels(CCUDevice):
	...
	def onEvent_press_short(self, notification):
		...
		if c:
			flag=c['shortpress']
			flag.signalWithSource(notification.deviceId)
	...
	def wasTriggered(self, sinceDelay=3.0):
		for channel in self.channels():
			for flag in channel.values():
				if flag.age()<sinceDelay:
					return True
...
class CCU:
	...
	def isPendingSignal(self, channel, shortPress=True):
		return self.vchannels.isPendingButton(channel, shortPress)
