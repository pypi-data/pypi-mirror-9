...
def updateDateTz(request_data):
    ...
    if type(request_data ['from_Date'])is unicode:
        request_data ['from_Date']= datetime.datetime(int(fd[0:4]),int(fd[5:7]),int(fd[8:10]),int(fd[11:13]),int(fd[14:16]),00)
        request_data ['to_Date']= datetime.datetime(int(td[0:4]),int(td[5:7]),int(td[8:10]),int(td[11:13]),int(td[14:16]),00)

    sign = int(request_data['previous'])

    try:    
        tz_adjustement = (int(tz[7:]) * sign) +sign
    except:
        tz_adjustement = sign
...
def correspondGranularity(request_data):
    ...
    if (isinstance(request_data['from_Date'],unicode)) == False: 
        ...
        if (isinstance(request_data['from_Date'],unicode)): 
            ...
            from_date = datetime.datetime(int(year),int(month),int(day),int(hour),int(minute),00)
        ...
        if (isinstance(request_data['to_Date'],unicode)): 
            ...
            to_date = datetime.datetime(int(year),int(month),int(day),int(hour),int(minute),00)
...
def adjustementGranularity(request_data,col_cube): 
    ...
    if (isinstance(request_data['from_Date'],unicode)) == False: 
        ...
        if (isinstance(request_data['from_Date'],unicode)): 
            ...
            request_data['from_Date'] = datetime.datetime(int(year),int(month),int(day),int(hour),int(minute),00)
        ...
        if (isinstance(request_data['to_Date'],unicode)): 
            ...
            request_data['to_Date'] = datetime.datetime(int(year),int(month),int(day),int(hour),int(minute),00)
