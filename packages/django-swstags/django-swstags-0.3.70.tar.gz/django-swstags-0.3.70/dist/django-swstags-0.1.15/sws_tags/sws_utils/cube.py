...
def namesColCube(col_cube_new):
    ...
    if type(col_cube_new) == list:
        ...
        # print 'NAMES COL CUBE:',col_cube_new
        try:
...
def accuratelyAdjustment(json_data,med_cube_prec,dict_type_data = False):
...
def filterFormat(new_queryset):
    ...
    try:
        for k,v in new_queryset[0].items():
            if (type(v) is not unicode) and (type(v) is not datetime.datetime): 
