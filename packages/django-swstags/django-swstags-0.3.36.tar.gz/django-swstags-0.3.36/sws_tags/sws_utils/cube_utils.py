...
class CUBE:
    ...
    def connect(self,ip,bbdd=[],cubes=[],redis=""):
        ...
        self.set_name_BBDD_used(bbdd[0])
...
class MDX:
        ...
        if part:
            ...
            # for on_row in part:
                ...
                if on_row in dimensiones:
                    ...
                    swslog('error','Failed MDX class 2 ','<<error in {0} >>'.format(str(on_row)))
                    mdx+='<<error in {0} >>'.format(str(on_row))
    ...
    def __order(self,cube,order,part_order):
        ...
        if order in dimensiones:
            ...
            swslog('error','Failed MDX class 3 ','<<error in order {0}>> '.format(str(order)))
    ...
    def __filter(self,cube,part_filter,part_on_rows,range_rows,exclude_rows):
        ...
        for filte in part_filter:
            ...
            if filte in dimensiones:
                ...
                swslog('error','Failed MDX class 4 ','<<error in filter {0}>> '.format(str(filte)))
    ...
    def __on_row(self,cube,part_on_rows,range_rows,exclude_rows,part_order,part_filter):
        ...
        if part_order :
            if len(part_order)==1:
                ...
                swslog('error','Failed MDX class 5 ','<< part_order must be like {0}>>'.format(str({('Measures','Calls'):'ASC'})))
