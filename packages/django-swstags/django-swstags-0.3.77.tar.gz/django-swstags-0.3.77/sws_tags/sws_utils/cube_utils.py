...
class Salida_Highcharts:
            ...
            for num in range(0,len(axis_y_values)/3):
                if axis_y_values[3*num] in dictValues:
                    if axis_y_values[(3*num)+1] in dictValues[axis_y_values[3*num]]:                        
                        ...
                        dictValues[axis_y_values[3*num]][axis_y_values[(3*num)+1]][axis_y_values[(3*num)+2]][axis_x_names[0]]=self.replaceSimbols(axis_x_values[2*num],format_cube)
                        dictValues[axis_y_values[3*num]][axis_y_values[(3*num)+1]][axis_y_values[(3*num)+2]][axis_x_names[1]]=self.replaceSimbols(axis_x_values[(2*num)+1],format_cube)
        ...
        if numero_dimensiones==2:
            for num in range(0,len(axis_y_values)/2):
                if axis_y_values[2*num] in dictValues:
                    ...
                    dictValues[axis_y_values[2*num]][axis_y_values[(2*num)+1]]['00:00'][axis_x_names[0]]=self.replaceSimbols(axis_x_values[2*num],format_cube)
                    dictValues[axis_y_values[2*num]][axis_y_values[(2*num)+1]]['00:00'][axis_x_names[1]]=self.replaceSimbols(axis_x_values[(2*num)+1],format_cube)
                else:
    ...
    def result_to_json(self,cube,result,types={},xAxis=['Time','Date'],dimension_v_name='client',exclude=[],complete_name=False,format_cube=False):
        ...
        if not error:
            ...
            for key in sort_keys:
                v_name.append(key+' '+axis_x_names[0].upper())
                v_name.append(key+' '+axis_x_names[1].upper())
            ...
            for num in range(0, len(v_name)):
                if axis_x_names[0].upper() in v_name[num][-len(axis_x_names[0]):]:
                    v_type.append(types[axis_x_names[0].upper()])
                if axis_x_names[1].upper() in v_name[num][-len(axis_x_names[1]):]:
                    v_type.append(types[axis_x_names[1].upper()])
            ...
            for key in sort_keys:
                ...
                for dia in v_xAxis:
                    ...
                    try:
                        data_list_1measure.append(dictValues[key][day][hour][axis_x_names[0]])
                    except:
                        data_list_1measure.append(0)
                    try:
                        data_list_2measure.append(dictValues[key][day][hour][axis_x_names[1]])
                    except:
                        data_list_2measure.append(0)
                v_data.append(data_list_1measure)
                v_data.append(data_list_2measure)
            if numero_dimensiones==2:
    ...
    #             dict_higcharts={}
    #             dict_higcharts['v_data']=v_data ## lista de listas con los valores para cada eje y 
    #             dict_higcharts['v_name']=v_name ## lista de nombres paa cada eje y 
    #             dict_higcharts['v_type']=v_type ## lista de tipos para cada eje y
    #             dict_higcharts['v_yAxis']=v_yAxis ## lista con numeros que indican preferencia de cada eje y 
    #             dict_higcharts['v_xAxis']=v_xAxis ## lista con valores de eje x fechas normalmente
    #             if error:
