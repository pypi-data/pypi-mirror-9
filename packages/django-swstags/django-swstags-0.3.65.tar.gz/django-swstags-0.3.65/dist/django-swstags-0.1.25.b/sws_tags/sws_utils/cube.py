...
def requestCube(ip_connect,name_bbdd,name_cube,col_cube,med_cube,request_data,ifDate,dimension_filters,database_cube_dict,total_data = False,where_tupla=('Date','Id','Id'),request_date_previous = False,complete_name=False):
    ...
    if part_order:
        ...
        # print 'col',col_cube[0]
        # print 'order',data_order
        # print 'res',col_cube[0]==data_order


        if not (col_cube[0]==data_order) and not(data_order in cube.medidas) and data_order!='':
            # print 'reordenandoooooo'
            t2=time.time()
            json_data= json.loads(json_data)
            jsond_data=reorderJson(json_data,col_cube)
            json_data=json.dumps(json_data)
    return json_data

def reorderJson(json_data,col_cube):
