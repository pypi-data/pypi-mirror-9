...
def tag_speedometer(view_name,speedometer_name,speedometer_text,minimun,maximum,text,default_value,invert='false',step=4,show_label='True',start_popup='Income',end_popup='€'):
...
def tag_grid_dict(view_name,
	...
	conf_option_default['shrinkToFit'] = 'true'
...
############################################--TAG MORELESS-###################################################
##############################################################################################################
@register.inclusion_tag('tags/tag_moreless.html')
def tag_moreless(view_name,
				name_moreless,
				text_moreless,
				position,
				rang,
	...
	"""
	  ...
	  **Param:**
		#. 'VP' -> view name
		#. 'Number of date' -> moreless text
		#. 'horizontal' -> moreless position (horizontal or vertical)
		#. 'false' -> false if not have range and true if have range
		#. 0 -> minimun value
		#. 15 -> maximum value
		#. default_min -> default minimun value in first load
	  ...
	  **Event:** 
		>>> case 'moreless': #when change value in moreless
	...
	"""
	return {'view_name': view_name,
			'name_moreless':name_moreless,
			'text_moreless':text_moreless,
			'position':position,
			'rang':rang,
			'min_val':min_val,
			'max_val':max_val,
