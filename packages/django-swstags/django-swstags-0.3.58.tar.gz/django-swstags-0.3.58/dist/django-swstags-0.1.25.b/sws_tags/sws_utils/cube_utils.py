...
There are three parameters which are initialized when the method CUBE.connect() is used:
    ...
    medidas={}
    connection=''
    name=settings.CUBE_DB_NAME
    key_memcache=settings.APP_NAME+'_dimYmed'
...
An object like <olap.xmla.xmla.XMLASource object at 0xa7ae5ac> : it`s the connection with OLAP service
        ...
        t1=time.time()
        # print '-----------------------',self.key_memcache
        if not isAdded(self.key_memcache):
            ...
            try:
                ...
                addDimension_Medidas(self.key_memcache,value)
            except Exception, e:
                swslog('error','wrong location',e)
        else:

            self.medidas,self.dimensiones=getDimension_Medidas(self.key_memcache)
...
An object like [u'test_cdr'] : it`s a list with the names of the BBDDs in that connection
        ...
        return row 
    def __getDimensiones(self,bbdd=[settings.CUBE_DB_NAME],cubes=['Cdrt']):
        dimensiones={}
...
-An list which has in each position an legible name of dimension 
    ...
    def __getMedidas(self,bbdd=[settings.CUBE_DB_NAME],cubes=['Cdrt']):
        medidas={}
        for catalogo in self.connection.getCatalogs():
...
A string which is a mdx without on rows in select
    ...
    def __rows_or_columns(self,cube,rows,part=[],range_rows={},exclude_rows={},part_order={}):
        ...
        return mdx
    def __rows_or_columns_vieja(self,cube,rows,part=[],range_rows={},exclude_rows={},part_order={}):
        dimensiones=cube.dimensiones
        medidas=cube.medidas
        ...
        for i in part_where:
            ...
            if 'Date' in i:
                ...
                if i in dimensiones:
                    ...
                    w2=w[position_value:]
                    position_value+=w2.find('.')+1
                    # print 'por aquiiiii---- PART DATE---TYPEE',part_where[i]
                    # if type(part_where[i]) == str:
                    mdx+='('+w[:position_value]+'&['+str(part_where[i])+'] ) *'
                    
                    # elif type(part_where[i])==tuple:
                    #     print '2Q431241234TUPLE-------',type(part_where[i][1])
                       
                    #     if part_where[i][0] == '' and type(part_where[i][1]) ==str:   #case filter add
                    #         print 'CASE 1 PARAM ADD'
                    #         mdx+='('+w[:position_value]+'&['+str(part_where[i][1])+'] ) *' 
                       
                    #     elif part_where[i][0] == '-' and type(part_where[i][1]) ==str: # case filter exclude
                    #         print 'CASE 1 PARAM EXCLUDE'
                        
                    #     elif part_where[i][0] == '' and type(part_where[i][1] == list):
                    #         print 'CASE LIST PARAM ADD'
                        
                    #     elif part_where[i][0] == '-' and type(part_where[i][1] == list):
                    #         print 'CASE LIST PARAM EXCLUDE'
        ...
        for i in part_where:
            if 'Date' in i:
                ...
                if i in dimensiones:
                    ...
                    w2=w[position_value:]
                    position_value+=w2.find('.')+1
                    # print 'DDDdimension->',i
                    # print 'PART WHERE',part_where[i]
                    ...
                    # print 'mdx->>',mdx
                    # EXAMPLE NOT DELETE
                    # select non empty  ([Client].[Client].[(All)]) on rows,  
                    # {[Measures].[Calls],[Measures].[ACD],[Measures].[ASR]}  on columns 
                    # from [Cdrt] 
                    # where ( {([Date].[Id].&[20130408] : [Date].[Id].&[20130414],[Hangupcode].[Id].[Id]-[Hangupcode].[Id].&[34]-[Hangupcode].[Id].&[41]) }  )
