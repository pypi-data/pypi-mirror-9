...
class YouTrackClient(object):
    ...
    def get_custom_field_details(self, field):
        field_data = self._request(field['url'], method='get')
        field_type = field_data.projectcustomfield['type']
        type_name = "%sBundle" % field_type[:field_type.find('[')]
        field_details = {
            'name': field_data.projectcustomfield['name'],
            'type': field_data.projectcustomfield['type'],
            'default_value': field_data.projectcustomfield['emptytext'],
            'values': self.get_custom_field_values(type_name, field_data.param['value']) if field_data.param else None
        }
