...
def discover():
  ...
  def zc_service_change(zeroconf, service_type, name, state_change):
    if state_change is Zeroconf.ServiceStateChange.Added:
      ...
      devices_found.append( {"name": name, "info": info} )
  ...
  Util.display_list( "Devices found:", devices_found,
    ( lambda index,value: click.style( value["info"].server, bold=True, fg="cyan" ) )
  )
...
def flash_card( disk ):
  ...
  # flash the disk
  Util.call_and_return('sudo', 'dd', "if="+raspbian_image, "of="+disk, "bs=4m")
...
def setup_card():
  ...
  if not os.path.exists( os.path.expanduser("~/.stak/images/netinst-image.iso") ):
    click.echo("Downloading image...")
    Util.call_and_return("curl", "-o", os.path.expanduser("~/.stak/images/netinst-image.iso.bz2"), "http://stak-images.s3.amazonaws.com/netinst-image.iso.bz2")
    Util.call_and_return("bunzip2", os.path.expanduser("~/.stak/images/netinst-image.iso.bz2") )
