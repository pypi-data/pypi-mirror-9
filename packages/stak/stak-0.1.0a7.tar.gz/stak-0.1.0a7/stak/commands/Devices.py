...
def find_valid_sd_cards():
  ...
  disks = Util.find_list_items_containing( "\d\:\s+[a-zA-Z_]+\s+(\d+\.\d+\s+[kKgGmM]B).*(disk[0-9])", diskutil_output,
    ( lambda m: [m.group(1).strip(), "/dev/"+m.group(2)] ))
  for disk in disks:
    diskutil_output = Util.call_and_return("diskutil", "info", disk[1])
...
def setup_card():
  ...
  click.echo(click.style('WARNNING!! Make SURE this is the correct card, this command will delete EVERYTHING on the target volume '+selected_card + " of size " + cards[ selected_index ][0], reverse=True))
  if not click.confirm('Are you sure you with to continue?'):
    return
