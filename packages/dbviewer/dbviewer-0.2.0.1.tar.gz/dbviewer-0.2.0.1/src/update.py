...
class Viewer (wx.Frame):
	def __init__(self, parent, title):
		...
		upd_btn = wx.Button(self.vwp, id = wx.ID_ANY, label="Update")
	...
	def data_update (self, event):
		data = []
		extend = self.aux.addQuest(self.columnize)
		sql= "update " + self.table_name + " set "+ extend + "where rowid = " \
								+ str(self.dataIndex)
		for sd in self.items:
			data.append(sd.GetValue())
		
		dial = wx.MessageDialog(None, "Are you sure to update ?", "Confirmation",
									wx.YES_NO | wx.NO_DEFAULT | wx.ICON_EXCLAMATION)
		
		response = dial.ShowModal()
		
		if response == wx.ID_YES:
			self.dbset.queries(sql, data)
