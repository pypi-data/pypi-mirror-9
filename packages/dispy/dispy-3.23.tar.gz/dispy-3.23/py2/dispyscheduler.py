...
class _Scheduler(object):
    ...
    def udp_server(self, coro=None):
        ...
        Coro(self.send_ping_cluster, _parse_nodes(self.node_spec).items(), set())
        ...
        while True:
            ...
            if msg.startswith('PULSE:'):
                ...
                try:
                    ...
                    continue
                auth = hashlib.sha1(info['sign'] + self.node_secret).hexdigest()
                node = self._nodes.get(info['ip_addr'], None)
                if node:
    ...
    def scheduler_task(self, conn, addr, coro=None):
        ...
        elif msg.startswith('DEL_CLUSTER:'):
            msg = msg[len('DEL_CLUSTER:'):]
            try:
                req = unserialize(msg)
    ...
    def timer_task(self, coro=None):
            ...
            for result_file in result_files:
                ...
                try:
                    ...
                    status = yield self.send_job_result(
                        result.uid, compute.id, cluster.client_ip_addr,
                        cluster.client_job_result_port, result, resending=True, coro=coro)
                    if status:
                        break
        ...
        while True:
            ...
            if self.ping_interval and (now - last_ping_time) >= self.ping_interval:
                last_ping_time = now
                for cluster in self._clusters.itervalues():
                    Coro(self.send_ping_cluster, cluster._compute.node_spec.items(),
    ...
    def file_xfer_process(self, reply, xf, sock, addr):
        ...
        try:
            ...
            yield client_sock.send_msg('FILEXFER:' + serialize(xf))
    ...
    def send_ping_cluster(self, node_spec, nodes, coro=None):
        # generator
        ...
        for node_spec, node_info in node_spec:
    ...
    def add_cluster(self, cluster, coro=None):
        ...
        yield self.send_ping_cluster(cluster._compute.node_spec.items(),
    ...
    def send_job_result(self, uid, cid, ip, port, result, resending=False, coro=None):
        # generator
        assert coro is not None
    ...
    def send_job_status(self, _job, ip, port, coro=None):
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock = AsyncSocket(sock, keyfile=self.cluster_keyfile, certfile=self.cluster_certfile)
        sock.settimeout(MsgTimeout)
        try:
        ...
        for _job in dead_jobs:
            ...
            if cluster._compute.reentrant:
                ...
                Coro(self.send_job_result, _job.uid, cluster._compute.id,
                     cluster.client_ip_addr, cluster.client_job_result_port, reply, resending=False)
    ...
    def job_reply_process(self, reply, sock, addr):
        ...
        if node is None:
            if self.shared:
                node = _Node(job.ip_addr, 0, getattr(reply, 'cpus', 0), '', self.node_secret,
        ...
        Coro(self.send_job_result, _job.uid, compute.id, cluster.client_ip_addr,
             cluster.client_job_result_port, reply, resending=False)
    ...
    def run_job(self, _job, cluster, coro=None):
        ...
        # messages
        Coro(self.send_job_status, _job,
