...
class _Scheduler(object):
    ...
    def udp_server(self, coro=None):
        ...
        Coro(self.send_ping_cluster, list(_parse_nodes(self.node_spec).items()), set())
    ...
    def timer_task(self, coro=None):
        ...
        while True:
            ...
            if self.ping_interval and (now - last_ping_time) >= self.ping_interval:
                last_ping_time = now
                for cluster in self._clusters.values():
                    Coro(self.send_ping_cluster, list(cluster._compute.node_spec.items()),
    ...
    def add_cluster(self, cluster, coro=None):
        ...
        yield self.send_ping_cluster(list(cluster._compute.node_spec.items()),
