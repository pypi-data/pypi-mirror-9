...
class TestEnsureDependencies(
    ...
    def test_success_install_apt(self, mock_print):
        ...
        side_effects = (
            ...
            (0, 'update 1', ''),  # Update the repository with new sources.
            (0, 'install add repo', ''),  # Install add-apt-repository.
            (0, 'add repo', ''),  # Add the juju stable repository.
            (0, 'update 2', ''),  # Update the repository with new sources.
    ...
    def test_distro_only_update_failure_apt(self, mock_print):
        # A ProgramExit is raised if updating APT sources fails.
        side_effects = (
            (127, '', 'no juju'),  # Retrieve the Juju version.
            (1, '', 'update error'),  # Update the repository with new sources.
        )
        with self.assert_program_exit('update error'):
            mock_call = self.call_ensure_dependencies(
                side_effects, distro_only=True)[0]
            self.assertEqual(2, mock_call.call_count)
...
class TestCheckEnvironment(
    ...
    def patch_resolve(self, return_value=None, side_effect=None):
        """Patch the charmstore.resolve function."""
        mock_resolve = mock.Mock(
            return_value=return_value, side_effect=side_effect)
        return mock.patch('quickstart.charmstore.resolve', mock_resolve)
    ...
    def test_environment_just_bootstrapped(self, mock_print):
        ...
        with self.patch_resolve(
                return_value='cs:trusty/juju-gui-42') as mock_resolve:
        ...
        mock_resolve.assert_called_once_with(
            settings.JUJU_GUI_CHARM_NAME, series=bootstrap_node_series)
    ...
    def test_existing_environment_without_entities(self, mock_print):
        ...
        with self.patch_resolve(
                return_value='cs:precise/juju-gui-42') as mock_resolve:
            ref, machine, service_data, unit_data = app.check_environment(
                env, 'my-gui', charm_url, env_type, bootstrap_node_series,
                check_preexisting)
    ...
    def test_existing_environment_with_entities(self, mock_print):
        ...
        bootstrap_node_series = 'precise'
        check_preexisting = True
        with self.patch_resolve() as mock_resolve:
            ref, machine, service_data, unit_data = app.check_environment(
                env, 'my-gui', charm_url, env_type, bootstrap_node_series,
                check_preexisting)
        # The environment status has been retrieved.
    ...
    def test_bootstrap_node_series_not_supported(self, mock_print):
        ...
        self.assert_reference_equal('cs:trusty/juju-gui-42', ref)
        mock_resolve.assert_called_once_with(
            settings.JUJU_GUI_CHARM_NAME, series='trusty')
    ...
    def test_local_provider(self, mock_print):
        ...
        bootstrap_node_series = 'trusty'
        check_preexisting = False
        with self.patch_resolve(return_value='cs:trusty/juju-gui-42'):
            ref, machine, service_data, unit_data = app.check_environment(
                env, 'my-gui', charm_url, env_type, bootstrap_node_series,
                check_preexisting)
    ...
    def test_default_charm_url(self, mock_print):
        ...
        bootstrap_node_series = 'precise'
        check_preexisting = False
        with self.patch_resolve(side_effect=IOError('boo!')):
            ref, machine, service_data, unit_data = app.check_environment(
                env, 'my-gui', charm_url, env_type, bootstrap_node_series,
                check_preexisting)
    ...
    def test_charm_url_provided(self, mock_print):
        ...
        with self.patch_resolve() as mock_resolve:
            ref, machine, service_data, unit_data = app.check_environment(
                env, 'my-gui', charm_url, env_type, bootstrap_node_series,
                check_preexisting)
        # There is no need to call the charm store API if the charm URL is
...
class TestDeployBundle(ProgramExitTestsMixin, unittest.TestCase):
    ...
    def test_bundle_deployment_with_id(self):
        ...
        ref = references.Reference.from_fully_qualified_url(
            'cs:bundle/django-single-42')
        ref.charmworld_id = 'django/single'
