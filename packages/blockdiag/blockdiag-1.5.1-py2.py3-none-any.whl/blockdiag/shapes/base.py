...
class Shape(object):
    ...
    def size(self):
        """ シェイプの大きさ(外縁)を返す """
        return self.metrics.cellsize_at(self.node.xy)

    @property
    def connectors(self):
        """ コネクタ(エッジの接続点)の位置をシェイプ左上からの相対座標で返す

        シェイプの配置が決まっていない可能性があるので、絶対座標は返さない
        """
        return Box(0, 0, *self.size)
