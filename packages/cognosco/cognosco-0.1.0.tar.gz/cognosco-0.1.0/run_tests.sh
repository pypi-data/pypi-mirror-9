...
Execute the Cognosco test suite.
...
if [ ${venv} = yes ]; then
    ...
    echo "Installing cognosco setup in the virtual environment"
    python ${dir}/setup.py install
    if [ $? -ne 0 ]; then
        echo "Failed to install cognosco setup in virtual environment" >&2
        exit 1
    fi
...
if [ ${pep8} != only ]; then
	...
	options="${options} --cover-package=cognosco"
    ...
    if [ $? -ne 0 ]; then
	echo "Tests on Cognosco failed" >&2
...
if [ ${pep8} != no ]; then
    ...
    echo
    run_pep8 ${dir}/cognosco ${dir}/tests
    if [ $? -ne 0 ]; then
