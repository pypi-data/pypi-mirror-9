...
def AND(*conditions):
    """
    Similar to the :py:func:`.XOR` function except the boolean operator is
    `AND`
    """
    assert conditions
...
def IF(clause, true_expr, false_expr):
    """
    The same as :py:func:`~.MAX` but instead generating a `MIN` function.
    """
    return _querybuilder.if_(clause, true_expr, false_expr)
