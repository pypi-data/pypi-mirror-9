...
# The following DEF statements were cloned from readdcd.h
DEF DCD_IS_CHARMM = 0x01
DEF DCD_HAS_4DIMS = 0x02
DEF DCD_HAS_EXTRA_BLOCK = 0x04
...
cdef __write_dcd_header(self not None, int natoms, int istart, int nsavc, double delta, bytes remarks_as_bytes):
    # At this point we assume the file has been opened for writing
    cdef dcdhandle *dcd = NULL
    cdef fio_fd fd
    cdef int rc = 0,
    cdef char *remarks = "DCD"
    cdef int with_unitcell = 1  # contains unit cell information (charmm format)
    cdef int charmm = DCD_IS_CHARMM
...
dcd.charmm = charmm
temp = PyCObject_FromVoidPtr(dcd, free); // Creates
a
New
Reference
...
debugging
purposes
temp = PyBuffer_FromMemory(dcd, sizeof(dcdhandle)); // Creates
a
New
Reference
...
if (!self) {
/ * we were in fact called as a module function,
try to retrieve
a
matching
object
from args * /
if ( !PyArg_ParseTuple(args, "OO!O!O!O!", & self, & PyArray_Type, & x, & PyArray_Type, & y, & PyArray_Type, & z,
& PyArray_Type, & uc) )
return NULL
} else {
/ *we
...
int
value. * /
if ( !PyArg_ParseTuple(args, "O!O!O!O!", & PyArray_Type, & x, & PyArray_Type, & y, & PyArray_Type, & z,
& PyArray_Type, & uc) )
return NULL
}
...
if ((rc = write_dcdstep(dcd.fd, dcd.nsets, curstep, dcd.natoms, (float *)
x - > data, (float *)
y - > data, (float *)
z - > data,
...
{
off_t
ndims, firstframesize, framesize, extrablocksize
off_t
filesize
struct
stat
...
/ *
*It
's safe to use ftell, even though ftell returns a long, because the
*header
size is <4GB.
* /
...
i, j, index
int
numatoms = 0, numframes = 0
int
start = 0, stop = -1, skip = 1, numskip = 0
dcdhandle * dcd = NULL
int * atomlist = NULL
npy_intp
dimensions[3]
...
{
dimensions[0] = numatoms;
dimensions[1] = numframes;
dimensions[2] = 3
} else
if (strncasecmp(format, "acf", 3) == 0) {
dimensions[0] = numatoms;
dimensions[1] = 3;
dimensions[2] = numframes
} else
if (strncasecmp(format, "fac", 3) == 0) {
dimensions[0] = numframes;
dimensions[1] = numatoms;
dimensions[2] = 3
} else
if (strncasecmp(format, "fca", 3) == 0) {
dimensions[0] = numframes;
dimensions[1] = 3;
dimensions[2] = numatoms
} else
...
coord = (PyArrayObject *)
PyArray_SimpleNew(3, dimensions, NPY_DOUBLE)
if (coord == NULL)
goto
error
...
/ *
*coord[a][b][c] = *(float *)(
    coord - > data + a * coord - > strides[0] + b * coord - > strides[1] + c * coord - > strides[2])
* /
if (strncasecmp(format, "afc", 3) == 0) {
* (double * )(coord- > data + j * coord- > strides[0] + i * coord- > strides[1] + 0 * coord- > strides[2]) = tempX[
index]
* (double * )(coord- > data + j * coord- > strides[0] + i * coord- > strides[1] + 1 * coord- > strides[2]) = tempY[
index]
* (double * )(coord- > data + j * coord- > strides[0] + i * coord- > strides[1] + 2 * coord- > strides[2]) = tempZ[
index]
} else
if (strncasecmp(format, "acf", 3) == 0) {
* (double * )(coord- > data + j * coord- > strides[0] + 0 * coord- > strides[1] + i * coord- > strides[2]) = tempX[
index]
* (double * )(coord- > data + j * coord- > strides[0] + 1 * coord- > strides[1] + i * coord- > strides[2]) = tempY[
index]
* (double * )(coord- > data + j * coord- > strides[0] + 2 * coord- > strides[1] + i * coord- > strides[2]) = tempZ[
index]
} else
if (strncasecmp(format, "fac", 3) == 0) {
* (double * )(coord- > data + i * coord- > strides[0] + j * coord- > strides[1] + 0 * coord- > strides[2]) = tempX[
index]
* (double * )(coord- > data + i * coord- > strides[0] + j * coord- > strides[1] + 1 * coord- > strides[2]) = tempY[
index]
* (double * )(coord- > data + i * coord- > strides[0] + j * coord- > strides[1] + 2 * coord- > strides[2]) = tempZ[
index]
} else
if (strncasecmp(format, "fca", 3) == 0) {
* (double * )(coord- > data + i * coord- > strides[0] + 0 * coord- > strides[1] + j * coord- > strides[2]) = tempX[
index]
* (double * )(coord- > data + i * coord- > strides[0] + 1 * coord- > strides[1] + j * coord- > strides[2]) = tempY[
index]
* (double * )(coord- > data + i * coord- > strides[0] + 2 * coord- > strides[1] + j * coord- > strides[2]) = tempZ[
index]
} else
if (strncasecmp(format, "caf", 3) == 0) {
* (double * )(coord- > data + 0 * coord- > strides[0] + j * coord- > strides[1] + i * coord- > strides[2]) = tempX[
index]
* (double * )(coord- > data + 1 * coord- > strides[0] + j * coord- > strides[1] + i * coord- > strides[2]) = tempY[
index]
* (double * )(coord- > data + 2 * coord- > strides[0] + j * coord- > strides[1] + i * coord- > strides[2]) = tempZ[
index]
} else
if (strncasecmp(format, "cfa", 3) == 0) {
* (double * )(coord- > data + 0 * coord- > strides[0] + i * coord- > strides[1] + j * coord- > strides[2]) = tempX[
index]
* (double * )(coord- > data + 1 * coord- > strides[0] + i * coord- > strides[1] + j * coord- > strides[2]) = tempY[
index]
* (double * )(coord- > data + 2 * coord- > strides[0] + i * coord- > strides[1] + j * coord- > strides[2]) = tempZ[
index]
}
}
// Check if we
've been interupted by the user
...
object
from args * /
if ( !PyArg_ParseTuple(args, "OO!O!O!O!|i", & self, & PyArray_Type, & x, & PyArray_Type, & y, & PyArray_Type, & z,
& PyArray_Type, & uc, & skip) )
return NULL;
} else {
/ *we
were
...
int
value. * /
if ( !PyArg_ParseTuple(args, "O!O!O!O!|i", & PyArray_Type, & x, & PyArray_Type, & y, & PyArray_Type, & z,
& PyArray_Type, & uc, & skip) )
return NULL
}
...
all
EOF
's encountered later must be errors */
...
rc = read_dcdstep(dcd.fd, dcd.natoms, (float *)
x - > data, (float *)
y - > data, (float *)
z - > data, unitcell,
...
file
was
generated by Charmm, or by NAMD > 2.5,
with the angle * /
/ *cosines
of
the
...
int
jump_to_frame(dcdhandle * dcd, int
frame)
{
int
rc
...
{
off_t
extrablocksize, ndims, firstframesize, framesize
off_t
pos
extrablocksize = dcd.charmm & DCD_HAS_EXTRA_BLOCK ? 48 + 8: 0
ndims = dcd.charmm & DCD_HAS_4DIMS ? 4: 3
