...
class MongoObject(base.MongoItemBase):
    ...
    def doInsert(self, manipulate=False, safe=None, check_keys=True,
        continue_on_error=False, **kwargs):
        self.modified = datetime.datetime.now(UTC)
        self.collection.insert(self.dump(), manipulate=manipulate, safe=safe,
            check_keys=check_keys, continue_on_error=continue_on_error,
            **kwargs)
            
    def doUpdate(self, upsert=False, manipulate=False, safe=None, multi=False,
        check_keys=True, **kwargs):
        self.modified = datetime.datetime.now(UTC)
        self.collection.update({'_id': self._id}, self.dump(), upsert=upsert,
            manipulate=manipulate, safe=safe, multi=multi,
            check_keys=check_keys, **kwargs)

    def doRemove(self, safe=None, **kwargs):
        # never call remove without an _id
        if self._id is None:
            raise ValueError("Empty _id given, this would remove all objects")
        self.collection.remove({'_id': self._id}, safe=safe, **kwargs)
