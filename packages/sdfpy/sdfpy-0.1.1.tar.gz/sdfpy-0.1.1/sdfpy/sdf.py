...
import logging
sdflog = logging.getLogger("sdf")
sdflog.setLevel(logging.INFO)
...
class HTTPDataStruct(DataStruct):
    ...
    def build_memmap(self):
        assert(self.size != -1)
        sdflog.info('Building memmap with offset: %i and size %i' % (self._offset, self.size))
    ...
    def parse_line(self, line, ascfile):
        ...
        try:
            ...
            if vtype not in _types:
                sdflog.warning("Skipping parameter %s", vname)
                return
...
class SDFIndex(object):
    ...
    def set_bounds(self):
        if ('x_min' in self.sdfdata.parameters and 'x_max' in self.sdfdata.parameters) or \
           ('theta_min' in self.sdfdata.parameters and 'theta_max' in self.sdfdata.parameters):
            ...
            sdflog.debug("Setting up older data")
            ...
            if not morton_xyz:
                sdflog.debug("Accounting for wandering particles")
                ...
                if ic_Nmesh != 0:
                    ...
                    if (f2 != ic_Nmesh):
                        ...
                        sdflog.debug("Expanding: %s, %s, %s" % (f2, ic_Nmesh, expand_root))
        ...
        sdflog.debug("MIDX rmin: %s, rmax: %s" % (self.rmin, self.rmax))
        sdflog.debug("MIDX: domain_width: %s, domain_dims: %s, domain_active_dims: %s " %
    ...
    def get_ibbox(self, ileft, iright):
        ...
        sdflog.debug('MIDX IBBOX: %s %s %s %s %s' % (ileft, iright, ix, iy, iz))
    ...
    def get_bbox(self, left, right):
        ...
        if np.any(iright-ileft) > self.domain_dims:
            sdflog.warn("Attempting to get data from bounding box larger than the domain. You may want to check your units.")
    ...
    def iter_data(self, inds, fields):
        num_inds = len(inds)
        num_reads = 0
        sdflog.debug('MIDX Reading %i chunks' % num_inds)
        i = 0
        while (i < num_inds):
            ind = inds[i]
            ...
            sdflog.debug('Reading chunk %i of length %i after catting %i starting at %i' % (i, length, combined, ind))
            num_reads += 1
            if length > 0:
                ...
                del data
            i += 1
        sdflog.debug('Read %i chunks, batched into %i reads' % (num_inds, num_reads))
    ...
    def iter_filtered_bbox_fields(self, left, right, data,
        ...
        if pos_fields is None:
            pos_fields = 'x','y','z'
        xf, yf, zf = pos_fields
        sdflog.debug("Using position fields: %s" % pos_fields)
        ...
        sdflog.debug("Periodic filtering, %s %s %s %s" % (left, right, pos.min(axis=0), pos.max(axis=0)))
        # Now get all particles that are within the bbox
        mask = np.all(pos >= left, axis=1) * np.all(pos < right, axis=1)
    ...
    def iter_bbox_data(self, left, right, fields):
        ...
        sdflog.debug('MIDX Loading region from %s to %s' %(left, right))
    ...
    def iter_sphere_data(self, center, radius, fields):
        ...
        """
        _ensure_xyz_fields(fields)
        sdflog.debug('MIDX Loading spherical region %s to %s' %(center, radius))
    ...
    def get_contiguous_chunk(self, left_key, right_key, fields):
        ...
        if length > 0:
            sdflog.debug('Getting contiguous chunk of size %i starting at %i' % (length, lbase))
    ...
    def get_key_data(self, key, fields):
        ...
        if length > 0:
            sdflog.debug('Getting contiguous chunk of size %i starting at %i' % (length, base))
    ...
    def get_cell_data(self, level, cell_iarr, fields):
        ...
        sdflog.debug("Reading contiguous chunk from %i to %i" % (lk, rk))
