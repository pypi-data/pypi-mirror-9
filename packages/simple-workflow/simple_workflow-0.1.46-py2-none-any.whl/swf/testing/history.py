...
class History(swf.models.History):
    ...
    def add_decision_task(self, execution_context=None):
        ...
        self.add_decision_task_completed(scheduled=self.last_id - 1,
    ...
    def add_activity_task_scheduled(self, activity, decision_id,
                                    activity_id=None, input=None, control=None):
        if control is None:
            control = {}
    ...
    def add_activity_task(self,
                          ...
                          control=None,
                          result=None):
        self.add_activity_task_scheduled(activity,
                                         decision_id,
                                         activity_id,
                                         input,
                                         control)
        ...
        if last_state == 'completed':
            self.add_activity_task_completed(scheduled=scheduled_id,
                                             started=started_id,
                                             result=result)
        elif last_state == 'failed':
            self.add_activity_task_failed(scheduled=scheduled_id,
                                          started=started_id)
        else:
    ...
    def add_activity_tasks_for_parts(self, parts, step_number, decision_id,
                                     last_state, input=None):
        """
        Add events for *parts* partition tasks.

        """
        for part in parts:
            self.add_activity_task(step_number,
                                   decision_id,
                                   last_state,
                                   input,
                                   control={'step_number': step_number,
                                            'partition': part})
    ...
    def add_child_workflow_start_initiated(self,
        ...
        #if 'step_number' not in control:
        #    control['step_number'] = workflow_step.number
    ...
    def add_child_workflow(self,
                           ...
                           result=None,
                           control=None):
        self.add_child_workflow_start_initiated(workflow,
        ...
        started_id = self.last_id
        if last_state == 'completed':
            self.add_child_workflow_completed(initiated_id,
                                              started_id,
                                              result=result)
        elif last_state == 'failed':
            self.add_child_workflow_failed(initiated_id,
                                           started_id)
        elif last_state == 'timed out':
            self.add_child_workflow_timed_out(initiated_id,
                                              started_id)
