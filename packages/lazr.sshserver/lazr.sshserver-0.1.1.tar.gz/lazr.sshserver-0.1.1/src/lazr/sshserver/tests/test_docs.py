...
def additional_tests():
    ...
    doctest_files = [
        os.path.abspath(resource_filename('lazr.sshserver', 'README.txt'))]
    if resource_exists('lazr.sshserver', 'docs'):
        for name in resource_listdir('lazr.sshserver', 'docs'):
            if name.endswith('.txt'):
                doctest_files.append(
                    os.path.abspath(
                        resource_filename('lazr.sshserver', 'docs/%s' % name)))
