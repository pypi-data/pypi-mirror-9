...
def repackage_input(input_, unzip_on_client=True):
    ...
    if isinstance(input_, str) or isinstance(input_,unicode):
        if unzip_on_client and is_zipfile(input_):
...
def process_input_in_batches(url, input_, is_tsv=False, verbose=False, skip_invalid_values=True, encoding='utf-8'):
    ...
    for batch_num, inp in enumerate(split_input):
        ...
        for i, (value, label) in enumerate(inp):
            ...
            data["label%d"%i] = dumps(label, encoding=encoding)
            data["user_value%d"%i] = dumps(value, encoding=encoding)
            data["is_tsv%d"%i] = dumps(is_tsv)

        num_uploaded_files += len(data.keys())/3
        data["skip_invalid_values"] = dumps(skip_invalid_values)
        data["commit_dataset"] = str(batch_num >= len(split_input) - 1)
