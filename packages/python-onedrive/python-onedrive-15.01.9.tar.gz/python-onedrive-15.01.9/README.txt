...
5.0) <http://msdn.microsoft.com/library/dn659752.aspx>`_ (formerly known
as SkyDrive).
...
`#3 <https://github.com/mk-fg/python-onedrive/pull/3>`_).
...
`DevCenter <https://dev.live.com/>`_, providing you with client\_id and
...
`this comment on
#4 <https://github.com/mk-fg/python-onedrive/issues/4#issuecomment-18233153>`_
...
-  Uploading of files larger than ~100 MiB via single POST/PUT request
   ...
   `#16 <https://github.com/mk-fg/python-onedrive/issues/16>`_ for
   details.

   Workaround in place is to fallback to (experimental at the moment of
   writing - 2014-11-23) `BITS
   API <https://gist.github.com/rgregg/37ba8929768a62131e85>`_ for
...
-  Be very careful using this module on Windows - it's very poorly
   ...
   report <https://github.com/kennethreitz/requests/issues/2039>`_, for
   instance.
...
-  Some proprietary formats, like "OneNote notebook" just can't be
   accessed (`see
   #2 <https://github.com/mk-fg/python-onedrive/issues/2>`_). OneDrive
...
-  It's been reported (#17) that `Onedrive for
   Business <https://onedrive.live.com/about/en-us/business/>`_ is not
   supported. It seem to have different `SharePoint 2013
   API <http://msdn.microsoft.com/en-us/library/fp142380%28v=office.15%29.aspx>`_.

-  Relying on `BITS
   API <https://gist.github.com/rgregg/37ba8929768a62131e85>`_ too much
   ...
   mentioned in
   `#34 <https://github.com/mk-fg/python-onedrive/issues/34>`_ and
   `#39 <https://github.com/mk-fg/python-onedrive/issues/39>`_.
...
-  As was mentioned in
   `#45 <https://github.com/mk-fg/python-onedrive/issues/45>`_,
...
`doc/api.md <https://github.com/mk-fg/python-onedrive/blob/master/doc/api.md>`_
...
`txOneDrive <https://github.com/mk-fg/txonedrive>`_ just by overriding
"request" method from OneDriveHTTPClient.

See also
`onedrive/cli\_tool.py <https://github.com/mk-fg/python-onedrive/blob/master/onedrive/cli_tool.py>`_
...
-  (unless your plan is to override that)
   `requests <http://docs.python-requests.org/en/latest/>`_ - version
   0.14.0 or higher.

   Should be installed automatically by pip if "[standalone]" or "[cli]"
   `extras-flag <https://pythonhosted.org/setuptools/setuptools.html#declaring-extras>`_
...
-  (optional) `chardet <http://pypi.python.org/pypi/chardet>`_ - only
...
2.0" <http://msdn.microsoft.com/en-us/library/dn659750.aspx>`_, which is
quite ambigous all by itself, and especially when being implemented by
well-known for it's proprietary "cripple-everything-else" extension
...
in `DevCenter <https://dev.live.com/>`_ for that to work. There's also
...
github <https://github.com/mk-fg/python-onedrive/issues/34>`_ and `this
github gist <https://gist.github.com/rgregg/37ba8929768a62131e85>`_. As
