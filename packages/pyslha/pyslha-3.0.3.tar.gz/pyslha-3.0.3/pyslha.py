...
def _irange(a,b):
    "Return an integer range that includes the end value"
    return range(a,b+1)
_PDGLIST = _irange(-6,-1) + _irange(-16,-11) + _irange(11,16) + \
           _irange(1000001,1000006) + _irange(-1000006,-1000001) + \
           _irange(2000001,2000006) + _irange(-2000006,-2000001) + \
           _irange(1000011,1000016) + _irange(-1000016,-1000011) + \
           _irange(2000011,2000016) + _irange(-2000016,-2000011) + \
           _irange(21,26) + [-24,35,36,37,-37] + \
           _irange(1000021,1000025) + [1000035,-1000024,1000037,-1000037,1000039]

# TODO: why the distinction? Jorgen?
# print set(_PDGLIST).difference(set(_PDGID2HERWIGID.keys()))
# => set([])... they are the same, so why make the separate list?
    ...
    for i, pid in enumerate(decays.keys()):
        ...
        for i_d, d in enumerate(decays[pid].decays):
            ...
            ## Squark/Slepton
                ...
                # d.ids = sorted(d.ids, cmp=cmp_bottomlast)
                d.ids = sorted(d.ids, cmp=cmp_susy_quark_lepton)
            ## Higgs
            elif pid in (25, 26):
            ...
            ids = map(str, ids)
            for id_i in ids:
                decayout += "%5d " %pdgid2herwigid( int(id_i) )
                if hasantiparticle:
                    # print pdgid2herwigid( ((-int(id_i)) if (-int(id_i)) in _PDGLIST else int(id_i)) )
                    decayoutanti += "%5d " %pdgid2herwigid( ((-int(id_i)) if (-int(id_i)) in _PDGLIST else int(id_i)) )
            decayout += "\n"
            if hasantiparticle:
                decayoutanti += "\n"
            # decayout += " ".join(ids) + "\n"
