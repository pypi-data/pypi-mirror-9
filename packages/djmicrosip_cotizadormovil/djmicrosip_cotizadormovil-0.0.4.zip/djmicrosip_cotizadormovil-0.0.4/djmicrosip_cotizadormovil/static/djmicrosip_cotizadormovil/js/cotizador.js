...
//SE MUESTRA EL BOTON DE VER EXISTENCIAS SOLO CUANDO EL FOCO ESTA EN UNA CAJA DE TEXTO AUTOCOMPLETE
$("input[name*='-articulo-autocomplete'],input[name*='-clave'],#modal-button").on("focusin",function(){
	$("#modal-button").show();
});

$("input[name*='-articulo-autocomplete'],input[name*='-clave'],#modal-button").on("focusout",function(){
	$("#modal-button").hide();
});
...
$("select[name*='-articulo']").on("change",function(){
	...
	var $existencia = select.parent().parent().parent().find("[name*='-existencia']");
	if (articulo_id != null)
	{
		$.ajax({
	 	   	...
	 	   	success: function(data){
	 	   		...
	 	   		{
	 	   			$existencia.attr("style","background-color: rgb(192, 29, 29); color:white");
	 	   		};
			},
		});
	}
