...
if imported_tiled:
    ...
    class tiled_map:
        ...
        def set_camera_pos(self, x, y, edge=True):
            """Sets camera position (centre), if edge is True it won't be outside the map."""
            self.x = x
            self.y = y
            self.edge = edge
            if self.edge:
                self.x = max([self.edge_x + self.screen_w / 2, min([self.x, self.edge_width - self.screen_w / 2])])
                self.y = max([self.edge_y + self.screen_h / 2, min([self.y, self.edge_height - self.screen_h / 2])])
    ...
    class map_obj:
        ...
        def blit(self):
            ...
            self.Map.screen.blit(self.picture, (self.x - self.Map.x + self.Map.screen_w / 2, self.y - self.Map.y + self.Map.screen_h / 2))
