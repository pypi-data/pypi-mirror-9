...
def ch_text(new_text, error=True):
    ...
    def function(obj, self):
        if type(self) == text_pusher:
            ...
            if error:
                raise TypeError(str(type(self))+ " should be text_pusher!")
    return function
...
class Main_menu:
    """Basic menu class."""
    def __init__(self, distance, *options, off=(0, 0), off_type='pixel',
        keyboard=True):
    ...
    def __repr__(self):
        ...
        fin += "with " + str(len(self.options)) + " option" + "s" * (not len(self.options) == 1) + ",\n"
        try:
    ...
    def set_options(self):
        ...
        for pos, opt in enumerate(self.options):
            opt.set_position(self.center[0], self.first_y + pos * self.distance)
            if opt == self.current:
                self.index = pos
        self.current.bold()
...
class option:
    ...
    def __init__(self, image, menu, do1=useless, do2=useless, time=0, pos_do=useless,
    ...
    def set_position(self, x, y):
        ...
        self.do1, self.do2, self.time, self.pos_do, self.anti_pos_do, self.infinity,
        self.proces)
...
class text_option(option):
    ...
    def __init__(self, font, text, color, menu, do1=useless, do2=useless, time=0, pos_do=useless,
        anti_pos_do=useless, infinity=0, proces=2):
        self.font = font
        self.text = text
        self.color = color
        super().__init__(self.font.render(self.text, True, self.color), menu, do1, do2, time, pos_do,
        anti_pos_do, infinity, proces)

    def set_position(self, x, y):
        """Sets option's position."""
        self.center = (x, y)
        super().set_position(x, y)
        self.pusher = text_pusher(self.x1, self.y1, self.font, self.text, self.color,
        self.menu, self.do1, self.do2, self.time, self.pos_do,
...
class text_pusher(pusher):
    ...
    def __init__(self, x1, y1, font, text, color, obj, do1, do2, time,
        ...
        self.text = text
        self.color = color
        super().__init__(x1, y1, self.font.render(self.text, True, self.color), obj, do1, do2, time,
        pos_do, anti_pos_do, infinity, proces)
