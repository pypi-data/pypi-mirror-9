...
def upload(serial_port, path, eeprom=False, run=True, gpio_pin=-1, progress=do_nothing):
...
def _action_upload(args):
    ...
    try:
        upload(args.serial, args.filename, (args.destination == "EEPROM"), args.run, args.gpio_pin, print_status)
