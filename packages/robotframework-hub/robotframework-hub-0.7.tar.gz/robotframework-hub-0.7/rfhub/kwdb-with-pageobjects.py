...
#        self.log.warning("I'm warnin' ya!")
    ...
    def add_library(self, name):
        ...
        if (False): # FIXME: change to "if the user set --pageobjects on the command line..."
            self._pageobject_hack(libdoc)
    ...
    def _pageobject_hack(self, libdoc):
        kw_names = [x.name for x in libdoc.keywords]
        # if this looks like a page object, strip out all of the common keywords
        if ("Resolve Selector" in kw_names and "Hash Should Be" in kw_names and name != "robotpageobjects.Page"):
            keywords = []
            for keyword in libdoc.keywords:
                if keyword.name not in ("Close","Get Hash","Go To","Hash Should Be", 
                                        "Is Visible", "Location Should Be", "Log", "Open",
                                        "Resolve Selector", "Wait For", "Wait Until Element Is Not Visible"):
                    keywords.append(keyword)
            libdoc.keywords = keywords
