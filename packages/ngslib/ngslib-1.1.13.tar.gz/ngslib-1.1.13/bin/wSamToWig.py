#!/usr/bin/python
#Last-modified: 16 Mar 2015 11:40:56 PM
...
def argParser():
    ''' Parse arguments. '''
    p=argparse.ArgumentParser(description='Convert SAM/BAM file to Wiggle file in specific binsize. SAM/BAM files must have headers. Sorting is not necessary. Reads are counted multiple times if not uniquely mapped. Contact Yunfei Wang to report any bugs (yfwang0405@gmail.com).',epilog='dependency pysam')
    p._optionals.title = "Options"
    p.add_argument("-t",dest="treatments",type=str,metavar="chip.bam", required=True, nargs='+',help="The ChIP experiment SAM/BAM file(s).")
    p.add_argument("-c",dest="control",type=str,metavar="control",default=None,help="The ChIP control SAM/BAM file.")
...
def samToBins(sname,binsize=50,extend=150,normedto=10000000,paired=False,forcestrand=False):
...
#                           print "same strand"
    ...
    for strand in bins:
        for chrom in bins[strand]:
            bins[strand][chrom]*=ratio/binsize/2
...
def binsToWig(prefix,outputdir,samfile,bins,control,ctlbins=None,chrsplit=False):
    ...
    starttime=time.clock()
    # Get sname prefix
    sname=os.path.basename(samfile)
    sname=os.path.splitext(sname)[0]
    if prefix!="":
        sname=prefix+"_"+sname
    sname=outputdir+"/"+sname
    if control:
        sname += "_"+os.path.splitext(os.path.basename(control))[0]
    ...
    for strand in bins:
        ...
        # Open wiggle file
        if not chrsplit:
            fh=open(sname+suffix,'w')
            print >>fh, "track type=wiggle_0 name="+os.path.basename(sname)
        ...
        for chrom in sorted(chroms.keys()):
            ...
            # Open Wiggle file
            if chrsplit:
                fh=open(sname+"_"+chrom+suffix,'w')
                print >>fh, "track type=wiggle_0 name="+os.path.basename(sname)+"_"+chrom
...
if __name__=="__main__":
    ...
    if args.control:
        ...
        ctlbins=samToBins(args.control,binsize,extend,normedto,isPaired,forcestrand)
    
    # read treatment
    for samfile in treatments:
        print >>sys.stderr, "Reading treatment:",samfile
        # Bam to Bins
        treatbins=samToBins(samfile,binsize,extend,normedto,isPaired,forcestrand)
        # Wiggle output
