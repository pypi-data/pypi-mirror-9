...
*0.6.2a9 - fixed issues when _flatten is called twice + timezone info being 'None' when not present
@author: oberron
...
from icalendar_SCM import RFC5545_SCM, ESCAPEDCHAR,COMMA,RFC5545_Properties,RFC5545_FREQ,weekday_map,MaxInteger, CRLF,RFC5545_eventprop_count, RFC5545_Components,RFC5545_VCALENDAR_Components
from RFC5546_SCM import RFC5546_METHODS
import uuid
import logging

__VERSION__ = "0.6.2a10"
...
class iCalendar:
    ...
    * Generator::
    ...
    def _reset(self):
        ...
        self.vevent = vevent()
        self.events = []
        self.iCalLoaded = False
        self.iCalParsed = False
...
#        print "line 703:",self.dSCM
    ...
    def local_load(self,sLocalFilePath,conformance=False):
        ...
        self.vevent.lSCM = []
        self.vevent.dSCM = {}
        self.iCalParsed = False #added 0.6.2a10
        self.iCalLoaded = False
...
#            start +=step_size
    ...
    def parse_loaded(self):
        ...
        #since the ical file can be loaded from multiple points (validator, string loader, ...) we check first that current ical file has not been already loaded
#         if self.iCalLoaded:
#             return
        VCALENDAR_Components = RFC5545_VCALENDAR_Components
        Component_Name = ""
        Component_Stack =[]
        ShowCallStack=False
        if ShowCallStack:
            print "temp 882 - entering parse loaded"
        ...
        if self.iCalLoaded == False:
            self._log("error ",[RFC5545_SCM["3_1"]])
            self.Validator("3_1", level=1) #("VCALENDAR VALIDATOR","no vCALENDAR loaded")
#            self.ical_error = 1
#            return 0
        elif self.iCalLoaded and not self.iCalParsed:
            self.iCalParsed = True
            #TODO: add here increments over the calendars
            self._log("ical loaded", [self.iCalLoaded,self.sVCALENDAR],0)
            line_count = 0
...
#                        raise Exception("IANA or X-PROPERTY","Not supported yet")
        ...
        #if we are supposed to report conformance
        if self.conformance:
            #then call the validator
            self._validate()
        
        #now consolidate the error message from events and calendar level per line
...
#            traceback.print_stack(frame)
        ...
        for event in self.events:
            ...
            if type(tmp_event["DTSTART"]) == type(datetime(year = 2003, month=3,day =5, tzinfo=UTC)):
                ...
                ##added in 0.6.2a9: if temp_event does not have a tz info, utcoffset == None, to avoid exception, only add utcoffset when it is not None
                if not utcoffset is None:   
