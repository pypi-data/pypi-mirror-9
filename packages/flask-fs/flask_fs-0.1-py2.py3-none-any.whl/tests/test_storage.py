...
class StorageTestCase(TestCase):
    ...
    def test_open(self, mock_backend):
        ...
        backend = mock_backend.return_value
        backend.open.return_value = io.BytesIO('content'.encode('utf8'))

        with storage.open('file.test') as f:
            self.assertEqual(six.text_type(f.read()), 'content')

        backend.open.assert_called_with('file.test')
    ...
    def test_url(self):
        ...
        with self.app.app_context():
            ...
            expected_url = url_for('fs.get_file', fs=storage.name, filename='test.txt', _external=True)
