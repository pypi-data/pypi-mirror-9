...
class FileFieldTest(MongoEngineTestCase):
    ...
    def test_save_from_file(self):
        ...
        # with open(self.storage.path(filename)) as in_storage, self.app.open_resource('flask.png') as source:
        #     self.assertEqual(in_storage, source)
    ...
    def test_save_with_callable_upload_to(self):
        ...
        expected_filename = '/'.join([upload_to(None), filename])
        self.assertTrue(tester.file)
        self.assertEqual(tester.file.filename, expected_filename)
        self.assertIn(expected_filename, self.storage)
        self.assertEqual(tester.to_mongo(), {
    ...
    def test_save_with_callable_basename(self):
        ...
        class Tester(db.Document):
            file = FileField(fs=self.storage, basename=basename)
...
class ImageFieldTest(MongoEngineTestCase):
    def resource(self):
        return self.filestorage('flask.png', self.app.open_resource('flask.png', 'rb'))
    ...
    def test_save_file(self):
        ...
        tester = Tester()
        tester.image.save(self.app.open_resource('flask.png', 'rb'), filename)
        tester.validate()
        ...
        with open(self.storage.path(filename)) as f_stored, self.app.open_resource('flask.png', 'rb') as f_original:
    ...
    def test_save_max_size(self):
        ...
        with self.app.open_resource('flask.png', 'rb') as f:
            with open(self.storage.path(filename_original)) as f_orig:
                with open(self.storage.path(filename)) as f_resized:
                    source = Image.open(f)
                    ...
                    self.assertAlmostEqual(resized.size[0] / resized.size[1], source.size[0] / source.size[1], 1)
    ...
    def test_save_wih_two_fields(self):
        ...
        # with self.app.open_resource('flask.png') as f:
        #     with open(self.storage.path(filename)) as f_orig:
        #         with open(self.storage.path(filename_32)) as f_32:
        #             with open(self.storage.path(filename_150)) as f_150:
        #                 source = Image.open(f)
        #                 original = Image.open(f_orig)
        #                 thumb_32 = Image.open(f_32)
        #                 thumb_150 = Image.open(f_150)
    ...
    def test_save_with_callable_basename(self):
        ...
        class Tester(db.Document):
            image = ImageField(fs=self.storage, basename=basename)
