...
If you need more information read what follows.  When you get lost
in the thicket of new terminology the `Glossary Narrative`_ in the
:ref:`glossary` should be your next port of call.
...
shows. It is an :class:`Lr1State` [#error-recovery-lie]_:
    ...
    .. class:: Lr1State(id, actions, gotos, rules)
        ...
        .. attribute:: actions
            ...
            The :term:`action table` of this parse :term:`state`.
            It is a :class:`dict`, indexed by all valid
...
That :term:`grammar` is composed of :class:`rules <Rule>` because
rules are what Python can parse.  |Lrparsing| translates the
:class:`rules <Rule>` into an alternate representation of the same
:term:`grammar` that uses :term:`productions <production>`.
Productions are what an LR(1) :term:`parser generator` understands
(this is the output produced by `MyGrammar.repr_productions()`)::
...
the :term:`tokens <token>` in a :term:`input tuple` that can contain
other information such as line and column the :term:`token` was
found in.

The :term:`parser generator` now creates a :term:`parse table`
from the :term:`productions <production>`.  To do that it goes
through an intermediate step of creating a graph.  Each node in
the graph is called an :term:`item set`.  The first :term:`item set`
is this::
...
one that representing entire input (including the special end of
input symbol `__end_of_input__`).  The :term:`non-terminal` on its
...
But the :term:`closure` is not finished yet as now
...
make are held in its :term:`action table`.  For the :term:`item set`
representing the :term:`start symbol`, the :term:`action table` will
be::
...
Each :term:`shift` has pushed the :term:`token` it accepted on a data
structure called the :term:`stack`.  The `'x'` and `'y'` on the stack
are popped from the :term:`stack` and the :term:`non-terminal`
...
just "read" in a table called the :term:`goto table`.  This table
can be displayed using `MyGrammer.repr_parse_table()`.  The
:term:`goto table` of the :term:`start symbols <start symbol>`
:term:`item set` is::
...
contain the :term:`goto table` and :term:`action table` for the node
the :term:`item set` represented.  This heavily stripped down
version of the :term:`item set` graph is the :term:`parse table`.
...
.. glossary::
  ...
  Action Table
  Shift Table
      The :term:`action table` maps a :term:`token` to another
      :term:`state`.  Each :term:`state` has its own
      :term:`action table` that lists every :term:`token` the
      ...
      as a graph, the :term:`action table` describes the lines in
      ...
      :term:`action table` lists all possible
      :term:`shift` :term:`actions <action>` that can happen when the
      :term:`parser` in a particular :term:`state`.  There is a
      similar table called a :term:`goto table` that describes the
      lines in the graph to be followed when a :term:`non-terminal`
      is recognised.  Between them, the
      :term:`action tables <action table>` and the
      :term:`goto tables <goto table>` contain all lines in the
      graph, and thus describe all possible transitions the
      :term:`parser` can make been :term:`states <state>`.  In some
      literature this table is called the :term:`shift table`.
  ...
  Goto Table
  Reduce Table
      The :term:`goto table` is a mapping of a :term:`non-terminal`
      ...
      :term:`goto table`.  Every :term:`state` has a
      :term:`goto table`, but it will be empty if all the
      ...
      thought of as a graph, the :term:`goto table` describes
      ...
      :term:`action table` and between the two they describe all
      the lines in the graph.  In the literature this table is
      sometimes called the :term:`reduce table`.
  ...
  Reduce
      ...
      in the :term:`goto table` to determine the next state.
  ...
  Shift
      ...
      :term:`action table` to determine the new :term:`state` it
      should be in.
