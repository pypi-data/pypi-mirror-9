...
def dispatch(schema, function_map, **kwargs):
    ...
    function = _select_function(schema, function_map)
    return function(
        schema=schema, function_map=function_map, **kwargs
        )


def _select_function(schema, function_map):
    """ Select function from `function_map`, depending on `schema`
        type.

        If `schema` is a subclass of `SchemaType` (and not an instance), then:
        - bypass the function mapped to `type`
        - return the function mapped to `SchemaType`
        """

    if isinstance(schema, type):
        if issubclass(schema, SchemaType):
            return _select_function(
                schema=SchemaType(), function_map=function_map
                )
...
def schema_type_dispatcher(method_name):
    ...
    def dispatcher_function(schema, data, **kwargs):
        ...
        return method(pre_dispatched_data)
