...
def MakePrimeList(length, FILESAVE=False, FILELOAD=False, DEBUG=False):
    """
    length만큼의 소수 리스트를 반환합니다

    FILESAVE는 생성한 소수 리스트를 binary file로 저장합니다.
    FILELOAD는 파일에 저장된 기존의 소수 리스트를 불러와 사용합니다.
...
def MakeRangedPrimeList(low, high, FILELOAD=False, DEBUG=False):
    """
    ...
    if len(primeList) > 0:
        if primeList[-1] >= high:
            ...
            for i in range(len(primeList)):
                if primeList[i] <= low:
                    start = i
                elif primeList[i] >= high:
                    end = i
                    break
            return primeList[start:end]
        else:            
            for prime in primeList:
                if prime >= low and prime <= high:
                    lBound = prime
                    tempList[prime] = False
                if prime > high:
                    for i in range(len(tempList)):
                        tempList[i] = not(tempList[i])
                        if i == lBound: break
    ...
    while i <= sqrt:
        if tempList[i] == True:
            for j in range(i*2, len(tempList), i):
                tempList[j] = False
        i += 1
