...
def __convert(logdir, outdir, is_open=False):
    from . import draw
...
def convert(logdir, outdir, is_open):
    __convert(logdir, outdir, is_open=is_open)
...
@click.option('-o', '--outdir', default='_out', help='Generated data dir')
@click.option('--open', 'is_open', is_flag=True, help='Open dir after finished')
def run(package, logdir, outdir, is_open):
    try:
        __start(package, logdir)
    except:
        pass
