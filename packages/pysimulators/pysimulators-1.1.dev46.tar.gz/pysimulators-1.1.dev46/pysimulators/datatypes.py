...
class FitsArray(Quantity):
    ...
    def __new__(cls, data, header=None, unit=None, derived_units=None,
        ...
        if isinstance(data, basestring):
            ...
            raise ValueError('The MPI communicator can only be set for input F'
                             'ITS files.')
    ...
    def empty(cls, shape, header=None, unit=None, derived_units=None,
        ...
        return cls(empty(shape, dtype, order), header=header, unit=unit,
                   derived_units=derived_units, dtype=dtype, copy=False,
                   **keywords)
    ...
    @classmethod
    def zeros(cls, shape, header=None, unit=None, derived_units=None,
              dtype=None, order=None, **keywords):
        if dtype is None:
            dtype = cls.default_dtype
        return cls(np.zeros(shape, dtype, order), header=header, unit=unit,
                   derived_units=derived_units, dtype=dtype, copy=False,
                   **keywords)
    ...
    def __new__(cls, data,  header=None, unit=None, derived_units=None,
        ...
        if origin is not None:
            ...
            if origin not in ('lower', 'upper'):
                raise ValueError("Invalid origin '" + origin + "'. Expected va"
...
class Tod(FitsArray):
    ...
    def mask(self, mask):
        ...
        if self.shape != mask.shape:
            raise ValueError("The input mask has a shape '" + str(mask.shape) +
                             "' incompatible with that of the Tod '" + str(
                             self.shape) + "'.")
    ...
    @classmethod
    def ones(cls, shape, mask=None, header=None, unit=None,
             derived_units=None, dtype=None, order=None, **keywords):
        if dtype is None:
            dtype = cls.default_dtype
        return cls(np.ones(shape, dtype, order), mask=mask, header=header,
                   unit=unit, derived_units=derived_units, dtype=dtype,
                   copy=False, **keywords)

    @classmethod
    def zeros(cls, shape, mask=None, header=None, unit=None,
              derived_units=None, dtype=None, order=None, **keywords):
        if dtype is None:
            dtype = cls.default_dtype
        return cls(np.zeros(shape, dtype, order), mask=mask, header=header,
                   unit=unit, derived_units=derived_units, dtype=dtype,
                   copy=False, **keywords)
    ...
    def imshow(self, mask=None, xlabel='Sample', ylabel='Detector number',
        ...
        return super(Tod, self).imshow(
            mask=mask, xlabel=xlabel, ylabel=ylabel, aspect=aspect,
            origin=origin, percentile=percentile, **keywords)
...
#        result = Tod(median(self, mask=self.mask, axis=axis),
        result = Tod(np.median(self, axis=axis),
                     ...
                     derived_units=self.derived_units, dtype=self.dtype,
                     copy=False)
...
#            result.mask.view(np.int8).ravel())
    ...
    def ravel(self, order='c'):
        ...
        return Tod(self.magnitude.ravel(), mask=mask,
                   header=self.header.copy(), unit=self.unit,
                   derived_units=self.derived_units, dtype=self.dtype,
                   copy=False)
