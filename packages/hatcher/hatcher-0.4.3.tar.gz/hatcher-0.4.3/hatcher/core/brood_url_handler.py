...
from hatcher.errors import MissingFilenameError
from .utils import hatcher_user_agent
...
class BroodURLHandler(object):
    ...
    def upload(self, path, data, filename, overwrite=False):
        """Issue a POST request to the given ``path`` containing the given json
        ``data`` and a file..
        ...
        """
        url = self._build_url(path, query={'overwrite': overwrite})
    ...
    def iter_download(self, path, destination_directory, filename=None):
    ...
    def download_file(self, path, destination_directory, filename=None):
        """Download the file specified by ``path`` and save it in the
        ``destination_directory``.
        ...
        length, iterator = self.iter_download(
            path, destination_directory, filename=filename)
