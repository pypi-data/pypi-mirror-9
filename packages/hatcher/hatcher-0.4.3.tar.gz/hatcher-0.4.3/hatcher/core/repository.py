...
class SinglePlatformRepository(object):
    ...
    def download_egg(self, python_tag, name, version, destination):
        ...
        path = self._egg_url(python_tag, name, version)
        self._url_handler.download_file(path, destination, filename=eggname)
    ...
    def iter_download_egg(self, python_tag, name, version, destination):
        ...
        return self._url_handler.iter_download(
            path, destination, filename=eggname)
    ...
    def upload_egg(self, filename, overwrite=False):
    ...
    def _egg_url(self, python_tag, name, version):
        return URLS.data.eggs.download.format(
            organization_name=self.organization_name,
            repository_name=self.repository_name,
            platform=self.name,
            name=name,
    ...
    def download_runtime(self, python_tag, version, destination):
        ...
        self._url_handler.download_file(path, destination)
    ...
    def iter_download_runtime(self, python_tag, version, destination):
        ...
        path = self._runtime_url(
            URLS.data.runtimes.download, python_tag, version)
        return self._url_handler.iter_download(path, destination)
...
class Repository(object):
    ...
    def upload_runtime(self, filename, overwrite=False):
        ...
        if m is None:
            raise ValueError('Invalid format for {0}'.format(filename))

        d = m.groupdict()
        metadata = {
            'language': d['language'],
            'filename': basename,
            'build_system_version': d['build_system_version'],
            'version': d['version'],
            'file_format': d['extension'],
            'build': int(d['build']),
