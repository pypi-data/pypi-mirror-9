...
class TestBatchUploadEggsMain(MainTestingMixin, unittest.TestCase):
    ...
    def test_with_error(self, Repository, progress, puts, colored):
        ...
        calls = [call(path, overwrite=False) for path in paths]
        called = [call_ in upload_egg.mock_calls for call_ in calls]
        self.assertTrue(
            any(called),
            msg='Expected one of the calls {} in set'.format(
                calls, upload_egg.mock_calls))

        puts_calls = [call('Error uploading {}'.format(path))
                      for path in paths]
        called = [puts_call in puts.mock_calls for puts_call in puts_calls]
        puts.assert_has_calls([call('')])
        self.assertTrue(
            any(called),
            msg='Expected one of the calls {} in set {}'.format(
                puts_calls, puts.mock_calls))
