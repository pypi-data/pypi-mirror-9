...
def lastedited(howmany=10):
    ...
    git_your_last_changed_files = [x for x in subprocess.check_output(["git", "whatchanged", "--pretty=format:\"%ae\"", "-n", "512",]).split("\n") if os.path.exists(x)]
    for line in git_your_last_changed_files:
        if re.compile(r"\"(.*?)\@(.*?)\"").match(line) is not None:
            if line == "\"{0}\"".format(your_email):
                my_commits = True
            else:
                my_commits = False
        elif line == "":
    ...
    for filename_line in my_filename_lines:
        ...
        if filename not in last_changed_files:
