...
def add_risk_value_types(cls):
    ...
    def RiskMetrics(self, graph):
        ...
        # --- load fx to convert RiskMktValues from USD to the Denominated
        #     currency
        cross = "{0:3s}/USD".format(graph(self, "Denominated"))
        fx = graph(cross, "Spot")

        # --- these curves are in USD
        mkt_values = graph(self, "RiskMktValues", start, end) / fx
