...
GET_MKS_QUERY = """
SELECT Date, Data->'_p_vt_data'->'Price'
FROM Archive
WHERE ObjName=%s AND Date BETWEEN COALESCE(%s,%s) AND COALESCE(%s,%s)
ORDER BY Date;"""

GET_MOST_RECENT = """
SELECT MAX(Date) FROM Archive
WHERE ObjName=%s AND Date <= COALESCE(%s,%s)
"""
...
class PriceFix(Archivable):
    ...
    def LastBefore(self, graph, date=None):
        ...
        res = ObjDbQuery(GET_MOST_RECENT, parms, attr="fetchone")[0]
        if res is None:
