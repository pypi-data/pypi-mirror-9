...
from agora.risk.core_functions import add_risk_value_types
...
class Trade(UfoBase):
    ...
    def __post_init__(self):
        ...
        # --- set default values
        self.PremiumDen = self.PremiumDen or "USD"

        if self.SettlementDate is None:
            cal = GetVal(self.PremiumDen, "HolidayCalendar")
    ...
    def Leaves(self, graph):
        ...
        if trd_type in ("Buy", "Sell"):
            ...
            cash = ForwardCash(Currency=den, PaymentDate=sett_dt)
    ...
    def PositionEffects(self, graph):
        ...
        for sec_info in securities:
            ...
            leaf = AddByInference(sec_info["Security"], in_memory=False)
            qty = sec_info["Quantity"]*trd_qty
            poseff += [
...
def prepare_for_test():
    ...
    trade_info = {
        ...
        "Quantity": 1000,
        "Party": books[0],
        "Counterparty": books[1],
        "Trader": traders[0],
        "Broker": brokers[0],
    }

    trades = [AddIfMissing(Trade(**trade_info))]
