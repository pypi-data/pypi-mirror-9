...
from echo import StreamServerRequest

from echo import send_request
...
try:
    # Build Submit API request: HTTP POST with params...
    param_d = {
        'content': test_data,
        'mode': 'update',
    }
    param_str = urllib.urlencode(param_d)

    req = StreamServerRequest(data=param_str)
    print "API URL: %r" % req.api_url
    print "Submit DATA: %r" % req.data
