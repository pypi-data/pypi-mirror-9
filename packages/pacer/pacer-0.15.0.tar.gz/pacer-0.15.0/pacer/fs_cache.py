...
class _CachedFunction(object):
    ...
    def _setup_cache(self):
        ...
        for file_name in os.listdir(self.folder):
            ...
            if "----" in stem:
                hash_code, __, name = stem.partition("----")
            else:
                hash_code, name = stem, None
            path = os.path.join(self.folder, file_name)
            self._cache[hash_code] = CacheItem(name, path, None, False, hash_code)
    ...
    def _put(self, name, hash_code_args, args, result):
        path = self._store(name, result, hash_code_args)
        item = CacheItem(name, path, None, False, hash_code_args)
    ...
    def _store(self, name, what, hashcode_fingerprint):
        ...
        if name is None:
            fname_base = hashcode_fingerprint
        else:
            fname_base = hashcode_fingerprint + "----" + name
        ...
        if ext is None or save_function is None:
            ...
            def save_function(what, path):
                with open(path, "wb") as fp:
                    dill.dump(what, fp)
        path = os.path.join(self.folder, fname_base + ext)
