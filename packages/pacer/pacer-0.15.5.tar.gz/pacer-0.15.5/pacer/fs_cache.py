...
class CacheItem(object):
    ...
    def __init__(self, name, path, value, loaded, hash_code, class_name):
        self.path = path
        self.loaded = loaded
        self.name = name
    ...
    def __str__(self):
        return "<CachetItem name=%s path=%s value=%r loaded=%s hash_code=%s class_name=%s>" % (
                self.name, self.path, self.value, self.loaded, self.hash_code, self.class_name)
...
class _CacheBuilder(object):
    ...
    def load(self, item):
        ...
        item.value = obj
        assert item.class_name == str(obj.__class__), "conceptional error, should never happen !"
        return obj
...
class _CachedFunction(object):
    ...
    def _store(self, name, what, hashcode_fingerprint):
        ...
        item = CacheItem(name, path, None, False, hashcode_fingerprint, str(what.__class__))
    ...
    def __str__(self):
        return "<_CachedFunction(%s)>" % self.function
...
class _LazyCachedFunction(_CachedFunction):
    ...
    def __str__(self):
        return "<_LazyCachedFunction(%s)>" % self.function
