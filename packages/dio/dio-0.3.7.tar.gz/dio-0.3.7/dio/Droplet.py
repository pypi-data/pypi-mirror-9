...
class Droplet( object ):
  ...
  def get_snapshots( self ):
    ...
    for id in self.snapshot_ids:
      data = self.api.call( "/images/%s" % id)
      if "image" in data:
        snapshot = Snapshot( self.__token, "", data["image"], self.api )
        self.snapshots.append( snapshot )
    return self.snapshots

  def get_actions( self ):
    self.actions = []
    data = self.api.call( self.actions_path )
    if "actions" in data:
      for dict in data["actions"]:
        self.actions.append( Action(dict) )
    return self.actions
  
  def get_available_kernels( self ):
    self.available_kernels = []
    data = self.api.call( self.kernels_path)
    if "kernels" in data:
      for dict in data["kernels"]:
        kernel = Kernel( dict )
