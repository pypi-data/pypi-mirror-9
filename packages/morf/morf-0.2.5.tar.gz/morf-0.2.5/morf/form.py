...
class Form(fields.FieldBase):
    ...
    def _initfields(self):
        ...
        if self.fields:
            ...
            fields = getmembers(self, _isfield)

        fields = sorted([(name, copy(f)) for name, f in fields],
                         key=lambda a: a[1].ordering)
    ...
    def __setstate__(self, state):
        ...
        # Reinit fields to ensure that the field has the correct name in the
        # copied context (eg a subform would have its fieldnames prefixed by
        # the parent fieldname).
        self._initfields()
