...
class TaskStateManager(object):
    ...
    def iterate_keys(self, *args, **kwargs):
        for key in self.db.RangeIter(include_value=False, *args, **kwargs):
            if not key.startswith('__'):
                yield key

    def iterate_key_values(self, *args, **kwargs):
        return ((k, json.loads(v)) for k, v in self.db.RangeIter(include_value=True, *args, **kwargs) if not k.startswith('__'))
...
class TaskState(collections.MutableMapping):
    ...
    def range(self, *args, **kwargs):
        ...
        if include_value:
            return self._mgr.iterate_key_values(*args, **kwargs)
        else:
            return self._mgr.iterate_keys(*args, **kwargs)

    # def _iterate_key_values(self, *args, **kwargs):
    #     # merge data from manager with data from _cache
    #     sorted_changes = sorted(self._changes)
    #     sorted_deletes = sorted(self._deletes)

    #     for key, val in self._mgr.iterate_key_values(*args, *kwargs):
    #         # yield all values from changed where x < key and x >= key_from
    ...
    #     # for key in changes[new_key_from:key_to]
    #     # yield key, self._cache[key]


    # def _iterate_keys(self, *args, **kwargs):
    #     pass
