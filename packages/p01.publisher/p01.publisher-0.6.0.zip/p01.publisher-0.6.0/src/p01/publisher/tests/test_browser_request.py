...
from p01.publisher.publisher import publish as publish_
from p01.publisher._compat import PYTHON2
from p01.publisher.tests.test_http_request import HTTPTests
from p01.publisher.tests.publication import DummyPublication
from p01.publisher.tests.publication import DefaultPublication
...
def publish(request):
    publish_(request, handleErrors=0)
...
class TestBrowserRequest(p01.publisher.request.BrowserRequest, HTTPCharsets):
    """Make sure that our request also implements IHTTPCharsets, so that we do
    not need to register any adapters."""

    def __init__(self, *args, **kw):
        self.request = self
        p01.publisher.request.BrowserRequest.__init__(self, *args, **kw)
...
class BrowserTests(HTTPTests):
    ...
    def testInterface(self):
        ...
        verifyObject(interfaces.IBrowserRequest, request)
        verifyObject(IBrowserApplicationRequest, request)
        verifyObject(ISkinnable, request)

    def test_IBrowserResponse_providedBy(self):
        request = self._createRequest()
        response = request.response
        self.assertTrue(interfaces.IBrowserResponse.providedBy(response))
        verifyObject(interfaces.IBrowserResponse, response)
    ...
    def test_Default_POST(self):
        ...
        5
        -----------------------------1
        Content-Disposition: form-data; name="b:int"
    ...
    def test_NoneFieldNamePost(self):
    ...
    def test_FileUpload(self):
        """Produce a SpooledTemporaryFile as upload"""
        ...
        request.processInputs()
        self.assertTrue(request.form['upload'])
    ...
    def testQueryStringIgnoredForPOST(self):
        ...
        5
        -----------------------------1
        Content-Disposition: form-data; name="d:int"
    ...
    def testFormStringTypes(self):
        ...
        request = self._createRequest(extra)
        publish(request)
        self.assertEqual(request.form, {u'a': u'test'})
    ...
    def testFormBooleanTypes(self):
        ...
        request = self._createRequest(extra)
        publish(request)
        self.assertEqual(request.form, {u'b': True, u'c': True})
...
@zope.interface.implementer(interfaces.IBrowserPublication)
class TestBrowserPublication(DummyPublication):
...
class APITests(BaseTestIPublicationRequest,
    ...
    def _Test__new(self, environ=None, **kw):
        if environ is None:
            environ = kw
        return p01.publisher.request.BrowserRequest(BytesIO(b''), environ)

    def test_IApplicationRequest_bodyStream(self):
        request = p01.publisher.request.BrowserRequest(BytesIO(b'spam'), {})
    ...
    def test_IBrowserRequest(self):
        verifyObject(interfaces.IBrowserRequest, self._Test__new())
