...
def memoize(func, cache, num_args):
    ...
    :param cache:
        a :class:`dictionary` to store the result in.
    :param num_args:
        the number of arguments from `func(*args)` to use when
        constructing the cache key; e.g. when `num_args` is ``2``
        then the key ``func(['foo','bar','baz']) -> args[:2]``
    """
    @wraps(func)
    def wrapper(*args):
