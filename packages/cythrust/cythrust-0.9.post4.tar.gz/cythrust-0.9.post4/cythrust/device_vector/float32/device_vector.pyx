...
cdef class DeviceVector:
    def __cinit__(self, size_t size=0):
        self._vector = new device_vector[float](size)
        self.dtype = DTYPE
        self.ctype = CTYPE

    @classmethod
    def from_array(cls, np.ndarray a):
        cdef np.ndarray _a = np.ascontiguousarray(a, dtype=np.float32)
        cdef size_t N = _a.size
        cdef DeviceVector vector = DeviceVector(N)
        cdef float *data = <float*>&_a.data[0]
    ...
    def __setitem__(self, key_or_slice, value):
        ...
        if isinstance(key_or_slice, slice):
            ...
            N = j - i
            assert(N <= self.size)
            _values = np.ascontiguousarray(value, dtype=np.float32)
            if _values.size == 1:
                # Input was a single value.
                fill_n(self._vector.begin() + <size_t>i, N, <float>_values[0])
            elif _values.size == N:
                copy_n(<float*>_values.data, N, self._vector.begin() +
                       <size_t>i)
            else:
                raise ValueError('Incorrect slice length.  Expected %d, but got %d'
                                % (N, _values.size))
        else:
            fill_n(self._vector.begin() + <size_t>key_or_slice, 1, <float>value)
...
cdef class DeviceVectorView:
    ...
    property first_i:
        ...
        def __set__(self, value):
            ...
            cdef device_vector[float].iterator begin
    ...
    property last_i:
        ...
        def __set__(self, int64_t value):
            if value < 0:
                value += self._vector.size()
            cdef device_vector[float].iterator end
    ...
    def __setitem__(self, key_or_slice, value):
        ...
        if isinstance(key_or_slice, slice):
            ...
            if _values.size == 1:
                # Input was a single value.
                fill_n(self._begin + <size_t>i, N, <float>_values[0])
            elif _values.size == N:
                copy_n(<float*>_values.data, N,
                    self._begin + <size_t>i)
            else:
                raise ValueError('Incorrect slice length.  Expected %d, but got %d'
                                % (N, _values.size))
        else:
            fill_n(self._begin + <size_t>key_or_slice, 1, <float>value)
    ...
    def __getslice__(self, i, j):
        ...
        cdef int N = j - i
        cdef np.ndarray result = np.empty(N, dtype=np.float32)
        copy_n(self._begin + <size_t>i, N, <float*>result.data)
        return result
