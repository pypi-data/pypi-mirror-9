...
cdef class DeviceVector:
    def __cinit__(self, size_t size=0):
        self._vector = new device_vector[int64_t](size)
        self.dtype = DTYPE
        self.ctype = CTYPE

    @classmethod
    def from_array(cls, np.ndarray a):
        cdef np.ndarray _a = np.ascontiguousarray(a, dtype=np.int64)
        cdef size_t N = _a.size
        cdef DeviceVector vector = DeviceVector(N)
        cdef int64_t *data = <int64_t*>&_a.data[0]
    ...
    def __getslice__(self, i, j):
        ...
        assert(i <= j)
        cdef int N = j - i
        cdef np.ndarray result = np.empty(N, dtype=np.int64)
    ...
    def __setitem__(self, key_or_slice, value):
        ...
        if isinstance(key_or_slice, slice):
            ...
            N = j - i
            assert(N <= self.size)
            _values = np.ascontiguousarray(value, dtype=np.int64)
            if _values.size == 1:
                # Input was a single value.
                fill_n(self._vector.begin() + <size_t>i, N, <int64_t>_values[0])
            elif _values.size == N:
                copy_n(<int64_t*>_values.data, N, self._vector.begin() +
                       <size_t>i)
            else:
                raise ValueError('Incorrect slice length.  Expected %d, but got %d'
                                % (N, _values.size))
        else:
            fill_n(self._vector.begin() + <size_t>key_or_slice, 1, <int64_t>value)
...
cdef class DeviceVectorView:
    ...
    property first_i:
        ...
        def __set__(self, value):
            ...
            cdef device_vector[int64_t].iterator begin
    ...
    property last_i:
        ...
        def __set__(self, int64_t value):
            if value < 0:
                value += self._vector.size()
            cdef device_vector[int64_t].iterator end
    ...
    def __setitem__(self, key_or_slice, value):
        ...
        if isinstance(key_or_slice, slice):
            ...
            if _values.size == 1:
                # Input was a single value.
                fill_n(self._begin + <size_t>i, N, <int64_t>_values[0])
            elif _values.size == N:
                copy_n(<int64_t*>_values.data, N,
                    self._begin + <size_t>i)
            else:
                raise ValueError('Incorrect slice length.  Expected %d, but got %d'
                                % (N, _values.size))
        else:
            fill_n(self._begin + <size_t>key_or_slice, 1, <int64_t>value)
    ...
    def __getslice__(self, i, j):
        ...
        cdef int N = j - i
        cdef np.ndarray result = np.empty(N, dtype=np.int64)
        copy_n(self._begin + <size_t>i, N, <int64_t*>result.data)
        return result
