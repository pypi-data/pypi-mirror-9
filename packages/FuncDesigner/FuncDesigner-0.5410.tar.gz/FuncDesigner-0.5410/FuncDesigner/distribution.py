        ...
        if ppf is not None:
            ...
            if N is None:
                ...
                if quantiles is None:
                    quantiles = np.arange(1.0, float(N+1)) / (N+2)
                else:
...
def gauss(m = 0.0, sigma = 1.0, N = None, quantiles = None):
    ...
    r = continuous(ppf = ppf_engine, quantiles = quantiles, N = N, Str = 'gauss (normal) distribution with m = %g and sigma = %g' % (m, sigma))
#    r.Mean = m
#    r.Var = sigma**2
#    r.Std = sigma
    return r
...
def uniform(location = 0.0, scale = 1.0, N = None, quantiles = None):
    ...
    r = continuous(ppf = ppf_engine, quantiles = quantiles, N = N, Str = 'uniform distribution with lb = %g and ub = %g' % (location, scale))
...
def mergeDistributions(d1, d2, operation):
    ...
    if not is_d1_stoch or not is_d2_stoch or cond_same_stoch:
        ...
        New = 1
        if New and np.all(d1.probabilities == d1.probabilities[0]) and np.all(d2.probabilities == d2.probabilities[0]):
...
def f_quantile(distrib, val, interpolate):
    ...
    if isinstance(distrib, multiarray):
...
def d_quantile(distrib, val, interpolate):
    ...
    assert not isinstance(distrib, multiarray), 'd_quantile is unimplemented for multiarrays'
    ...
    r = (vals[ind+1] - val) / (vals[ind+1] - vals[ind]) 
...
def mean(arg, *args, **kw):
    if isinstance(arg, stochasticDistribution):
        ...
        elif arg._isProd and (np.isscalar(arg._prod_elements[-1])\
...
def std(arg, *args, **kw):
    if isinstance(arg, stochasticDistribution):
        ...
        if arg._isProd and (np.isscalar(arg._prod_elements[-1])\
        or (isinstance(arg._prod_elements[-1], ndarray) and not isinstance(arg._prod_elements[-1], OOArray))):
            multiplier, tmp = arg._fixed_part, arg._unfixed_part
