...
class FacebookAPIClient(object):
    ...
    def local_object_to_remote_object(self, local_object):
        """
        Maps an AdRoll Facebook MirrorObject to a facebookads object and
        sets the default read fields with MirrorObject.get_read_fields()
        ...
        account_id = getattr(local_object, 'account_id_string', None)
    ...
    #--------------------------------#
    # AdAccount Report Stats Methods #
    #--------------------------------#

    def get_report_stats(self, facebook_account, params):
        """
        Accesses Facebook's report stats api and returns the results

        Args:
            facebook_account: Local representation of a Facebook AdAccount
            params: what params to be passed to the api end point specifying filters

        Returns:
            Results of pinging the report stats api
        """
        remote_object = self.local_object_to_remote_object(facebook_account)
        remote_object.update(facebook_account.marshal_for_read())
        return remote_object.get_report_stats(params=params)

    def get_relevance_scores(self, facebook_account, date_preset='today'):
        """
        A wrapper for get_report_stats method to grab relevance scores.

        Args:
            facebook_account: Local representation of a Facebook AdAccount
            date_preset: value that indicates which days will be returned by the report
        """

        params = {'data_columns':['adgroup_id', 'relevance_score'], 'date_preset':date_preset}
        return self.get_report_stats(facebook_account, params)
