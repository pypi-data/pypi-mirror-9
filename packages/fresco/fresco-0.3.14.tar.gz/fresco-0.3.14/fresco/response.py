...
class Response(object):
    ...
    def __init__(self, content=None, status=None, headers=None,
                 onclose=None, _nocontent=[], passthrough=False, **kwargs):
        """
        ...
        self.status = make_status(status)
        if onclose is None:
            self.onclose = []
        ...
        elif content_islist and len(content) == 0:
            self.content_iterator = content
        elif content_islist and isinstance(content[0], bytes):
            self.content_iterator = content
        elif content_islist and isinstance(content[0], ustr):
            charset = self.charset
            self.content_iterator = [c.encode(charset) for c in content]
        elif isinstance(content, bytes):
            self.content_iterator = [content]
        elif isinstance(content, ustr):
    ...
    def add_header(self, name, value):
        ...
        """
        return self.replace(headers=self.headers + [(name, value)])
    ...
    def remove_headers(self, *headers):
        ...
        return self.replace(headers=[h for h in self.headers
                                       if h[0].lower() not in toremove])

    def add_cookie(self, name, value, maxage=None, expires=None, path=None,
        ...
        """
        return self.add_header('Set-Cookie', Cookie(name, value, maxage,
                                                    expires, path, secure,
                                                    domain,
                                                    comment=comment,
                                                    http_only=http_only))
    ...
    def add_vary(self, *vary_on):
        ...
        for k, v in self.headers:
            if k.lower() == 'vary':
                vary_on.update(s.strip() for s in v.split(','))
            else:
                newheaders.append((k, v))
        return self.replace(
            headers=newheaders + [('Vary', ', '.join(vary_on))])
    ...
    def replace(self, content=None, status=None, headers=None, **kwheaders):
        ...
        return self.__class__(content, status, headers, onclose=onclose)
