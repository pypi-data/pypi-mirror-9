...
class Glower(object):
	...
	def _request(self, endpoint, data, files=None):
		data['fail_if_mistakes'] = "yes" if self.fail_if_mistakes else "no"
		data['delete_previous_data'] = "yes" if self.delete_previous_data else "no"
		data['save_data'] = "yes" if self.save_data else "no"
		data['stats'] = "yes" if self.stats else "no"
		data['hold'] = "yes" if self.hold else "no"
		data['update'] = "yes" if self.update else "no"
		data['max_number'] = self.max_number
		data['accuracy'] = "yes" if self.accuracy
