...
#ATTENTION RIEN n'EST TESTE : juste une fausse piste!!
...
class lcd_12864_io(lcd_io):
	""" Ecran graphique LCD 12864 branché en série SPI
	"""
	
	def __init__(self, pin_cs, pin_rst, pin_a0, pin_clk, pin_si, width = 128, lines = 64):
		"""Initialisation
			- pin_cs	:	pin_io	CS
			- pin_rst	:	pin_io	RST
			- pin_a0	:	pin_io	A0
			- pin_clk	:	pin_io	CLK
			- pin_si	:	pin_io	SI
			width = largeur de l'écran en pixel
			lines : nb de lignes en pixel
		"""
		self.pin_cs = pin_cs
		...
		self.pin_rst.setmode(OUTPUT)
		self.pin_a0.setmode(OUTPUT)
		self.pin_clk.setmode(OUTPUT)
		self.pin_si.setmode(OUTPUT)
		self.pin_cs.set(HIGH)
		self.pin_rst.set(LOW)
		self.pin_rst.set(HIGH)
		self.send
	...
	def ascii168_string(self, xPos, yPos, string):
		stringLen = len(string)
		for i in range(0, stringLen):
			self.ascii168(xPos+i*8,yPos,ord(string[i])-32)
	
	def ascii168(self.xPos, yPos, char):
		self.set_page(yPos,xPos)
		for i in range(0, 8): 
			self.tranfer_data(lcd_12864_io.ASCII168[char][i],1)
		self.set_page(yPos+1,xPos)   
		for i in range(8, 16): 
			self.tranfer_data(lcd_12864_io.ASCII168[char][i],1)
	...
	def clear(self):
		...
		for i in range(0, 8):            
			...
			for j in range(0, 128):
				self.tranfer_data(0x00,1)
		self.pin_cs.set(HIGH)
	
	def set_page(self, page, column):
		lsb = column & 0x0f
		msb = column & 0xf0
		msb = msb>>4
		msb = msb | 0x10                  
		page = page | 0xb0                               
		self.tranfer_data(page,0)
		self.tranfer_data(msb,0)
		self.tranfer_data(lsb,0)
	
	def tranfer_data(self, value, SI):
		self.pin_cs.set(LOW)
		self.pin_clk.set(HIGH)
		if(SI):
			self.pin_a0.set(HIGH)
		else:
			self.pin_a0.set(LOW)
		self.byte(value)
		self.pin_cs.set(HIGH)
...
if __name__ == '__main__':
	pc=rpiduino()
	lcd = lcd_12864_io(14,255,255,12,13)
	lcd.ascii168_string(0,0,"This is demo of")
	lcd.ascii168_string(0,2,"raspberry pi LCD")
	lcd.ascii168_string(0,4,"0123456789:;<=>?")
	lcd.ascii168_string(0,6,"@ABCDEFGHIJKLMNO")
