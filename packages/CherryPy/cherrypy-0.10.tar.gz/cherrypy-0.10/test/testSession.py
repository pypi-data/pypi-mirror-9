...
import testCode, os, shutil

# Test that we get a correct error message if we try to use sessions without enabling them in the config file
code1="""
CherryClass Root:
view:
    def index(self):
        request.sessionMap['dummy']=1
"""
...
urlList1=[""]
expectedResultList1=['You are trying to use sessions but sessions are not enabled']
...
view:
    def index(self):
        request.sessionMap['counter']=request.sessionMap.get('counter', 0)+1
        return str(request.sessionMap['counter'])
"""
...
urlList2=["", "", ""]
expectedResultList2=["1", "2", "3"]

def test(infoMap, failedList, skipped):
    print "    Testing sessions..."
    print "        (1)....",
    testCode.checkPageResult("Sessions (1)", infoMap, code1, config1, "", urlList1, expectedResultList1, failedList, 0)
    print "        (2)....",
    testCode.checkPageResult("Sessions (2)", infoMap, code2, config2, "", urlList2, expectedResultList2, failedList, 0)
    print "        (3)....",
    try: shutil.rmtree('sessionData')
    except: pass
    os.mkdir('sessionData')
    testCode.checkPageResult("Sessions (3)", infoMap, code2, config3, "", urlList2, expectedResultList2, failedList, 0)
    print "        (4)....",
    testCode.checkPageResult("Sessions (4)", infoMap, code2, config4, "", urlList2, expectedResultList2, failedList, 0)
