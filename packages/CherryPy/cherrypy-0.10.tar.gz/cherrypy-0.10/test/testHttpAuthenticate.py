...
# Test HttpAuthenticate
code1="""use HttpAuthenticate
CherryClass Protected(HttpAuthenticate):
mask:
    def index(self):
        OK

CherryClass HiddenProtected(HttpAuthenticate) hidden:
mask:
    def index(self):
        OK
...
urlList=["protected/index", "normal/index"]
expectedResultList=["<html><body>Unauthorized</body></html>\n", "OK\n"]

def test(infoMap, failedList, skipped):
    print "    Testing HttpAuthenticate classes...",
    testCode.checkPageResult("HttpAuthenticate", infoMap, code1, config1, "", urlList, expectedResultList, failedList)
