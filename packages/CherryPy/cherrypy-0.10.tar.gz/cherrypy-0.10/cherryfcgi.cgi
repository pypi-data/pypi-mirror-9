...
import  os, sys, string, socket, errno, urllib
...
def _test():
    ...
    try:
        while isFCGI():
            ...
            req = Accept()
            #fs = req.getFieldStorage() # Don't read parameters

            if req.env.has_key('CONTENT_LENGTH'): length=int(req.env['CONTENT_LENGTH'])
            else: length=0
            if length: data=req.inp.read(length)
            else: data=""
    
            s=socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            s.connect(socketFile)
            s.send('%s %s HTTP/1.1\r\n'%(req.env['REQUEST_METHOD'], req.env['REQUEST_URI']))
            ...
            if req.env.has_key('HTTP_USER_AGENT'):
                s.send('User-Agent: %s\r\n'%req.env['HTTP_USER_AGENT'])
            if req.env.has_key('HTTP_CGI_AUTHORIZATION'):
                s.send('Authorization: %s\r\n'%req.env['HTTP_CGI_AUTHORIZATION'])
            if req.env.has_key('CONTENT_LENGTH'):
                s.send('Content-Length: %s\r\n'%req.env['CONTENT_LENGTH'])
            if req.env.has_key('CONTENT_TYPE'):
                s.send('Content-Type: %s\r\n'%req.env['CONTENT_TYPE'])
            if req.env.has_key('HTTP_COOKIE'):
                s.send('Cookie: %s\r\n'%req.env['HTTP_COOKIE'])
            if req.env.has_key('USER_AGENT'):
                s.send('User-Agent: %s\r\n'%req.env['USER_AGENT'])
            if req.env.has_key('REMOTE_HOST'):
                s.send('Remote-Host: %s\r\n'%req.env['REMOTE_HOST'])
            if req.env.has_key('REMOTE_ADDR'):
                s.send('Remote-Addr: %s\r\n'%req.env['REMOTE_ADDR'])
            if req.env.has_key('HTTP_REFERER'):
                s.send('Referer: %s\r\n'%req.env['HTTP_REFERER'])
            if req.env.has_key('HTTP_ACCEPT_ENCODING'):
        ...
        pass
        # Uncomment the following lines if you want the error to be saved in "/tmp/traceback"
        #import traceback
