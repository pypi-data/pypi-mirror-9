...
__revision__ = '$Id: xmldiff.py,v 1.31 2003/10/02 10:07:50 syt Exp $'
...
def process_files(file1, file2, norm_sp, xupd, ezs, verbose,
    ...
    if verbose:
        from logilab.xmldiff.objects import repr, N_ISSUE, N_CHILDS
        print "Source tree\n", repr(tree1)
    ...
    # output formatter
    if xupd:
        from logilab.xmldiff.format import XUpdatePrinter
        formatter = XUpdatePrinter()
    else:
        from logilab.xmldiff.format import InternalPrinter
...
def run(*args):
    """
    Main. To be called with list of command-line arguments.
    ...
    if os.path.isdir(fpath1) and os.path.isdir(fpath2):
        from logilab.xmldiff.misc import process_dirs, list_print
        ...
        for filename in common[0]:
            print '-'*80
            print 'FILE:', filename
            diffs = process_files(os.path.join(fpath1, filename), os.path.join(fpath2, filename),
    ...
    # if  args are files
        ...
        if prof:
            ...
            profile.run('process_files(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'% (
              repr(fpath1), repr(fpath2), repr(norm_sp), repr(xupd),
            ...
            exit_status = process_files(fpath1, fpath2, norm_sp, xupd, ezs, verbose,
                                        ext_ges, ext_pes, include_comment, encoding, html)
    else:
        exit_status = -1
