...
__revision__ = "$Id: fmes.py,v 1.18 2003/09/27 14:46:29 nico Exp $"

from logilab.xmldiff.objects import NT_ROOT, NT_NODE, NT_ATTN, NT_ATTV, \
...
from logilab.xmldiff.mydifflib import lcs2, quick_ratio
from logilab.xmldiff.misc import intersection, in_ref, index_ref
# c extensions
from logilab.xmldiff.maplookup import has_couple , partner, fmes_init, \
...
##                             else: 
                ...
                if w[N_VALUE] != x[N_VALUE]:
                    ...
                    if w[N_TYPE] == NT_NODE:
                        ...
                        attr_name = self._before_attribute(w[N_PARENT], w, x[N_VALUE])
...
#            print 'after', node_repr(tree1)
    ...
    def _make_move(self, n1, n2, k):
        ...
        if n1[N_TYPE] == NT_TEXT:
            ...
            # avoid to move an attribute node from a place to another on the same node
    ...
    def _before_attribute(self, parent_node, attr_node, new_name=None):
        ...
        for w in parent_node[N_CHILDS]:
            ...
            if w[N_VALUE] == attr_name:
                new_name = 'LogilabXmldiffTmpAttr%s'%attr_name.replace(':', '_')
