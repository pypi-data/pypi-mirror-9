...
def make_mlp(train, devel, batch_size, nhidden, epoch, max_iter=0,
  ...
  shuffler = bob.learn.mlp.DataShuffler(train, target)
  shuffler.auto_stdnorm = True

  shape = (shuffler.data_width, nhidden, 1)
  machine = bob.learn.mlp.Machine(shape)
  #machine.activation = bob.learn.activation.HyperbolicTangent() #the defaults are anyway Hyperbolic Tangent for hidden and output layer
  machine.randomize()
  #import ipdb; ipdb.set_trace()
  machine.input_subtract, machine.input_divide = shuffler.stdnorm()

  trainer = bob.learn.mlp.RProp(batch_size, bob.learn.mlp.SquareError(machine.output_activation), machine)
  ...
  best_machine = bob.learn.mlp.Machine(machine) #deep copy
