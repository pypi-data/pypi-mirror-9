...
class MultiDownloadThread(threading.Thread):
    ...
    def __init__(self, url, file_name, file_path, id=0, ids=0, quiet=False, modified=True):
...
class Download:
    '''
        download class
    '''
    def __init__(self, quiet=False, modified=True):
        self.__url = '' # 1
        ...
        # quiet == False stands for log; Or no log.(Default, log(False))
        self.__quiet = quiet
        # if file had been modified, download or not?Default download(True)
    ...
    def _do_leave_now(self):
        return self.__leave_now

    def _set_all_info(self, url, file_name, file_path):
        assert url
        assert file_name
        self.set_url(url) # 1
        # self.set_internet_available()
        self.set_file_path_and_name(file_name, file_path) # 2
        self._set_server_response_header() # 3
        # if internet is not available, leave
        if self._do_leave_now():
            return 

        self._set_accept_range() # 4
        self._set_open_file_mode() # 5
        self._set_content_length() # 6
        self._set_file_size_unit() # 7 
        self._set_file_name_exists() # 8
        self._set_tmp_file_name_exists() # 9
        self._set_tmp_file_name_size() # 10
        self._set_range_headers() # 11
        self._set_all_file_size() # 
        self._set_file_modified() # 13 @TODO
        self._set_requests_stream_object() # 12

    def isFileDownloaded(self):
        return self.__file_already_download

    def _set_all_file_size(self):
    ...
    def _set_server_response_header(self):
        assert self.__url
        try:
    ...
    def _set_accept_range(self):
        '''
            Simple Check Accept-Ranges: 
                Only headers exist `Accept-Ranges` is OK
        '''
        assert self.__url
    ...
    def _set_file_name_exists(self):
    ...
    def _set_tmp_file_name_exists(self):
    ...
    def _set_tmp_file_name_size(self):
        if not self.__accept_range:
            return
    ...
    def _set_requests_stream_object(self):
        assert self.__url
    ...
    def _set_file_modified(self):
        ...
        if self.__file_name_exists:
            ...
            # local_file_size = self._get_file_name_size()
            ...
            #          (self.__file_name, self._get_file_name_size()))

            # 是否要下载已经修改过的文件，　默认为 True 
            if self.__modified:
                return
    ...
    def calculate_time(self, left_time):
        ...
        second = int(left_time % 60)
        second = '0' + str(second) if second < 10 else str(second)
        minute = int(left_time / 60 % 60)
    ...
    def download(self, url, file_name, file_path=None, id=1, ids=1):
        """
            download main
        ...
        assert file_name
        self._reset()
        self._set_all_info(url, file_name, file_path)
        ...
        if self.isFileDownloaded():
            if not self.__quiet:
                ...
                id_log = ('%s/%s' % (id, ids)).center(8)
                file_log = 'ID: [%s] File: %s ' % (id_log, self.__file_name)
        ...
        id_log = ('%s/%s' % (id, ids)).center(8)
        modified = '' if not self.__file_modified else 'Modified'
        file_log = 'ID: [%s] File: %s %s [ %.2f %s ]' % (
                        id_log,
                        cut_str(self.__file_name, 15), 
                        modified,
                        all_size,
                        all_size_unit,
                        )
...
def cut_str(str, length=10):
    ...
    if is_encode:
        ...
        if l < len(str_encode):
            ...
            l = l - 3
            str_encode = str_encode.ljust(l, ' ')[:l]
            try:
                str = str_encode.decode('gb18030')
            except:
