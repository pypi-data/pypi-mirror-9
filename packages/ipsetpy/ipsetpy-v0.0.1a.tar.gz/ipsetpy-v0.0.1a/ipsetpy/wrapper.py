...
def ipset_create_set(set_name, type_name, exist=False):
    arguments = ["create", set_name, type_name]
    if exist:
        arguments.append('-exist')
    return ipset_send_command(*arguments)


def ipset_add_entry(set_name, entry, timeout=None, exist=False):
    arguments = [IPSET_COMMAND, "add", set_name, entry]
    if timeout:
        arguments.append("timeout")
        arguments.append("%d" % (int(timeout),))
    if exist:
        arguments.append('-exist')
    return ipset_send_command(*arguments)


def ipset_del_entry(set_name, entry, exist=False):
    arguments = [IPSET_COMMAND, "add", set_name, entry]
    if exist:
        arguments.append('-exist')
    return ipset_send_command(*arguments)
...
def ipset_test_entry(set_name, entry):
    try:
        result = ipset_send_command("test", set_name, entry)
        if ("is in set " + set_name) in result:
            return True
    except IpsetEntryNotFound:
        return False

    raise IpsetInvalidResponse()


def ipset_destroy_set(set_name):
    return ipset_send_command("destroy", set_name)
...
def ipset_restore_from_command_list(command_list):
    return ipset_send_command("restore", command_list=command_list)


def ipset_flush_set(set_name):
    return ipset_send_command("flush", set_name)


def ipset_rename(set_name_from, set_name_to):
    return ipset_send_command("rename", set_name_from, set_name_to)


def ipset_swap(set_name_from, set_name_to):
    return ipset_send_command("swap", set_name_from, set_name_to)


def ipset_version():
    return ipset_send_command("version")