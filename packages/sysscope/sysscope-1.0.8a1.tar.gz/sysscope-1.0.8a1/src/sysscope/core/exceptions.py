...
class sysscopeError(Exception):
	"""Base exception class
	
	This should be the base class for all exceptions, so that they are
	handled properly.
	
	Create the exception classes in your module like the following:
	
	    from sysscope.exceptions import sysscopeError
	    class MyModuleError(sysscopeError): pass
...
class RrdToolError(Exception):
	"""Exception raised when rrdtool fails."""
	pass
