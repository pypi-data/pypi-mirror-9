...
class JshDate(object):
    ...
    def get_month_day_chart_ticks(self, month, days=0):
        prefix = '{}-'.format(MONTH_NAMES[int(month) - 1])
    ...
    def get_quarter_choices(self, year=None):
        _, ticks = self.get_year_ticks(year or datetime.now().year)
        return [('Q{}'.format(q[-1]), 'Q{}'.format(q[-1])) for q in ticks]

    def get_month_choices(self, year=None):
        ticks, _ = self.get_year_ticks(year or datetime.now().year)
        months = MONTH_NAMES[:len(ticks) - 1]
