...
class HTMLParser(object):
    def get_data_path(self, elt, attr='data-path'):
        """
        Returns path defined for container and removes path if cond defined.
        """
        path = elt.get(attr, ".")
        if path == ".":
            return ""

        # removes path if cond defined
        if path:
            m = COND_REGEXP.match(path)
            if m:
                slot = m.group('slot') if m.group('oper') else m.group('value')
                if attr == 'data-path' and len(m.group('oper')) > 0:
                    if self.strict:
                        raise Exception("not condition %s with path in %s" % (path, elt))
                    elt["data-cond"] = path
                    del elt["data-path"]
                path = slot
    ...
    def get_all_pathes(self, elt, container='', exclude={}):
        """ Returns all pathes used from a simple element """
        ...
        for k, v in elt.attrs.iteritems():
            ...
            if v:
                ...
                # search from data-value
                    ...
                    # in case of select a path should be added
                    if not elt.name in iterator_tags and elt.get("data-path"):
                        path = self.get_data_path(elt, "data-path")
                        v = compose_path(path, v)
                    pathes.add(v)
    ...
    def get_all_pathes_recursiv(self, *args, **kwargs):
        """ Returns all pathes used from an element and its children"""
        ...
        for elt in (elt for elt in itertools.chain(*args) if isinstance(elt, Tag)):
            ...
            if elt.name in iterator_tags:
                ...
                props = self.get_all_pathes(elt, container=container, exclude={'data-path'})
                ...
                iterator = PathIterator(compose_path(container, data_path), iterator_props)
    ...
    def get_prefixed_path(self, stcl, path):
        """ Return the substencil prefixed by path """
        sub_stcl = {}
        for p,v in stcl.iteritems():
            if p.startswith(path):
                sub_stcl[p[len(path)+1:]] = v
        return sub_stcl


class StudiogdoWrapper(object):
    """
    A stencil is a dictionnary with slot as keys.
    """
    def is_empty(self, stcl, slot):
        """ Tests if a slot is empty """
    ...
    def get_value(self, stcl, path, _format="s", alert=True):
        """ Returns property value and property path """
...
class HTMLRenderer(SoupWrapper, HTMLParser, StudiogdoWrapper):
    ...
    def extract_pathes(self, skel):
        """ Used for test or verification (but not internally) """
    ...
    def render(self, path, skel, mode=None, stcl=None):
        ...
        body = soup.body
        if not stcl:
            init = {"data-path": path, ".": path}
            stcl = self.create_stencil(init, path, body, accept_no_stencil=True)
    ...
    def expand(self, stcl, elt):
        ...
        elif elt.name in ["section"]:
            decomposed = self.expand_section(stcl, elt)
            if decomposed:
                return
    ...
    def expand_container(self, stcl, container):
        ...
        if path:
            ...
            if not empty:
                new_stencil = self.get_prefixed_path(stcl, path)
                self.expand_children(new_stencil, container)
                self._set_final_value(new_stencil, container)
            else:
                container.decompose()
            return True
        else:
    ...
    def expand_section(self, stcl, section):
        """ Returns True if the container is decomposed """
        path = self.get_data_path(section)
        props = self.get_all_pathes_recursiv(section)
        stencils = self.get_stencils(stcl, path, props)
        try:
            if stencils:
                for s in stencils:
                    nsection = self.clone_tag(section, parent=section.parent)
                    self.expand_attributes(s, nsection)
                section.decompose()
            else:
                section.decompose()
            return True
            ...
            msg = "Error: %s[%s], %s" % (e, traceback.tb_lineno, stencils)
            raise Exception(msg)
        return False
    ...
    def expand_select(self, stcl, select):
        ...
        # set post parameter
        data_path = compose_path(stcl.get("data-path", ""), path)
        self.set_data_path(select, data_path)
        name = select.get('name', "%s_" % _format if _format else "s_")
        select["name"] = name + encode(data_path)
    ...
    def expand_one_dimension_table(self, stcl, table):
        ...
        stencils = self.get_stencils(stcl, pr, props)
        try:
            for s in stencils:
                s[".."] = stcl
    ...
    def expand_input(self, stcl, input):
        ...
        if tag == "textarea":
            ...
            if _type == "checkbox":
                if value and value.lower() in ['true', '1', 'vrai', 'ok', 'o', 'oui']:
                    input["checked"] = "checked"
            elif _type == "radio":
                if value and value.lower() == input.get("value"):
                    input["checked"] = "checked"
            else:
                input["value"] = value
    ...
    def remove_path(self, elt):
        """ Removes all path attributes for security """
    ...
    def create_stencil(self, stcl, path, container, accept_no_stencil=False):
        """ Completes all stencil properties with container needs """
        ...
        if self.debug:
            self.api.log.debug("get_stencil : props are %s (for path %s)" % (props, path))
        if props:
            single_path = '"%s"' % (path,)
            values = self.get_list(stcl, single_path, props)
            try:
                content = values.content
                if content:
                    stencils = json.loads(content)
                    if self.debug:
            ...
            if not accept_no_stencil:
                msg = 'get_stencil is empty'
                raise Exception(msg)

        return stcl

    def get_stencils(self, stcl, path, props):
        """ Completes all stencil properties with container needs """

        if self.debug:
            self.api.log.debug("get_stencils : from %s get %s in %s" % (stcl, props, path))

        # get new stencil properties
        if self.debug:
            self.api.log.debug("get_stencil : props are %s (for path %s)" % (props, path))
        list_path = '["%s"]'% (path,)
        values = self.get_list(stcl, list_path, props)
        try:
            content = values.content
            if content:
                stencils = json.loads(content)
