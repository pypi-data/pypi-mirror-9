...
class StudiogdoApi(object):
    ...
    def send(self, method, url, data=None, content_type='application/x-www-form-urlencoded;charset=UTF-8', files=None):
        ...
        if not response.ok:
            try:
                error = u"studiogdo error (code: %s, url: %s) : %s" % (response.status_code, request_kwargs['url'], response.reason)
            except UnicodeDecodeError:
                error = "studiogdo error (code: %s, url: %s) : %s" % (response.status_code, request_kwargs['url'], response.reason)
            params = pformat(self.params, indent=2)
            raise Exception(u"\n".join([error,pformat(params, indent=2)]))
