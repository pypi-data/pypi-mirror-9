...
"""
QtStuff.console:  a simple embeddable python shell
=====================================================
...
class QPythonConsole(QtGui.QWidget):
    ...
    passing a dict as the "locals" argument. If no locals are passed, 
    data.plots, data.annotations and data.legend are still available. 

    For plotting applications, it is reccomeded that the ChartWidget object is passed as a local,
    to allow plotting functionality from within the console
    """

    def __init__(self,parent=None,locals=None):

        #if locals is None:
        #    locals = {'plots': data.plots, 'annotations': data.annotations,
        #              'legend': data.legend}
        #else:
        #    locals['plots'] = data.plots
        #    locals['annotations'] = data.annotations
        #    locals['legend'] = data.legend
        #    locals['legend_facecolour'] = data.legend_facecolour
        #    locals['legend_edgecolour'] = data.legend_edgecolour
        #    locals['legend_alpha'] = data.legend_alpha

        self.locals = locals
        super(QPythonConsole,self).__init__(parent)
        self.ui = _QPythonConsoleUI(self)
        self.interpreter = _QPythonConsoleInterpreter(self.ui,self.locals)
