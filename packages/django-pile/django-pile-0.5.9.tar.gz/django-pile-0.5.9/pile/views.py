...
from .base import *
from .search_base import get_haystack_query
...
class CategoryListView(View):
    ...
    def get_queryset(self, **kwargs):
        ...
        if self.query:
            queryset = get_haystack_query(self.model, self.query)
        else:
        ...
        try:
            return qs.order_by(self.get_value('order'))
        except:
            return qs
    ...
    def get_orders(self):
        ...
        for (o, label) in self.orders:
            yield { 'id': o, 'name': label, 'down': '-' == order[0],
...
def breadcrumbs(*args):
    ...
    for model in args:
        if type(model) is str:
            ...
            yield (reverse(model[0]), _(model[1]))
        elif model is None:
            pass
