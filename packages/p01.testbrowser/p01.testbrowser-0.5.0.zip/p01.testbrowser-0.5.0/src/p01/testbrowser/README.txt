...
applications. It can be imported from ``p01.testbrowser.wsgi``:

    >>> from p01.testbrowser.wsgi import Browser
...
You can also use it with zope layers if you
  ...
  * write a subclass of ``p01.testbrowser.wsgi.Layer`` and override the
...
``p01.testbrowser.wsgi.AuthorizationMiddleware`` as all these application
...
Example when using the layer:
    ...
    >>> import p01.testbrowser.wsgi
    >>> class ZopeSimpleLayer(p01.testbrowser.wsgi.Layer):
    ...     def make_wsgi_app(self):
    ...         return p01.testbrowser.wsgi.AuthorizationMiddleware(simple_app)
