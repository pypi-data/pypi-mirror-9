...
(function() {
  ...
  angular.module('ajenti.filesystem').directive('fileDialog', function($timeout, filesystem, notify, hotkeys) {
    return {
      scope: {
        ngShow: "=?",
        onSelect: "&",
      ...
      link: function($scope, element, attrs) {
        ...
        $scope.select = function(item) {
          if (item.isDir) {
            ...
            if ($scope.mode === 'save') return $scope.name = item.name;
          }
        };
        $scope.save = function() {
        ...
        $scope.$watch('ngShow', function() {
          if ($scope.ngShow) return $scope.restoreFocus();
        });
...
(function() {
  ...
  angular.module('ajenti.filesystem').service('filesystem', function($http, $q) {
    this.read = function(path) {
      var q;
      q = $q.defer();
      $http.get("/api/filesystem/read/" + path).success(function(data) {
    ...
    this.write = function(path, content) {
      var q;
      q = $q.defer();
      $http.post("/api/filesystem/write/" + path, content).success(function(data) {
        return q.resolve(data);
      }).error(function(err) {
        return q.reject(err);
      });
    ...
    this.chmod = function(path, mode) {
      var q;
      q = $q.defer();
      $http.post("/api/filesystem/chmod/" + path, {
        mode: mode
