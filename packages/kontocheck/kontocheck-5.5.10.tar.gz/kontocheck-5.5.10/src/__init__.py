...
__all__ = ['get_version', 'lut_load', 'lut_is_valid', 'cleanup',
            ...
            'get_postalcode', 'get_city', 'scl_get_routing',
...
# Connect to SCL database
sclpath = os.path.join(_PATH, 'data', 'scl.db')
if not os.path.isfile(sclpath):
    raise RuntimeError('SCL database not found')
dbscl = sqlite3.connect(sclpath, check_same_thread=False)
dbscl.row_factory = sqlite3.Row
atexit.register(dbscl.close)
...
def scl_get_routing(bic):
    ...
    bic = bic.ljust(11, 'X')
    cur = dbscl.cursor()
    query = "SELECT sct, sdd, cor1, b2b FROM scl WHERE bic = ?;"
    cur.execute(query, (bic,))
    result = cur.fetchone()
    if not result:
        cur.execute(query, (bic[:8],))
        result = cur.fetchone()
    cur.close()
    if not result:
        raise ValueError('unknown bic "%s"' % bic)
    return dict(zip(result.keys(), map(bool, result)))
...
def scl_get_bankname(bic):
    ...
    bic = bic.ljust(11, 'X')
    cur = dbscl.cursor()
    query = "SELECT name FROM scl WHERE bic = ?;"
    cur.execute(query, (bic,))
    result = cur.fetchone()
    if not result:
        cur.execute(query, (bic[:8],))
    ...
    if not result:
        raise ValueError('unknown bic "%s"' % bic)
    return result[nativestring('name')]
