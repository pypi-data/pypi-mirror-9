...
Usage:
     ...
     jiver overlay <file>
     jiver run-tabs [--no-search-service] [--target-log]
     jiver upgrade-analyzer
     jiver upgrade-project <new-version>
...
Documentation:
     ...
     \033[1mjiver run-tabs\033[0m
            ...
            --no-search-service
                Prevents the search service from running.

            --target-log
                Uses the log from {project root}/target/jiveHome/logs/sbs.log
     ...
     \033[1mjiver upgrade-project\033[0m
        Upgrades a project to use the latest maven pom and upgrades appropriate files.
...
def start():
    ...
    if arguments.get('core-checkout', None):
        ...
        os.setpgrp() # used with os.killpg to kill the process
        try:
    ...
    elif arguments.get('upgrade-project', None):
        new_version  = arguments['<new-version>']
        upgrade_project.upgrade(new_version)
        ...
        no_search_service = arguments['--no-search-service']
        target_log = arguments['--target-log']
        tabs.run(no_search_service, target_log)
