...
_raw_sha3_256_lib = load_pycryptodome_raw_lib("Crypto.Hash._SHA3_256",
                        """
                        int SHA3_256_init(void **shaState);
                        int SHA3_256_destroy(void *shaState);
                        int SHA3_256_update(void *hs,
                                          const uint8_t *buf,
                                          size_t len);
                        int SHA3_256_digest(const void *shaState,
                                          uint8_t digest[16]);
                        int SHA3_256_copy(const void *src, void *dst);
                        """)
...
class SHA3_256_Hash(object):
    ...
    def __init__(self, data=None):
        state = VoidPointer()
        result = _raw_sha3_256_lib.SHA3_256_init(state.address_of())
        if result:
            raise ValueError("Error %d while instantiating SHA-3/256"
                             % result)
        self._state = SmartPointer(state.get(),
                                   _raw_sha3_256_lib.SHA3_256_destroy)
        if data:
            self.update(data)
    ...
    def digest(self):
        ...
        result = _raw_sha3_256_lib.SHA3_256_digest(self._state.get(),
                                                   bfr)
        if result:
            raise ValueError("Error %d while instantiating SHA-3/256"
                             % result)
    ...
    def copy(self):
        ...
        clone = SHA3_256_Hash()
        result = _raw_sha3_256_lib.SHA3_256_copy(self._state.get(),
                                                 clone._state.get())
        if result:
...
def new(data=None):
    ...
    :Parameters:
       data : byte string
        ...
        It is equivalent to an early call to `SHA3_256_Hash.update()`.
        Optional.

    :Return: A `SHA3_256_Hash` object
    """
    return SHA3_256_Hash().new(data)
