...
_raw_sha3_512_lib = load_pycryptodome_raw_lib("Crypto.Hash._SHA3_512",
                        """
                        int SHA3_512_init(void **shaState);
                        int SHA3_512_destroy(void *shaState);
                        int SHA3_512_update(void *hs,
                                          const uint8_t *buf,
                                          size_t len);
                        int SHA3_512_digest(const void *shaState,
                                          uint8_t digest[16]);
                        int SHA3_512_copy(const void *src, void *dst);
                        """)
...
class SHA3_512_Hash(object):
    ...
    def __init__(self, data=None):
        state = VoidPointer()
        result = _raw_sha3_512_lib.SHA3_512_init(state.address_of())
        if result:
            raise ValueError("Error %d while instantiating SHA-3/512"
                             % result)
        self._state = SmartPointer(state.get(),
                                   _raw_sha3_512_lib.SHA3_512_destroy)
        if data:
            self.update(data)
    ...
    def digest(self):
        ...
        result = _raw_sha3_512_lib.SHA3_512_digest(self._state.get(),
                                                   bfr)
        if result:
            raise ValueError("Error %d while instantiating SHA-3/512"
                             % result)
    ...
    def copy(self):
        ...
        clone = SHA3_512_Hash()
        result = _raw_sha3_512_lib.SHA3_512_copy(self._state.get(),
                                                 clone._state.get())
        if result:
...
def new(data=None):
    ...
    :Parameters:
       data : byte string
        ...
        It is equivalent to an early call to `SHA3_512_Hash.update()`.
        Optional.

    :Return: A `SHA3_512_Hash` object
    """
    return SHA3_512_Hash().new(data)
