...
def get_asg_hosts(args, dna_path, logger=None):
    ...
    response = _get_api_response(args, logger=logger)
...
def _get_api_response(args, logger=None):
    if logger is None:
    ...
    request = urllib2.Request('{0}/nodes/group?status=running'.format(
        args['api_url']
    ))

    base64string = base64.encodestring('{0}:{1}'.format(
        args['haldane_username'], args['haldane_password']
    )).replace('\n', '')
