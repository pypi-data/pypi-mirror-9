...
class _RunStatistic(object):
    """
    this class checks for each run if the likelihood got better and holds the 
    best parameter set.
    ...
    def __init__(self):
        self.rep = None
        self.params = None
        self.likelihood = -1e308
    def __call__(self,rep,likelihood,params):
        if type(likelihood)==type([]):
            if likelihood[0]>self.likelihood:
                self.likelihood = likelihood[0]#Show only the first best likelihood when working with more than one Likelihood
                self.params = params
                self.rep = rep
        else:
        ...
        return False
    def __repr__(self):
        return 'Best likelihood: %g' % self.likelihood
...
class _algorithm(object):
    ...
    dbname: str
        Name of the database where parameter, likelihood value and simulation 
    ...
    def __init__(self, spot_setup, dbname='test', dbformat='ram', parallel='seq',save_sim=True):
        ...
        self.likelihood   = self.setup.likelihood
