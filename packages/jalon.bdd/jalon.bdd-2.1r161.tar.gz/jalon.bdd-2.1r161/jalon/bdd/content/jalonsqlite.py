...
def getInscriptionPedago(session, SESAME_ETU, COD_ELP, COD_VRS_VET=None):
    ...
    if COD_VRS_VET:
        listeIDUE = [idue[0] for idue in session.query(ICE.COD_ELP).filter(and_(ICE.SESAME_ETU == SESAME_ETU, ICE.COD_ELP_PERE == '%s-%s' % (COD_ELP, COD_VRS_VET))).all()]
    else:
        listeIDUE = []
        listeInscription = [idue[0] for idue in session.query(ICE.COD_ELP).filter(ICE.SESAME_ETU == SESAME_ETU).all()]
        if not listeInscription:
            return None
        listeEnfant = [idue[0] for idue in session.query(ERE.COD_ELP_FILS).filter(ERE.COD_ELP_PERE == COD_ELP).all()]
        ...
        if taille > 500:
            ...
            while deb < taille:
                ...
                if fin > taille:
                    fin = taille
                listeEnfant2.extend([idue[0] for idue in session.query(ERE.COD_ELP_FILS).filter(ERE.COD_ELP_PERE.in_(listeEnfant[deb:fin])).all()])
                deb = deb + 500
        else:
            listeEnfant2 = [idue[0] for idue in session.query(ERE.COD_ELP_FILS).filter(ERE.COD_ELP_PERE.in_(listeEnfant)).all()]
        if listeEnfant2:
...
def getUeEtape(session, COD_ETP, COD_VRS_VET=None, TYP_ELP_SELECT=None):
    ...
    if COD_VRS_VET:
        ...
        COD_ELP = COD_ETP
    listeIDUE = [idue[0] for idue in session.query(ERE.COD_ELP_FILS).filter(and_(ERE.COD_ELP_PERE == COD_ELP, ERE.TYP_ELP == TYP_ELP_SELECT)).all()]
    if not listeIDUE:
        return []
    recherche = session.query(ELP.COD_ELP, ELP.LIB_ELP, ELP.TYP_ELP).filter(and_(ELP.COD_ELP.in_(listeIDUE)))
    return convertirResultatBDD(recherche)
...
def getElpAttach(session, COD_ELP, TYP_ELP_SELECT, TYP_ELP):
    ...
    if (TYP_ELP in ['ue', 'uel'] and TYP_ELP_SELECT == 'etape') or TYP_ELP == 'groupe':
        ERE_TYP_ELP = TYP_ELP
...
def getIndAndNameByElp(session, COD_ELP):
    ...
    IND = aliased(tables.IndividuSQLITE)
    listeInds = session.query(IND.SESAME_ETU, IND.LIB_NOM_PAT_IND, IND.LIB_PR1_IND).filter(IND.SESAME_ETU.in_(listeSesames)).order_by(IND.LIB_NOM_PAT_IND).all()
