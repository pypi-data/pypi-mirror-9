...
class Race(object):
    ...
    def init_ballots(self):
        ...
        '''
        sums_by_hopeful = self.get_sums_by_hopeful()
        for ballot in self.ballots:
            ballot.reset_candidates()
            self.transfer_to_next_candidate(ballot, sums_by_hopeful)

    def transfer_to_next_candidate(self, ballot, sums_by_hopeful):
        ...
        hopefuls_below_quota = [hopeful for hopeful, sum_ballots in sums_by_hopeful.iteritems()
                if sum_ballots < self.quota]
        # Find next candidate that has not been excluded or elected.
        while candidate and candidate not in hopefuls_below_quota:
            candidate = ballot.pop_next()
        if candidate:
    ...
    def transfer(self, ballots, sums_by_hopeful):
        ...
        for ballot in ballots:
            to_candidate = self.transfer_to_next_candidate(ballot, sums_by_hopeful)
            if to_candidate:
    ...
    def elect(self, candidate, ballots, sums_by_hopeful, transfer_surplus=True):
        ...
        if transfer_surplus and sum_ballots >= self.quota:
            ...
            transfers = self.transfer(ballots, sums_by_hopeful)
    ...
    def exclude(self, candidate, ballots, sums_by_hopeful):
    ...
    def get_sorted_hopefuls(self, sums_by_hopeful):
        ...
        '''
        sorted_hopefuls = [(sums_by_hopeful[candidate], candidate, ballots)
                for candidate, ballots in self.hopefuls.iteritems()]
        sorted_hopefuls.sort(reverse=True)
        return sorted_hopefuls

    def get_sums_by_hopeful(self):
        '''Gets the sum of tallies for each candidate
        Maps:
        hopefuls.iteritems() = [(candidate, balots), ...]
        to:
        sums_by_hopeful = {candidate: sum}
        '''
        return {candidate: sum(ballots) for candidate, ballots in self.hopefuls.iteritems()}
    ...
    def get_status(self):
        ...
        return {'quota': self.quota,
                ...
                'hopefuls': [(candidate, ballot_sum) for (ballot_sum, candidate, ballots) in self.get_sorted_hopefuls(self.get_sums_by_hopeful())],
    ...
    def run_round(self):
        ...
        sums_by_hopeful = self.get_sums_by_hopeful()
        sorted_hopefuls = self.get_sorted_hopefuls(sums_by_hopeful)
        actions = []

        # Determines which candidate has met the quota.
        ...
        top_candidate = next(self.get_top_candidates(sorted_hopefuls), None)
        if self.still_have_open_seats() and top_candidate:
            sum_ballots, candidate, ballots, break_tie = top_candidate
            if sum_ballots >= self.quota:
        ...
        if candidates_to_be_elected:
            for candidate, ballots in candidates_to_be_elected:
                # Elect candidate that met the quota
                actions.append({'elect': candidate, 'transfer': self.elect(candidate, ballots, sums_by_hopeful)})
        else:
            ...
            if break_tie:
                actions.append(break_tie)
            self.hopefuls.pop(candidate)
            actions.append({'exclude': candidate, 'transfer': self.exclude(candidate, ballots, sums_by_hopeful)})
    ...
    def run_remaining(self):
        ...
        for (sum_ballots, candidate, ballots, break_tie) in self.get_top_candidates(sorted_hopefuls):
            ...
            if sum_ballots >= self.minimum and len(self.elected) < self.num_seats:
                actions.append({'elect': candidate, 'transfer': self.elect(candidate, ballots, sums_by_hopeful, transfer_surplus=False)})
            else:
    ...
    def still_have_open_seats(self):
        return len(self.elected) < self.num_seats
