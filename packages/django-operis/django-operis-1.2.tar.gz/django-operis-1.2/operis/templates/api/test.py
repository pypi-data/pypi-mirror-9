...
class {{ model.singular }}Test(TestCase):
    ...
    def test_details(self):
        ...
        test_data = self.client.generate_test_data({{model.fields}})
        ...
        list_data = json.loads(list_response.content)
        # Check that the rendered context contains 5 customers.
        self.assertEqual(len(list_data['{{ model.plural_converted}}']), 1)
        ...
        for item in list_data['{{ model.plural_converted}}']:
            ...
            test_data_one = self.client.generate_test_data({{model.fields}},1)
