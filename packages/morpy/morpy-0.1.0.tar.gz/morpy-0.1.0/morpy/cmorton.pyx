...
class MortonLib(object):
    ...
    def get_cell(self, anyuint key):
        cdef np.ndarray[np.uint32_t, ndim=1] iarr =\
            np.zeros(self.ndim, dtype='uint32')
        cdef int tkey = key
        cdef int level = 0
    ...
    def get_cells(self, np.ndarray[anyuint, ndim=1] key_arr):
        cdef np.ndarray[np.uint32_t, ndim=2] iarr =\
            np.zeros([key_arr.size, self.ndim], dtype='uint32')
        cdef int level = 0
        cdef int ndim = self.ndim
        cdef int d
        cdef int k
    ...
    def get_key(self, np.ndarray[anyuint, ndim=1] inarr):
        cdef np.ndarray[np.uint32_t, ndim=1] iarr = np.zeros(inarr.shape[0], dtype='uint32')
        cdef int d
        cdef int m = 0
        cdef int level = 0
        cdef int ndim = self.ndim
    ...
    def get_keys(self, np.ndarray[anyuint, ndim=2] inarr):
        ...
        cdef np.ndarray[np.uint32_t, ndim=1] iarr = np.zeros(inarr.shape[1], dtype='uint32')
        cdef int m
        cdef int level
        cdef int c, d, i
