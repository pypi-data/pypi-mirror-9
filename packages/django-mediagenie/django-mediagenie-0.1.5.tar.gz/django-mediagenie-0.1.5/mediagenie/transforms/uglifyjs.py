...
def _uglify_js(files, outfile):
    ...
    for named_file, filepath in zip(files, file_paths):
        new_path = os.path.join(get_env().generated_path, 'smap', named_file.name)
        full_dirname = os.path.dirname(new_path)
        if not os.path.exists(full_dirname):
            try:
                os.makedirs(full_dirname)
            except OSError:
                # not threadsafe, so different thread might have made already.
                pass
    ...
    sourcemap_path = os.path.join(get_env().generated_path, 'smap', outfile + '.map')
    args = (
        ['uglifyjs'] + copied_files +
        ['--source-map', sourcemap_path,
         '--source-map-url', 'smap/' + outfile + '.map',
         '-p', 'relative',
         '-o', outfile_path,
