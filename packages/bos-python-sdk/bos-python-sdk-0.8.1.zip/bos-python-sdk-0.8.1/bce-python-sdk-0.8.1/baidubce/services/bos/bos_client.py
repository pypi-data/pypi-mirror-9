...
class BosClient(BceBaseClient):
    ...
    @staticmethod
    def _save_body_to_file(http_response, response, filename, buf_size):
        f = open(filename, 'w')
        try:
    ...
    @required(bucket_name=(str, unicode), key=str, filename=str)
    def get_object_to_file(self, bucket_name, key, filename, range=None, config=None):
        """
    ...
    @required(bucket=str, key=str, filename=str)
    def put_object_from_file(self, bucket, key, filename,
        ...
        try:
            ...
            if content_type is None:
                content_type = utils.guess_content_type_by_filename(filename)
            return self.put_object(bucket, key, fp,
                                   content_length=content_length,
                                   content_md5=content_md5,
                                   content_type=content_type,
    ...
    def copy_object(self,
        ...
        if e_tag is not None:
            headers[http_headers.BCE_COPY_SOURCE_IF_MATCH] = e_tag
    ...
    @required(bucket_name=(str, unicode), key=str)
    def initiate_multipart_upload(self, bucket_name, key, config=None):
        """
    ...
    def upload_part_from_file(self, bucket_name, key, upload_id,
                              part_number, part_size, filename, offset, part_md5=None,
                              config=None):
        """
    ...
    def _prepare_object_headers(
        ...
        if e_tag is not None:
            headers[http_headers.ETAG] = '"%s"' % utils.convert_to_standard_string(e_tag)
    ...
    def _get_path(config, bucket_name=None, key=None):
        return utils.append_uri(baidubce.URL_PREFIX, bucket_name, key)

    def _merge_config(self, config):
        if config is None:
            return self.config
        else:
            return copy.copy(self.config).merge_non_none_values(config)
