...
def _parse_host_port(endpoint, default_port):
    host_port_list = endpoint.split(':')
...
def send_request(
    ...
    host, port = _parse_host_port(config.endpoint, config.protocol.default_port)
    ...
    while True:
        ...
        try:
            ...
            conn = _get_connection(config.protocol, host, port, config.connection_timeout_in_mills)
