...
PACKAGE_DIR = os.path.abspath(os.path.join(DIR, os.pardir))
...
def mkwsa(dir, name, js, css, config_version, template, manifest, force):
    ...
    with open(os.path.join(PACKAGE_DIR, "defaults/modules.py"), 'rb') as fp:
        content = fp.read()
    ...
    if js:
        ...
        with open(os.path.join(PACKAGE_DIR, "defaults/template.js"), 'rb') as fp:
            content = fp.read()
            data = six.u(content)
