...
class PrometheusBeforeMiddleware(BasePrometheusMiddleware):
    def __init__(self):
        self.requests_total = self.addCounter('requests_total', 'Total count of requests')
        self.responses_total = self.addCounter('responses_total', 'Total count of responses')
...
class PrometheusAfterMiddleware(BasePrometheusMiddleware):
    def __init__(self):
        self.ajax_requests = self.addCounter('ajax_requests', 'Count of AJAX requests')
        self.requests_by_method = self.addCounter('requests_by_method', 'Count of requests by method', ['method'])
        self.requests_by_transport = self.addCounter('requests_by_transport', 'Count of requests by transport', ['transport'])
        self.requests_by_view = self.addCounter('requests_by_view', 'Count of requests by view', ['view_name'])
        self.requests_by_view_transport_method = self.addCounter('requests_by_view_transport_method', 'Count of requests by view, transport, method', ['view', 'transport', 'method'])
        self.requests_body_bytes = self.addHistogram('requests_body_bytes', 'Histogram of requests by body size', buckets=PowersOf(2, 30))
        self.responses_by_status = self.addCounter('responses_by_status', 'Count of responses by status', ['status'])
        self.responses_body_bytes = self.addHistogram('responses_body_bytes', 'Histogram of responses by body size', buckets=PowersOf(2, 30))
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        ...
        self.requests_by_view_transport_method.labels(name, transport, request.method).inc()
