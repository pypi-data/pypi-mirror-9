    ...
    def __init__(self, app):
        ...
        env.register(
            ...
            Bundle(
                ...
                'css/bootstrap-social-btns.css',
                output='pilotstatic.css'
            )
        )

Pilot.__(pilot={}, pilot_social_login={})
class SocialLogin(object):
    def __init__(self, app):

        _pilot = app.config.get("PILOT")
        if _pilot:
            Pilot.__(pilot=_pilot)

            if _pilot["SOCIAL_LOGIN"] \
                    and _pilot["SOCIAL_LOGIN"]["ENABLE"]:
                client_ids = {}
                buttons = []
                for name, prop in _pilot["SOCIAL_LOGIN"].items():
                    if isinstance(prop, dict):
                        if prop["ENABLE"]:
                            _name = name.lower()
                            client_ids[_name] = prop["CLIENT_ID"]
                            buttons.append(_name)
                Pilot.__(pilot_social_login={
                    "client_ids": client_ids,
                    "buttons": buttons
                })


Pilot.bind(TemplatesLoader)
Pilot.bind(SocialLogin)
...
def login_view(model,
    ...
    class Login(S3UploadMixin):
        ...
        def social_auth_hello(self):
            """
            A return back from verifying with social sites
            """
            return "HELLO"
        ...
        @route("social-login", methods=["POST"])
        def social_login(self):
            ...
            if not email:
                ...
                user = User.social_login(provider=provider,
                                         provider_user_id=provider_user_id,
                                         email=email,
                                         name=name,
                                         image_url=image_url,
        ...
        def set_emailp(self):
            ...
            if email and self.SESSION_KEY_SET_EMAIL_DATA in session:
                ...
                if user:
                    ...
                    User.new(email=email, name=_data["name"])
