...
class Part_Detail_View(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        def map_fields(name, value, verbose_name):
            ...
            while True:
                ds_path = 'epic/datasheets/%s.pdf' % fname
            ...
            if ds_path is not None:
                ds_link = ('&ensp;<a href="%s/%s" target="datasheet">'
                           '<i class="glyphicon glyphicon-book"></i></a>') \
                    % (settings.STATIC_URL, ds_path)

            url = 'https://octopart.com/partsearch#!?q={mfg}%20{mfg_pn}' \
                .format(**args)
            return verbose_name, \
                ('%s%s&ensp;<a href="%s" target="part_lookup">'
                 '<img src="%s/epic/img/octopart.ico" alt=''></a>') \
                % (value, ds_link, url, settings.STATIC_URL)
    ...
    def get_context_data(self, **kwargs):
        def map_fields(name, value, verbose_name):
            if name == 'ts':
                ...
                return 'Order Date', \
                    defaultfilters.date(datetime.strptime(value, fmt) \
                                        .replace(tzinfo=timezone.utc), 'Y-m-d')
            return verbose_name, value
