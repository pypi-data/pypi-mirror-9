...
from egauge_django.users.models import Contacts as Contact
from egauge_django.users.models import MyUser as User
...
class URL_Iterator:
    ...
    def next(self):
        pat = self.urlpattern_iterator.next()
        args = {}
...
class Access_Tests(TestCase):
    ...
    def setUpClass(cls):
        c = Contact(id=1,
                    firstname='Testy',
                    lastname='McTest',
                    addr1='eGauge Systems LLC',
                    addr2='4730 Walnut St, Suite 110',
                    city='Boulder',
                    zipcode=80301,
                    state='CO',
                    country='US',
                    mobile='999.999.9999',
                    email='joshua@egauge.net')
        c.save()
        u = User(id=1, login='testuser', contactid=c.pk,
                 last_login=timezone.now(),
                 access=','.join([perms.VIEW, perms.EDIT]))
        u.set_password('magic')
        u.save()
    ...
    def test_authenticated_accesses(self):
        self.client.login(login='testuser', password='magic')
