...
def chessproblems_equal(cp1, cp2):
    if cp1 == cp2:
        ...
        for simple_member in _CHESSPROBLEM_SIMPLE_MEMBERS:
            if not _members_equal(cp1, cp2, simple_member):
                LOGGER.debug('chessproblems_equal() %s -> False' % simple_member)
                return False
        # String list members
