...
/*! \file pyreadsdpa.c 
  \brief Read SDPA data files for python interface, pass data into DSDP solver, and print solution.
*/	
...
static int rank=0;
int PyReadSDPAFile(char*,char *, double**, int*, double**, int*, double **stat, int *nstats);

#define CHKDATA(a,b,c)  { if (c){ printf("Possible problem in variable %d, block %d. \n",a+1,b+1);} }

/* module functions */
static PyObject *                                 /* returns object */
pyreadsdpa(PyObject *self, PyObject *args)           /* self unused in modules */
{ 
    PyObject* y_ptr = NULL;
    PyObject* x_ptr = NULL;
    PyObject* stats_ptr = NULL;
    double *y, *xsol, *STATS; 
    int nvars, i, nXvars, nstats, info=0;                   /* args from Python call */
    char *data_filename, *options_filename;
    if (! PyArg_Parse(args, "(ss)", &data_filename, &options_filename))  /* convert Python -> C */
        return NULL;                              /* null=raise exception */
    else {
        PyReadSDPAFile(data_filename, options_filename, &y, &nvars, &xsol, &nXvars, &STATS, &nstats);
    
        y_ptr = PyList_New(nvars);
        x_ptr = PyList_New(nXvars);
        stats_ptr = PyList_New(nstats);
        if (y_ptr == NULL || x_ptr == NULL || stats_ptr==NULL) {
           printf("Memory Over for Solution Space \n");
           Py_XDECREF(y_ptr);
	   Py_XDECREF(x_ptr);
           Py_XDECREF(stats_ptr);
           return PyErr_NoMemory();
        }
        
        for (i=0; i< nvars; i++) 
        {   
            PyList_SetItem(y_ptr, i, PyFloat_FromDouble(y[i]));
            //printf("%e\n", y[i]);
        }
        DSDPFREE(&y,&info);
        ...
        for (i=0; i< nXvars; i++) 
        {   
            PyList_SetItem(x_ptr, i, PyFloat_FromDouble(xsol[i]));
            // printf("%e\n", xsol[i]);
        }
        DSDPFREE(&xsol,&info);
        ...
        for (i=0; i< nstats; i++) 
        {   
            PyList_SetItem(stats_ptr, i, PyFloat_FromDouble(STATS[i]));
            // printf("%e\n", xsol[i]);
        }
        DSDPFREE(&STATS,&info);
        ...
        return Py_BuildValue("OOO", y_ptr, x_ptr, stats_ptr);
    }
}

/* registration table  */
static PyMethodDef pydsdp5_methods[] = {
    {"pyreadsdpa",  pyreadsdpa, METH_VARARGS, "func doc"},    /* name, &func, fmt, doc */
    {NULL, NULL, 0, NULL}                               /* end of table marker */
};

/* module definition structure */
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef pydsdp5module = {
   PyModuleDef_HEAD_INIT,
   "pydsdp5",         /* name of module */
   "mod doc",       /* module documentation, may be NULL */
   -1,              /* size of per-interpreter module state, -1=in global vars */
   pydsdp5_methods    /* link to methods table */
};
#endif
...
#if PY_MAJOR_VERSION >= 3
PyMODINIT_FUNC
PyInit_pydsdp5()                         /* called on first import */
{                                      /* name matters if loaded dynamically */
    
    return PyModule_Create(&pydsdp5module);
}
#else
PyMODINIT_FUNC
initpydsdp5()
{
    Py_InitModule3("pydsdp5", pydsdp5_methods, "This is a module");
}
#endif

#undef __FUNCT__
#define __FUNCT__ "PyReadSDPAFile"
/*!
\fn int PyReadSDPAFile(char* data_filename, char *options_filename, double **ysol, int *nvars, double **xsol, int *xlength, double **stat, int *nstats);
\brief Read SDPA formatted file and solve the semidefinite program.
\param argc number of command line arguments
\param argv command line arguments
\ingroup Examples
*/
int PyReadSDPAFile(char* data_filename, char *options_filename, double **ysol, int *nvars, double **xsol, int *xlength, double **stat, int *nstats){	
