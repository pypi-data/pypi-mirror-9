...
class Notification (Exception):	# Rollback: no, precooked caption and icon: no
	def __init__ (self, message, caption, icon = '', report = ''):
		...
		if report == '':
			self.report = message + StackTraceUnavailable 
		else:
			self.report = report
...
def defaultNotificationHandler (notification):
	...
	if app.logNotifications and not notification.report is None:
		app.notificationLogger (notification)
		
	if app.showNotifications and not notification.message is None:
...
# --- Circumventing lambda function limitations
...
def getFunction (valueOrFunction):
	if not valueOrFunction is None:
...
def getList (valueOrList):
	if not valueOrList is None:
...
def insertBranch (treeToInsertInto, insertionPath, branchToInsert, before = False, inPlace = False):
	if inPlace:
	...
	if before:
		if containsRoot (parentTree, branchToInsert):
			raise Objection ('Can not insert dupplicate into tree')
		parentTree.insert (index, branchToInsert)
	else:
		if containsRoot (tree, branchToInsert):
			raise Objection ('Can not insert dupplicate into tree')
		tree.append (branchToInsert)
	
	return localTreeToInsertInto

def reorderTree (treeToReorder, removalPath, insertionPath, before = False):
	branchToMove = cloneBranch (treeToReorder, removalPath)
	reorderedTree = removeBranch (treeToReorder, removalPath)
	insertBranch (reorderedTree, insertionPath, branchToMove, before, True)
