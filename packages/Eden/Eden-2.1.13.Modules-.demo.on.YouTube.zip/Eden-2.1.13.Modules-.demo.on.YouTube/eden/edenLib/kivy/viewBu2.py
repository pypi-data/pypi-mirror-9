...
application.scrollDistance = 15
application.scrolling = False
...
class ViewBase (object): 
	...
	def createWidget (self, rootView):
		...
		def touchMove (*args):	# Only called if pointer is down		
			...
			if self.pointerIsInside:
				if yDistance (application.pointerPosition, application.pointerWentDownPosition) > application.scrollDistance:
					print 'Scroll !!', self.__class__
					application.scrolling = True
...
class ListItemWidget (ListItemButton, ViewWidget):	
	def __init__ (self, **args):
		self.listView = args ['listView']
		self.fieldIndex = args ['fieldIndex']
		...
		def delayedTouchDown (*args):
			if not application.scrolling:	# Selection is allowed no matter what
		...
		def touchDown (*args):
			if self.collide_point (*self.to_parent (*self.to_widget (*application.pointerPosition))):
				Clock.schedule_once (delayedTouchDown, 0.2)
				return True	# Prevent selection by the Kivy mechanism
		...
		def touchUp (*args):	# Deselection is only allowed if not repeating or dragging
			if self.collide_point (*self.to_parent (*self.to_widget (*application.pointerPosition))):
