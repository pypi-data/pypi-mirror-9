...
from appdynamics.lib import values, make_uuid
...
class SnapshotService(object):
    ...
    """
    def __init__(self, agent, interval_ms=10):
    ...
    def start_snapshot(self, bt, trigger):
        ...
        with self.bt_lock:
            ...
            if bt.snapshot_guid is None:
                if trigger in (snapshot.CONTINUING, snapshot.REQUIRED):
    ...
    def sample(self, bt=None):
        ...
        if bt:
            bt.sample_data.take(thread_frames, now, bt._active_exit_call)
        else:
            for bt in values(self.bts):  # Grab strong refs to BTs and iterate
                bt.sample_data.take(thread_frames, now, bt._active_exit_call)
