...
class CorrelationHeader(object):
    ...
    COMMA_SEPARATED_KEYS = set([FROM_COMPONENT_ID_KEY, TO_COMPONENT_ID_KEY, EXIT_TYPE_ORDER_KEY])
    COMMA_SEPERATOR = ','
    ...
    def add_component_link(self, exit_type, tier_id):
        last_component = self[TO_COMPONENT_ID_KEY].split(self.COMMA_SEPERATOR)[-1]
    ...
    def pb_dict(self):
        ...
        exit_points = self[EXIT_TYPE_ORDER_KEY].split(self.COMMA_SEPERATOR)

        links = [
            {
                'fromComponentID': from_component_id,
                'toComponentID': to_component_id,
                'exitPointType': exit_point,
            }
            for from_component_id, to_component_id, exit_point in zip(from_components, to_components, exit_points)
        ]
...
def _get_unresolved_backend_id(component):
    try:
        if component.startswith(CorrelationHeader.UNRESOLVED_PREFIX) and component.endswith(CorrelationHeader.UNRESOLVED_SUFFIX):
...
def make_header(agent, bt, exit_call, do_not_resolve=False):
    ...
    try:
        ...
        if exit_call is None or exit_call.backend is None or exit_call.backend.registered_id is None:
        ...
        if bt:
            agent.logger.debug('BT:%s (name=%r) created notxdetect header: %s', bt.request_id, bt.name, exc.message)
        else:
    ...
    hdr[GUID_KEY] = bt.snapshot_guid or make_uuid()
