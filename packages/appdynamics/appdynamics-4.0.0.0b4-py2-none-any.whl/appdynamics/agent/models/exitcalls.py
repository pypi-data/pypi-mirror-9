...
from appdynamics.lib import iteritems, make_name_value_pairs
...
class Backend(object):
    ...
    optional_properties : dict
        ...
        backend. These are visible in the UI, but not used to identify the
        backend.

    """
    __slots__ = (
        'exit_point_type', 'identifying_properties', 'name',
        'registered_id', 'component_id', 'optional_properties',
        'is_foreign_app'
    )

    def __init__(self, exit_point_type, identifying_properties, name,
                 registered_id=None, component_id=None, is_foreign_app=False):
    ...
    def sorted_identifying_properties(self):
        properties = iteritems(self.identifying_properties)
        sorted_properties = sorted(properties, key=lambda p: p[0])
        return tuple(sorted_properties)

    @property
    def backend_type_name(self):
        full_name = pb.ExitPointType.Name(self.exit_point_type)
        return full_name.replace('EXIT_', '')

    def __str__(self):
        return 'Backend: exit type={0}, identifying properties={1}, name={2}'.format(
            self.exit_point_type, self.identifying_properties, self.name)
...
class ExitCall(object):
    ...
    __slots__ = (
        ...
        'caller', 'was_added', '__weakref__'
    )

    def __init__(self, sequence_info, timer, backend, category, params, caller, operation=None):
