...
def command(options, args):
    ...
    SUBCOMMANDS = {'start': start, 'stop': stop}
    subcommand = args[0]
...
def start(args=None):
    ...
    start_fn = start_watchdog if use_watchdog else start_proxy
...
def stop(args=None):
    ...
    stop_process('proxy', get_proxy_pid_file())
...
def start_proxy(run_proxy_script=None, script_args=None, debug=False, proxy_logger=None):
    ...
    command_args = [run_proxy_script, '-d', run_proxy_dir, '-r', run_proxy_dir, comm_dir, log_dir]
