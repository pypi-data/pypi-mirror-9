...
from appdynamics.scripts.pyagent import CommandInvocationError, parse_options
...
SetEnv APPD_WSGI_SCRIPT {their_wsgi_file}
SetEnv APPD_WSGI_CALLABLE_OBJECT {callable}
...
env=APPD_WSGI_SCRIPT={wsgi_file}{callable}
"""
...
def config_wsgi(args):
    ...
    if wsgi_type == 'file':
        ...
        with open(wsgi_file, 'r') as fp:
            ...
            print(fp.read())
            print('{callable} = appdynamics.agent.frameworks.wsgi.wsgi_middleware({callable})'.format(callable=wsgi_callable))
        ...
        print('gunicorn -e "APPD_WSGI_SCRIPT={wsgi_file}" {callable} appdynamics.agent.frameworks.wsgi:application'.format(
            wsgi_file=wsgi_file,
            callable='-e "APPD_WSGI_CALLABLE_OBJECT=%s"' % wsgi_callable if wsgi_callable else ''))
        ...
        callable_env = " APPD_WSGI_CALLABLE_OBJECT={callable}".format(callable=wsgi_callable) if wsgi_callable else ''
