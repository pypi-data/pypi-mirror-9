...
static PyObject* authGSSClientInit(PyObject* self, PyObject* args, PyObject* keywds)
{
    ...
    pystate = PyCapsule_New(state, "gss_client_state", NULL);

    if (PyCapsule_IsValid(pydelegatestate, "delegatestate"))
      delegatestate = PyCapsule_GetPointer(pydelegatestate, "delegatestate");
...
static PyObject *authGSSClientClean(PyObject *self, PyObject *args)
{
    ...
    if (!PyCapsule_IsValid(pystate, "gss_client_state")) {
        PyErr_SetString(PyExc_TypeError, "Expected a context object");
        return NULL;
    }

    state = (gss_client_state *)PyCapsule_GetPointer(pystate, "gss_client_state");
    if (state != NULL)
    {
...
static PyObject *authGSSServerHasDelegated(PyObject *self, PyObject *args)
{
    ...
    if (!PyCapsule_IsValid(pystate, "gss_server_state")) {
        PyErr_SetString(PyExc_TypeError, "Expected a context object");
        return NULL;
    }

    state = (gss_server_state *)PyCapsule_GetPointer(pystate, "gss_server_state");
    if (state == NULL)
        return NULL;
...
static PyObject *authGSSServerInit(PyObject *self, PyObject *args)
{
    ...
    pystate = PyCapsule_New(state, "gss_server_state", NULL);
...
PyInit_kerberos(void)
{
    ...
    PyModule_AddObject(m, "BasicAuthError", BasicAuthException_class);
    ...
    PyModule_AddObject(m, "PwdChangeError", PwdChangeException_class);
