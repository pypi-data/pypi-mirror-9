...
class Client:
    ...
    def __init__(self, url, username='ubuntu', key_path=None, replacements=None, timeout=30):
        ...
        logger.info('Connect to remote host "{username}@{url}" via SSH'.format(username=username, url=url))
    ...
    def run_command(self, command, timeout=30):
        ...
        try:
            ...
            interface.exit_error('Stopped on timeout in {} seconds'.format(timeout))
        else:
    ...
    def apt_install(self, packages):
        ...
        logger.info('Install {} packages via apt-get'.format(len(packages)))
