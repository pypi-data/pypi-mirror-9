...
class __Connections:
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            interface.exit_error('Configuration file not found')

        except boto.exception.BotoServerError as e:
            interface.exit_error('Bad AWS credentials' if e.error_code == 'InvalidClientTokenId' else e.message)
...
class helpers:
    ...
    @classmethod
    def get_or_create_security_group(cls, name, public_tcp_ports=None, public_udp_ports=None):
        """
        ...
        :type public_tcp_ports: iterable
        :param public_tcp_ports: list of tcp-ports to opening

        :type public_udp_ports: iterable
        :param public_udp_ports: list of tcp-ports to opening
        ...
        if public_tcp_ports:
            for port in public_tcp_ports:
                try:
                    ...
                    logger.info('Open {} tcp-port'.format(port))
                except boto.exception.EC2ResponseError as e:
                    if e.error_code != 'InvalidPermission.Duplicate':
                        interface.exit_error(e.message)
                    logger.debug('Port {} is already open for tcp'.format(port))

        # Open specified udp-ports
        if public_udp_ports:
            for port in public_udp_ports:
                try:
                    group.authorize('udp', port, port, '0.0.0.0/0')
                    logger.info('Open {} udp-port'.format(port))
                except boto.exception.EC2ResponseError as e:
                    if e.error_code != 'InvalidPermission.Duplicate':
                        interface.exit_error(e.message)
                    logger.debug('Port {} is already open for udp'.format(port))
    ...
    @classmethod
    def create_instance(cls, key_name, group_name, image_id=None, instance_type=None, name=None):
        """
        ...
        :type image_id: str
        :param image_id: image id (Ubuntu by default)
        ...
        if name and cls.get_instance(name):
            interface.exit_error('The instance "{}" is already exists'.format(name))

        if not image_id:
            image_id = cls.find_last_ubuntu_image_id('14.04')
        ...
        try:
            ...
            if e.error_code == 'InvalidAMIID.NotFound':
                interface.exit_error('Image with id "{}" is not found'.format(image_id))
            else:
                interface.exit_error(e.message)
        if name:
    ...
    def get_instance(cls, lookup, reload_from_aws=False):
        ...
        if isinstance(lookup, str):
            ...
            if len(response) > 1:
                interface.exit_error('More than one instance found by name "{}"'.format(lookup))
            elif len(response) == 1:
    ...
    def wait_host(cls, dns_name):
        ...
        while True:
            try:
                ...
                interface.exit_error('Interrupt')
