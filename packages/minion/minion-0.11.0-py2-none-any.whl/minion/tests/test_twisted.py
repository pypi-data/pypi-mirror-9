...
class TestRequestIntegration(RequestIntegrationTestMixin, SynchronousTestCase):
    ...
    def test_it_properly_routes_when_mounted_on_a_subpath(self):
        minion = Application()
        minion.route(b"/hello")(lambda request : Response(b"World"))
        twistedResource = Resource()
        twistedResource.putChild(b"minion", MinionResource(minion))
        site = Site(twistedResource)
        request = makeRequest(path=b"/minion/hello")
        render(resource=twistedResource, request=request)
