...
def create_trials_by_month(trials_df):
    by_month = df_with_all_months_index(trials_df)
    start_month_counts = trials_df[['Start Month']].groupby('Start Month').size()
    start_month_counts.name = 'Trials by Start Month'
    trials_by_month = by_month.join(start_month_counts)

    received_month_counts = trials_df[['Received Month']].groupby('Received Month').size()
    received_month_counts.name = 'Trials by Received Month'
    trials_by_month = trials_by_month.join(received_month_counts)

    completion_month_counts = trials_df[['Completion Month']].groupby('Completion Month').size()
    completion_month_counts.name = 'Trials by Completed Month'
    trials_by_month = trials_by_month.join(completion_month_counts)

    return trials_by_month.sort().fillna(0)
...
def plot_collaborator(collaborator, df, start_month='2000-01', end_month=prev_month_year_dash_month_string(), date_type=""):
