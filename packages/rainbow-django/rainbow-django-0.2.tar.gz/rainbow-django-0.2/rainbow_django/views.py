...
def rainbow_handler_internal(func):
    ...
    def wrapper_func(request, *args, **kwargs):
        ...
        if not is_valid_request(timestamp, nonce, signature):
            rsp = HttpResponse('')
            rsp[RAINBOW_INVALID_REQUEST_REASON] = '60001'
            return rsp
        ...
        try:
            ...
            if not identity:
                rsp = HttpResponse('')
                rsp[RAINBOW_INVALID_REQUEST_REASON] = '60002'
                return rsp
        ...
        try:
            ...
            if rsp.subscribes:
                http_rsp[RAINBOW_CLIENT_CHANNEL_SUB] = ';;'.join(
                    rsp.subscribes)
...
def on_message(request, msg_type):
    ...
    try:
        if request.method != 'POST':
            rsp = HttpResponse('')
            rsp[RAINBOW_INVALID_REQUEST_REASON] = '60003'
            return rsp
