...
# Origin: Steve Ivy <steveivy@gmail.com>
# Improve: bree Kay <root@nextoa.com>
# http://monkinetic.com
...
class StatsdClient(object):
    ...
    def incr(self, stats, sample_rate=1):
        """
        Increments one or more stats counters

        >>> client = StatsdClient()
        >>> client.incr('example.incr')
        >>> client.incr('example.incr', 0.5)
        """
        self.count(stats, 1, sample_rate)

    def decr(self, stats, sample_rate=1):
        """
        Decrements one or more stats counters

        >>> client = StatsdClient()
        >>> client.decr('example.decr')
        """
        self.count(stats, -1, sample_rate)

    def count(self, stats, value=1, sample_rate=1):
        """
