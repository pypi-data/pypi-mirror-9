...
class Material(object):
    ...
    def __init__(self, elements, quantities=None, kind='mirror', rho=0, t=None,
                 table='Chantler', efficiency=None, efficiencyFile=None):
        r"""
        ...
        *efficiency* can be given for *kind* = 'grating' and *kind* = 'FZP'. It
        must correspond to the field *order* of the OE. It can be given as a
        constant per diffraction order or as an energy dependence, also per
        diffraction order. It is a sequence of pairs [*order*, *value*], where
        *value* is either the efficiency itself of an index in the data file
        given by *efficiencyFile*. The data file can either be (1) a pickle
        file with *energy* and *efficiency* arrays as two first dump elements
        and *efficiency* shape as (len(*energy*), *orders*) or (2) a column
        file with energy in the leftmost column and the order efficiencies in
        the next columns. The *value* is a corresponding array index
        (zero-based) or a column number (also zero-based, the 0th column is
        energy). An example of the efficiency calculation can be found in
        ``\examples\withRaycing\38_Wave\waveGrating.py``.
        """
...
class Multilayer(object):
    ...
    functions of local *x* and *y*.
    """
    def __init__(self, tLayer, tThickness, bLayer, bThickness, nPairs,
                 substrate):
        """
        The top layer material (*tLayer*), the bottom layer material (*bLayer*)
        and the substrate material (*substrate*) are instances of
        :class:`Material`. The thicknesses of the layers (*tThickness* and
        *bThickness*) are given in Å. *nPairs* is the number of layer pairs."""
        self.tLayer = tLayer
    ...
    def get_dtheta_symmetric_Bragg(self, E, order=1):
        ...
        d_ = abs((nt-1) * self.tThickness + (nb-1) * self.bThickness) / self.d
        return self.get_Bragg_angle(E) - np.arcsin(
            ((order * ch / E)**2 + self.d**2 * 8*d_)**0.5 / (2*self.d))

    def get_t_thickness(self, x, y):
        return self.tThickness

    def get_b_thickness(self, x, y):
        return self.bThickness

    def get_amplitude(self, E, beamInDotNormal, x=None, y=None, cl_list=None):
        r"""
...
#        t0 = time.time()
        ...
        p2b = np.complex128(np.exp(1j * Qb * self.get_b_thickness(x, y)))
        p2t = np.complex128(np.exp(1j * Qt * self.get_t_thickness(x, y)))
...
#            t2 = time.time()
            ...
            cl_ctx, cl_queue, cl_program = cl_list
            #cl_ctx = cl_list(0)
            #cl_queue = cl_list(1)
            #cl_program = cl_list(2)
            rbs_s_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                  cl.mem_flags.COPY_HOST_PTR, hostbuf=rbs_s)
            rbs_p_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                  cl.mem_flags.COPY_HOST_PTR, hostbuf=rbs_p)
            rtb_s_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                  cl.mem_flags.COPY_HOST_PTR, hostbuf=rtb_s)
            rtb_p_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                  cl.mem_flags.COPY_HOST_PTR, hostbuf=rtb_p)
            rvt_s_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                  cl.mem_flags.COPY_HOST_PTR, hostbuf=rvt_s)
            rvt_p_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                  cl.mem_flags.COPY_HOST_PTR, hostbuf=rvt_p)
            p2t_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                cl.mem_flags.COPY_HOST_PTR, hostbuf=p2t)
            p2b_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                                cl.mem_flags.COPY_HOST_PTR, hostbuf=p2b)
            ri_s_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_WRITE |
                                 cl.mem_flags.COPY_HOST_PTR, hostbuf=ri_s)
            ri_p_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_WRITE |
                                 cl.mem_flags.COPY_HOST_PTR, hostbuf=ri_p)
            global_size = ri_p.shape
            local_size = None
            cl_program.get_amplitude_multilayer(cl_queue,
                                                global_size,
                                                local_size,
                                                np.int32(self.nPairs),
                                                rbs_s_buf, rbs_p_buf,
                                                rtb_s_buf, rtb_p_buf,
                                                rvt_s_buf, rvt_p_buf,
                                                p2t_buf, p2b_buf,
                                                ri_s_buf, ri_p_buf).wait()
            cl.enqueue_read_buffer(cl_queue,
                                   ri_s_buf,
                                   ri_s).wait()
            cl.enqueue_read_buffer(cl_queue,
                                   ri_p_buf,
                                   ri_p).wait()
#            t2 = time.time()
#        print "Cycle:", t2-t1, "s. Total t:", t2-t0, "s"
        return ri_s, ri_p
