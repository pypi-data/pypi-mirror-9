...
###############################################################################
"""
$Id: browser.py 3839 2013-09-08 05:10:10Z roger.ineichen $
"""
...
def getFileResult(context, request, blocksize=32768):
    ...
    # set Content-Length
    request.response.setHeader('Content-Length', str(context.csize))

    # set Content-Type
    request.response.setHeader('Content-Type', context.ctype)

    # set Last-Modified based on (upload) date
    modified = context.date
    ...
    chunkReader = context.getFileReader()
