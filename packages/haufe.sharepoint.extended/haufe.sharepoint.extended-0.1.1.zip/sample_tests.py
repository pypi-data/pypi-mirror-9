...
""" All tests are without NTLM. If you need NTLM support, please add the NTLM """
...
## for this item
print "Getting all the attachments for item: %s" % last.ID
result = service.getAttachments(dict(listItemID=last.ID))

## show all the attachments
for i in result:
	print "You have an attachment named: %s" % i[1]

## deletes all the items
## attachments

print "Deleting all the attachments"
for i in result:
	service.deleteAttachment(dict(listItemID=last.ID, url=i[1]))
