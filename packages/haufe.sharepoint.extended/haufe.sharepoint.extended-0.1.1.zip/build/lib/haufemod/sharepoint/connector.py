...
def Connector(url, username, password, list_id, NTLM=True, verbose=False, copy=False, timeout=65):
    global CLIENT
    global VERBOSE
...
class ListEndpoint(object):
    ...
    def addAttachment(self, data):
        ...
        if data['raw']:
           ...
           f = base64.b64encode(open(os.path.abspath(data['file']), 'r+').read())
        ...
        stdon()
        #return ParsedSoapResult(result) ## no need if we have an error it will warn
    ...
    def upload(self,data):
        ...
        for k,v in data['fields'].iteritems():
            ...
            fit.append(Attribute('InternalName', k))
            fit.append(Attribute('DisplayName', k))
            ##fit.append(Attribute('Id', 'Organization'))
