"""The binary module contains all classes related to communicating in
the Binary protocol.
"""
...
class BinaryDevice(object):
    ...
    def send(self, *args):
        ...
        Notes:
            ...
            as a convenience to the programmer. Calling ``send(1, 2)``
            is equivalent to calling ``send(BinaryCommand(1, 2))``.
        ...
        Raises:
            UnexpectedReplyError: The reply read was not send by this
                device.
        """
        if isinstance(args[0], BinaryCommand):
            command = args[0]
        else:
            command = BinaryCommand(*args)
    ...
    def move_abs(self, position):
        ...
        """
        self.send(self.number, 20, position)
    ...
    def move_rel(self, distance):
        ...
        """
        self.send(self.number, 21, distance)
    ...
    def move_vel(self, speed):
        ...
        Notes:
            ...
            moving.
        """
        self.send(self.number, 22, speed)

    def stop(self):
        """Sends the "stop" command (23), then waits for the device to
        reply."""
        self.send(self.number, 23)
    ...
    def get_status(self):
        ...
        """
        return self.send(self.number, 54).data
...
class BinarySerial(object):
    ...
    def __init__(self, port):
        ...
        This class will open the port immediately upon instantiation.
        This follows the pattern set by PySerial, which this class uses
        internally to perform serial communication.
    ...
    def write(self, *args):
        ...
        if not isinstance(message, BinaryCommand):
    ...
    def read(self, message_id = False):
        ...
        Raises: 
            TimeoutError: No data was read before the specified timeout
                elapsed.
        """
        reply = self._ser.read(6)
        if len(reply) != 6:
            raise TimeoutError("read timed out.")
        return BinaryReply(reply, message_id)
