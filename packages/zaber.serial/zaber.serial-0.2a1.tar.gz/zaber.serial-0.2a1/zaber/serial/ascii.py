...
class AsciiAxis(object):
    ...
    def move_abs(self, position, blocking = True):
        ...
        Args:
            ...
            blocking: An optional boolean, True by default. If set to
                ...
                the device further.
        """
        self.send("move abs {0:d}".format(position))
    ...
    def move_rel(self, distance, blocking = True):
        ...
        Args:
            ...
            blocking: An optional boolean, True by default. If set to
                ...
                the device further.
        """
        self.send("move rel {0:d}".format(distance))
        if blocking: self.poll_until_idle()
    ...
    def move_vel(self, speed, blocking = False):
        ...
        """
        self.send("move vel {0:d}".format(speed))
        if blocking: self.poll_until_idle()
...
class AsciiDevice(object):
    ...
    def __init__(self, port, address):
        """
        Args:
            port: An AsciiPort object representing the port to which
...
class AsciiReply(object):
    ...
    def __init__(self, reply_string):
        ...
        if reply_string[-3] == ':':
            ...
            # Then truncate the '0x' prefix added by hex().
            correct_checksum = hex(((sum & 0xFF) ^ 0xFF) + 1)[-2:].upper()
        ...
        else: raise ValueError("Invalid response type: %c" % (reply_string[0],))

        # Add any remaining tokens together as the data field.
        try:
            while True: self.data = self.data + ' ' + next(tokens)
        except StopIteration:
            pass
    ...
    def encode(self):
        ...
        if self.message_type == '@':
            ...
            retstr = "#{:02d} {:d} {:s}".format(self.device_address,
                    self.axis_number, self.data) if self.message_id is None \
...
class AsciiSerial(object):
    ...
    def __init__(self, port, baud = 115200):
        ...
        Notes: When *port* is not None, this constructor immediately
    ...
    def read(self):
        ...
        Raises:
            TimeoutError: The duration specified by *timeout* elapsed
                before a full reply could be read.
        """
        line = self._ser.readline()
        if not line:
            raise TimeoutError("read timed out.")
        return AsciiReply(line.decode())
