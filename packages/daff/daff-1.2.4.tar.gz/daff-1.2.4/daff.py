				...
				if (not quoting):
					...
					if ((ch == 34) or ((ch == 39))):
						if (i == self.cursor):
							...
							quote = ch
							if (i != start):
								result = (HxOverrides.stringOrNull(result) + Std.string(u"".join(python_lib_Builtin.map(hxunichr,[ch]))))
							continue
						elif (ch == quote):
							quoting = True
					result = (HxOverrides.stringOrNull(result) + Std.string(u"".join(python_lib_Builtin.map(hxunichr,[ch]))))
	...
	def markSpaces(sl,sr):
		...
		while ((il < python_lib_Builtin.len(sl))):
			...
			if (cl == cr):
				...
				slo = (HxOverrides.stringOrNull(slo) + Std.string(u"".join(python_lib_Builtin.map(hxunichr,[9251]))))
				il = (il + 1)
		return slo
...
class TerminalDiffRender(object):
	...
	def pickSizes(self,t):
		...
		while ((_g < w)):
			...
			most = None
			try:
				most = python_lib_Builtin.int(((mean + ((stddev * 2))) + 0.5))
			except Exception as _hx_e:
				_hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
				e = _hx_e1
				most = None
...
class haxe_io_BytesBuffer(object):
...
haxe_io_BytesBuffer = _hx_classes.registerClass(u"haxe.io.BytesBuffer", fields=[u"b"])(haxe_io_BytesBuffer)

class haxe_io_Input(object):
	pass
haxe_io_Input = _hx_classes.registerClass(u"haxe.io.Input")(haxe_io_Input)
...
haxe_io_BytesInput = _hx_classes.registerClass(u"haxe.io.BytesInput", superClass=haxe_io_Input)(haxe_io_BytesInput)
...
haxe_io_BytesOutput = _hx_classes.registerClass(u"haxe.io.BytesOutput", fields=[u"b"], methods=[u"writeByte",u"writeBytes"], superClass=haxe_io_Output)(haxe_io_BytesOutput)
    ...
    def isSimilar(self,alt):
        ...
        for c in hxrange(self.width):
            for r in hxrange(self.height):
                v1 = u"" + self.getCell(c,r)
                v2 = u"" + alt.getCell(c,r) 
                if (v1!=v2):
