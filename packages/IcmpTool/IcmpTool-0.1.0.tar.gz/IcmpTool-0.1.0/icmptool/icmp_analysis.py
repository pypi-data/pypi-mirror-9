...
def print_router_details(extra_info, r_type):
    ...
    if r_type == 0: #unresponsive
        ...
        print "\t- [on-off] burst size: %s packets, period length: %s s" % (extra_info['onoff_config_filtered']['bsize'],
                                                                extra_info['onoff_config_filtered']['iat'])
        print "\t- [on-off] answering rate: %s pps" % (extra_info['onoff_rate_max'])
...
#    poorly_responsive = -4
    ...
    for an_id in ids:
        ...
        routers_by_type[router_type][an_id] = {'id': an_id, 'regular': regular,
...
def classify_router(router, analyzed_traces, v, verbose=False, fp_dict=None, allowed_noise=0.0):
    """
    ...
    this router appears and it classifies it as being:
        ...
        - pure on-off if bursts appear, their IAT CoV and burst size CoV are < max_cv, 
        ...
        - busy on-onoff: most often pure on-off, but with occasional irregular episodes
            Case 1) Similar parameters, with noise. For any probing rates, CoVs are high but average IAT
    ...
    traces_low_cvs = [t for t in traces_this_router
                        if t[IAT_variation] is not None
                        and t[burst_size_variation] is not None
                        and 0 <= t[IAT_variation] < max_cv
    ...
    table_to_print = prettytable.PrettyTable(['rates', 'IAT_cv', 'bsize_cv',
                                            'IAT_avg', 'bsize_avg', 'loss_rate',
...
#                                    and t[burst_size_variation] is None]
            ...
            if verbose:
                for t in sorted(traces_this_router, key=lambda x: x['actual_rate']):
                    new_row = [t['actual_rate'],
                                round(t[IAT_variation], 4) if t[IAT_variation] else None,
        ...
        parameters = sorted([{'iat': t[IAT_avg],
                            'rate': int(t['actual_rate']),
                            'bsize': t[ans_burst_size_avg],
                            'iat_cv': round(t[IAT_variation], 4) if t[IAT_variation] else None,
...
#                # first approximation of ans rate
    ...
    # # # # # # # # # # # ## # # # ## # # # # ## # #
    extra_info['iat_cvs'] = [t[IAT_variation] for t in traces_low_cvs]
    extra_info['bs_cvs'] = [t[burst_size_variation] for t in traces_low_cvs]
    extra_info['min_iat_cv'] = min(extra_info['iat_cvs'])
    extra_info['min_bs_cv'] = min(extra_info['bs_cvs'])
    extra_info['max_iat_cv'] = max(extra_info['iat_cvs'])
    ...
    # and its parameters
    experiments_low_covs = [{'IAT_avg': t[IAT_avg],
                'rate': int(t['actual_rate']),
                'ans_burst_size_avg': t[ans_burst_size_avg],
                'iat_cv': round(t[IAT_variation], 4),
    ...
    # * * grouping (1) * * *
    #group values when IAT and burst size are roughly the same
    for p in experiments_low_covs:
        #iat
        if p['IAT_avg'] > 0.9:
            p['IAT_avg'] = round(p['IAT_avg'], 1)
        else:
            p['IAT_avg'] = round(p['IAT_avg'], 3)
        #burst
        p['ans_burst_size_avg'] = round(p['ans_burst_size_avg'], 0)
    pairs = list(set([(p['IAT_avg'], p['ans_burst_size_avg']) for p in experiments_low_covs]))
    onoff_params = []
    for p in pairs:
        ...
        lossrates_this_pair = sorted(list(set([round(el['loss_rate'], 4) for el in experiments_low_covs
                            if el['IAT_avg'] == p[0]
                            and el['ans_burst_size_avg'] == p[1]])))
        iatcvs_this_pair = sorted(list(set([round(el['iat_cv'], 3) for el in experiments_low_covs
                            if el['IAT_avg'] == p[0]
                            and el['ans_burst_size_avg'] == p[1]])))
        bcvs_this_pair = sorted(list(set([round(el['b_cv'], 3) for el in experiments_low_covs
                            if el['IAT_avg'] == p[0]
                            and el['ans_burst_size_avg'] == p[1]])))
        onoff_params.append({'IAT_avg': p[0], 'ans_burst_size_avg': p[1],
                            'IAT_cvs': iatcvs_this_pair, 'bsize_cvs': bcvs_this_pair,
    ...
    # * * grouping (2) * * *
    onoff_config_params = []
    pairs_onoff = sorted([(p['IAT_avg'], p['ans_burst_size_avg'], p['rates']) for p in onoff_params])
    ...
    for p in pairs_onoff[1:]:
        ...
        for i, g in enumerate(groups):
            if all( 0.75 <= 1.0 * iat / el[0] <= 1.25 and   
        ...
        if not group_found:
            ...
            groups.append([p])
    for g in groups:
        iat_occurrences = [[el[0]] * sum(1 for p in experiments_low_covs if p['IAT_avg'] == el[0]) for el in g]
        iat = np.mean([el for sublist in iat_occurrences for el in sublist])
        ...
        onoff_config_params.append({'iat': iat, 'bsize': round(bsize, 1),
...
#                          for t in traces_below_min_rate])
    ...
    onoff_rate = int(round(max([params['bsize']*1.0 / params['iat']
                                for params in extra_info['onoff_config_params']]), 0))
    onoff_rate_max = onoff_rate
    extra_info['onoff_rate_max'] = onoff_rate_max
    extra_info['onoff_rate_ans'] = max([t['answering_rate'] for t in onoff_traces_onoff_range])  #TODO BAD results
    extra_info['onoff_rate_most'] = extra_info['onoff_config_filtered']['bsize'] * 1.0 / extra_info['onoff_config_filtered']['iat']
    extra_info['onoff_rate'] = extra_info['onoff_rate_most']
    ...
    extra_info['pre_onoff_tests'] = "\t- [r_min, r_onoff]-test: %s ; (r_onoff, r_0)-test: %s" % (
                                                    test1_str, test2_str)
    ...
    is_pure_onoff_old_old = all([0 <= t[IAT_variation] < max_cv 
                    and 0 <= t[burst_size_variation] < max_cv
                    for t in traces_higher_rates])
    onoff_traces = [ t for t in traces_higher_rates
                    if 0 <= t[IAT_variation] < max_cv 
                    and 0 <= t[burst_size_variation] < max_cv]
#    is_pure_onoff_oldish = max_rate_low_cvs == r_max
    ...
    if verbose:
        for p in onoff_params:
            new_row = [insert_newline_every_n(p['rates'], 2),
                        insert_newline_every_n(p['IAT_cvs'], 2),
    ...
    if is_pure_onoff:
        ...
        logging.info(extra_info['pre_onoff_tests'])
        logging.info(table_to_print)
        table_txt = table_to_print.get_string()
        return rtypes.pure_onoff, onoff_params, [], extra_info
    else:
        irregular = [{ 'rate': int(t['actual_rate']),
                    'iat_cv': round(t[IAT_variation], 4)
                            if t[IAT_variation] is not None else None,
                    'bsize_cv': round(t[burst_size_variation], 4)
                            if t[burst_size_variation] is not None else None,
                    'iat': round(t[IAT_avg], 3)
                            if t[IAT_avg] is not None else None,
                    ...
                    for t in traces_this_router
                    if t['actual_rate'] >= min_rate_low_cvs
                    and not (0 <= t[IAT_variation] < max_cv
                    and 0 <= t[burst_size_variation] < max_cv)]
...
#        allowed_noise = 0.35 #now an input parameter
        ...
        if is_busy_onoff:
            ...
            return rtypes.busy_onoff, onoff_params, irregular, extra_info
        else:
            ...
            if verbose:
                ...
                table2_txt = table2.get_string()
                table_txt = table_to_print.get_string()
            return rtypes.irregular_onoff, onoff_params, irregular, extra_info
...
def extract_data_from_bursts(post_info, v=''):  #v stands for variant
    ...
    if post_info[filtered_ans_bursts] and post_info[filtered_unans_bursts] and post_info[IAT]:
        # compute coefficients of variation for ans bursts size, unans bursts size, IAT
        ...
        # IAT variation
        post_info[IAT_variation] = None
        if len(post_info[IAT]) > 1:
            post_info[IAT_variation] = scipy.stats.variation(post_info[IAT])
        # compute mean ans bursts size, unans bursts size, IAT
        ...
        post_info[IAT_avg] = np.mean(post_info[IAT])
        ...
        post_info[IAT_std] = np.std(post_info[IAT])
        ...
        post_info[ans_burst_size_variation] = post_info[unans_burst_size_variation] = post_info[IAT_variation] = None
        post_info[ans_burst_size_avg] = post_info[unans_burst_size_avg] = post_info[IAT_avg] = None
        post_info[ans_burst_size_std] = post_info[unans_burst_size_std] = post_info[IAT_std] = None
...
def arrange_by_burst(post_info, variants, period_starts_with_ans=True):
    ...
    for var in variants:
        ...
        post_info['IAT' + v] = get_IAT_from_bursts(post_info['filtered_ans_bursts' + v],
                                                    post_info['filtered_unans_bursts' + v],
                                                    period_starts_with_ans=period_starts_with_ans,
                                                    removed_last=removed_last)
        logging.info('\t[%s] - retrieving data: IAT')
        # store coeffient of variation, mean and std for ans burst size, unans burst size and IAT
...
def extract_bursts_from_timeseries(full_timeseries, tolerate_gaps_of=0):
    ...
    tmp_burst = []
    IAT = [] #inter-arrival times, or rather period length
...
def get_IAT_from_bursts(ans_bursts, unans_bursts, period_starts_with_ans=True,
                        removed_last=None):
...
#    answered_bursts_indices = [b['indices'] for b in ans_bursts]
    ...
    if period_starts_with_ans:
        ...
        for i in xrange(smaller_len): # because you want to avoid any IndexErrors
            ...
            iat = next_ans_burst_first_probe['probe']['timestamp'] - first_ans_probe['probe']['timestamp']
            logging.info("\t\t---> iat = %s" % iat)
            IAT.append(iat)
    else:   
        ans_index_offset = 0
        ...
        for i in xrange(smaller_len): # because you want to avoid any IndexErrors
            ...
            iat = last_ans_probe['probe']['timestamp'] - first_unans_probe['probe']['timestamp']
            logging.info("\t\t---> iat = %s" % iat)
            IAT.append(iat)
    #if all probes have been answered or none has, let's define a period as the length of the experiment
    # otherwise IAT would be empty and its variation would be nan
    if not IAT:
        IAT.append(full_timeseries[-1]['probe']['timestamp'] - full_timeseries[0]['probe']['timestamp'])
        logging.info('\tImposing IAT = %s' % IAT)
    logging.info('\t-> periods: %s' % IAT)
    return IAT



def get_bursts_and_IAT_variation(answered_bursts, unanswered_bursts, IAT):
    ...
    IAT_variation = scipy.stats.variation(IAT)
    logging.info('\t=> burst variation: %s, periods variation: %s' % (
                                unanswered_burst_variation, IAT_variation))
    return answered_burst_variation, unanswered_burst_variation, IAT_variation
...
def extract_available_burst_defs(trace_info):
    ...
    """
    burst_defs_available = [k.split('IAT_variation')[1] for k in trace_info.keys()
                        if 'IAT_variation' in k]
