...
"""
# use Python build-in array instead
...
class box(trailAttrs):
    ...
    def __setattr__(self, name, value):
        if name in ['axis','size','length','width','height']:
            ...
            if name == 'axis':
                #self.__dict__['attrsupdt'].add('axis')
                self.addattr('axis')
                #cmd = {"idx": self.idx, "attr": "axis", "val": val.values()}            
                #baseObj.cmds.append(cmd)
            elif name == 'size':
                self.__dict__['axis'] = self.axis.norm() * value[0]
                self.__dict__['height'] = value[1]
                self.__dict__['width'] = value[2]
                #self.__dict__['attrsupdt'].add(name)
                self.addattr(name)
                #cmd = {"idx": self.idx, "attr": name, "val": val.values()}            
                #baseObj.cmds.append(cmd)                
            elif name == 'length':
                ...
                #self.__dict__['attrsupdt'].add('size')
                self.addattr('size')
    ...
    def __setattr__(self, name, value):
        if name in ['pos','color','x','y','z','red','green','blue','size','size_units','shape']:
            ...
            if name == 'size':
                self.__dict__[name] = value
                self.addattr(name)
                #cmd = {"idx": self.idx, "attr": "size", "val": self.size}            
                #baseObj.cmds.append(cmd)
            elif name == 'size_units':
                if value in ['pixels','world']:
                    self.__dict__[name] = value
                    self.addattr(name)
                    #cmd = {"idx": self.idx, "attr": "size_units", "val": self.size_units}            
                    #baseObj.cmds.append(cmd)
            elif name == 'shape':
                if value in ['round','square']:
                    self.__dict__[name] = value
                    self.addattr(name)
                    #cmd = {"idx": self.idx, "attr": "shape", "val": self.shape}            
                    #baseObj.cmds.append(cmd)
            elif name == 'pos':
                if type(value) is list:
                    for idx, val in enumerate(value):
    ...
    def __setattr__(self, name, value):
        if name in ['length','axis','shaftwidth','headwidth','headlength','fixedwidth']:
            ...
            if name == 'length':
                ...
                #cmd = {"idx": self.idx, "attr": "axis_and_length", "val": self.axis.values()}            
                #baseObj.cmds.append(cmd)
            elif name == 'axis':
                self.addattr('axis_and_length')
                #cmd = {"idx": self.idx, "attr": "axis_and_length", "val": val.values()}            
                #baseObj.cmds.append(cmd)
