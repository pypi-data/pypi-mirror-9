...
if IPython.__version__ >= '3.0.0' :
    IPython.html.nbextensions.install_nbextension(path=package_dir+"/data/jquery-ui.custom.min.js",overwrite=True,verbose=0)
    IPython.html.nbextensions.install_nbextension(path=package_dir+"/data/glow.1.1.min.js",overwrite=True,verbose=0)
    IPython.html.nbextensions.install_nbextension(path=package_dir+"/data/glowcomm.js",overwrite=True,verbose=0)
else:
...
class baseObj(object):
    ...
    def appendcmd(self,cmd):
        ...
        if (baseObj.glow != None):
            ...
            print("appendcmd sending cmd to baseObj.cmds " + str(cmd))
            baseObj.cmds.append(cmd)
...
def commsend():
    global next_call, commcmds, lock
    lock.acquire()
    try:
        if (baseObj.glow != None):
            if (len(baseObj.cmds) > 0):
