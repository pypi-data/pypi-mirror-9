...
class SliderPlugin(CMSPlugin):
    ...
    effect = CharField(_('effect'), choices=EFFECT_CHOICES, max_length=50,
                       default="random")
    manual_advance = BooleanField(_('manual advance'), default=False)
    ...
    thumbnails = BooleanField(_('thumbnails'), default=False,
                              help_text=_('Thumbnails for navigation [only '
                                          'works with the default theme!]'))
    random_start = BooleanField(_('random start'), default=False)
