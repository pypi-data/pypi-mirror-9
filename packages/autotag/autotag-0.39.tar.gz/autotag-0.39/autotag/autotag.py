...
#             featuresets = [(entry['features'], tag in entry['tags']) for entry in data if entry['tags']]
            ...
            features = [unicode(str(feature), encoding = 'utf-8', errors = 'replace') for feature in classifier.most_informative_features(10)]
            logging.info(unicode(tag)+u': '+ u' '.join(features))
            with open(u'classifier.'+unicode(tag).encode('utf-8'),'w') as classifier_file:
    ...
    def test(self,data):
        ...
        for tag in tags:
            print(tags.index(tag, ))
            fname = u'classifier.'+unicode(tag)
            if not os.path.isfile(fname):
                continue
            with open(fname,'r') as classifier_file:
