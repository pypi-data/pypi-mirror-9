...
- Use one simple call to ``@cache``, and a majority of all hidden complexity
  ...
  - ``@lazy`` and ``@singletons`` are existing, but are merely aliases to ``@cache``
    but provide a little more sane defaults and some little syntax check.
...
You can use the ``cache`` decorator with properties, and this provides
a good way to have lazy evaluated attributes::
...
To make things clearer, and choose sane defaults (especially for the
CacheStore which really needs only one cell), we have a ``@lazy``
decorator, that applies only on property or method/function with one
argument, so::
    ...
    Traceback
    ...
    SyntaxError: lazy can be called only on properties or method and function with no arguments.
...
Or these::
    ...
    >>> class WithLazyProperty(MyObject):
    ...
    ...    @lazy
    ...    @property
    ...    def sum(self):
    ...
    >>> xx = WithLazyProperty(2, 2)
    >>> xx.sum, xx.product, xx.div()
    evaluating sum...
    evaluating product...
    evaluating div...
    (4, 4, 1)
...
Please notice also that the ``use=`` optional argument is kept, even if
this make no sense to have a complex store, you could have some use of
caching stats (was your function called, how many times, how long did it take).
...
You can use the ``cache`` decorator with classes, and this provide a good
way to make your class a singleton factory::

    >>> @cache
    ... class WithClasses(MyObject):
    ...    pass

    >>> WithClasses() is WithClasses()
    True

This is quite similar to the lazy evaluation scenario. You could use ``@lazy``:
...
But we provide a ``@singleton`` decorator for this purpose, which is an alias to
``@lazy`` which have specific defaults for singletons.
