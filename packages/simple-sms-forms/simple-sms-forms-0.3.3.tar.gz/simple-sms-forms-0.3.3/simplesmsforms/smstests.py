...
class PersonForm(SMSForm):
    ...
    POSITION_CHOICES = ("clerk", "officer", "supervisor", "director")
    ...
    def get_fields(self):
        return [self.first_name, self.last_name, self.age, self.location, self.position, self.date]
...
class TestSMSForms(unittest.TestCase):
    ...
    def test_form_validation_date(self):
        self.person_form.first_name.prefixes = ["fn"]
    ...
    def testDuplicateFields(self):
        self.person_form.first_name.prefixes.append('Mn')
        text = "REG fnAndre MnMwansa lnLesa ag12 locLusaka"
        validated, python_fields, errors = self.person_form.process_form(text)
        for error in errors:
            self.assertIsInstance(error, DuplicateFieldsException)


    def testMultipleChoiceDuplicate(self):
        text = "REG fnAndre lnLesa ag12 locLusaka Clerk Director Clerk"
        validate, python_fields, errors = self.person_form.process_form(text)
        for error in errors:
            #self.assertIsInstance(exception, DuplicateFieldsException)
            self.assertIsInstance(error, DuplicateFieldsException)


    def testGarbageDataDetection(self):
        text = "REG fnAndre lnLesa ag12 locLusaka Clerk garbage_data, more_garbage"

        validate, python_fields, errors = self.person_form.process_form(text)
        for error in errors:
            #self.assertIsInstance(exception, DuplicateFieldsException)
            self.assertIsInstance(error, UnrecognizedDataFoundException)
