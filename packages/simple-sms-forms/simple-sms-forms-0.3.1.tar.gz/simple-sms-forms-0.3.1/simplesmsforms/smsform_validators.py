...
def reject_garbage_data(form_keyword, raw_text_string, bound_fields_dict):
    ...
    for field in bound_fields_dict:
        ...
        modified_raw_text = re.sub(value_reg, repl="", string=modified_raw_text)

        if prefix:
            prefix_reg = r'\b{prefix}'.format(prefix=prefix)
            modified_raw_text = re.sub(prefix_reg, repl="", string=modified_raw_text)
    #Remove the form keyword.
    modified_raw_text = re.sub(form_keyword, repl="", string=modified_raw_text)
    #Remove all non-alphanumeric characters
    modified_raw_text = re.sub('[\W_]', '', modified_raw_text)
