...
def _get_config_scheme():
    scheme = {
        ...
        "collector": {
            "done_lifetime": optconf.Option(default=0, help="Seconds to wait before deleting completed job"),
        },
    }
    for app in ("api", "worker"):
        scheme[app].update({
            "backend_connections": optconf.Option(
                default={"api": 1, "worker": 10}[app],
                help="Maximum number of backend connections",
            ),
        })
    for app in ("worker", "collector"):
        scheme[app].update({
            ...
            "empty_sleep": optconf.Option(
                default={"worker": 0.1, "collector": 1}[app],
                help="Interval after which process will sleep when there are no jobs (seconds)",
            ),
        })
    return scheme
