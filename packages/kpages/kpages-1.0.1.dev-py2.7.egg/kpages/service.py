                ...
                while True:
                    try:
                        ...
                        for func in srv_funcs:
                            ...
                            print "{}: CONSUMER {}[{}: {}]".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), data.get('index', ''), func, data.keys())

                            try:
                                func(data)
                            except:
                                print "{} FUNC EXCEPT [{}]".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), traceback.format_exc())
