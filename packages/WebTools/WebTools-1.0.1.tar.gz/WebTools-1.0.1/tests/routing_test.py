...
from webtools.handlers import RedirectHandler
from webtools.adapters import BaseHandlerAdapter
from webtools.request import Request
from webtools.response import Response
from webtools.route import BaseRoute, Route, Router, SimpleRoute
from webtools.wsgi import WSGIApplication, exc
...
class TestRoute(test_base.BaseTestCase):
    ...
    def test_reverse_uri_template(self):
        route = Route('/foo', None)
        # Access route.regex just to set the lazy properties.
        regex = route.regex
        self.assertEqual(route.reverse_uri_template, '/foo')

        route = Route('/foo/<bar>', None)
        # Access route.regex just to set the lazy properties.
        regex = route.regex
        self.assertEqual(route.reverse_uri_template, '/foo/%(bar)s')

        route = Route('/foo/<bar>/<baz:\d>', None)
        # Access route.regex just to set the lazy properties.
        regex = route.regex
        self.assertEqual(route.reverse_uri_template, '/foo/%(bar)s/%(baz)s')
    ...
    def test_invalid_template(self):
        ...
        # Access route.regex just to set the lazy properties.
        regex = route.regex
        self.assertEqual(route.reverse_uri_template, '/<foo/<:bar')
    ...
    def test_set_adapter(self):
        def custom_adapter(router, handler):
            class MyAdapter(BaseHandlerAdapter):
    ...
    def test_methods(self):
        ...
        self.assertTrue(router.match(req) is not None)
        req.method = 'PUT'
        self.assertRaises(exc.HTTPMethodNotAllowed, router.match, req)
