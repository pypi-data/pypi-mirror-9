...
QUERY_OBJ_GET = """
SELECT ObjName, ObjType, Version, ChangedBy, TimeCreated, LastUpdated, Data
FROM Objects WHERE ObjName=%s;"""
QUERY_OBJ_UPDATE = """
UPDATE Objects
SET Version=%s, ChangedBy=%s, LastUpdated=%s, Data=%s
WHERE ObjName=%s AND Version=%s;"""
QUERY_OBJ_DELETE = """
WITH deleted_row AS (
    DELETE FROM Objects
    WHERE  ObjName=%s AND Version=%s
    RETURNING ObjType, ObjName)
...
AND NOT EXISTS (
   ...
   AND objs.ObjName <> deleted_row.ObjName );"""
QUERY_OBJ_VERSION = "SELECT Version FROM Objects WHERE ObjName=%s LIMIT 1;"
QUERY_OBJ_EXISTS = """
SELECT EXISTS (SELECT 1 FROM Objects WHERE ObjName=%s) AS "exists";"""
