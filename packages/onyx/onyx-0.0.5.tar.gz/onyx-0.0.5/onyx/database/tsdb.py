...
QUERY_CRV_EXISTS = "SELECT 1 FROM {0:s} WHERE Name=%s LIMIT 1;"
...
class TsDbClient(object):
    ...
    def get_curve(self, name, start=None, end=None):
        ...
        curs = self.conn.cursor()
        try:
            curs.execute(QUERY_CRV_EXISTS.format("Curves"), (name, ))
        except psql.OperationalError:
            print_error(curs)
            raise
    ...
    def get_hlocv(self, name, start=None, end=None, field=None):
        ...
        curs = self.conn.cursor()
        try:
            curs.execute(QUERY_CRV_EXISTS.format("HLOCVCurves"), (name, ))
        except psql.OperationalError:
            print_error(curs)
            raise
