...
QUERY_CLS_GET = "SELECT * FROM ClassDefinitions;"
QUERY_CLS_DELETE = """
DELETE FROM ClassDefinitions row WHERE row.ObjType=%s AND NOT
EXISTS (SELECT 1 FROM Objects WHERE ObjType=row.ObjType LIMIT 1);"""

QUERY_OBJ_INSERT = "INSERT INTO Objects VALUES (%s,%s,%s,%s,%s,%s,%s);"
QUERY_OBJ_GET = "SELECT ObjName, ObjType, Version, ChangedBy, TimeCreated, " \
                "LastUpdated, Data FROM Objects WHERE ObjName=%s;"
QUERY_OBJ_UPDATE = "UPDATE Objects SET Version=%s,ChangedBy=%s," \
                   "LastUpdated=%s,Data=%s WHERE ObjName=%s AND Version=%s;"
QUERY_OBJ_DELETE = "DELETE FROM Objects WHERE ObjName=%s AND Version=%s"
QUERY_OBJ_VERSION = "SELECT Version FROM Objects WHERE ObjName=%s LIMIT 1;"
QUERY_OBJ_EXISTS = "SELECT 1 FROM Objects WHERE ObjName=%s LIMIT 1;"
...
class ObjDbClient(ObjDbBase):
    ...
    def initialize(self):
        ...
        curs.execute("""
            ...
            CREATE TABLE
            Objects (
                ObjName     varchar(64) PRIMARY KEY,
            ...
            CREATE INDEX objects_namever_idx ON Objects (ObjName, Version);
            CREATE INDEX objects_objtype_idx ON Objects (ObjType);

            -- this table is used to store archived (i.e. dated) objects
            CREATE TABLE
            Archive (
                ObjName   varchar(64) REFERENCES Objects,
                ObjType   varchar(64) REFERENCES ClassDefinitions,
                Date      date        NOT NULL,
                ChangedBy varchar(64) NOT NULL,
                TimeStamp timestamptz NOT NULL,
                Data      json        NOT NULL,
                CONSTRAINT NameDate UNIQUE (ObjName, Date) );

            CREATE INDEX archive_name_idx ON Archive (ObjName);""")
    ...
    def class_lookup(self, cls_name):
        try:
            ...
            curs = self.conn.cursor()
            curs.execute("SELECT ClassDef FROM "
                         "ClassDefinitions WHERE ObjType=%s;", (cls_name,))
            row = curs.fetchone()
            if row is None:
    ...
    def get(self, name, refresh=False):
        try:
            ...
            if row is None:
                ...
                cls = self.class_lookup(row[1])
                obj = cls.__new__(cls)

                for k, attr in enumerate(SPECIAL):
                    setattr(obj, attr, row[k])
    ...
    def update(self, obj):
        ...
        try:
            ...
            if curs.rowcount == 0:
                ...
                if curr_ver < db_ver[0]:
                    msg = MSG_RELOAD.format(obj.Name, curr_ver, db_ver[0])
        ...
        try:
            ...
            if curs.rowcount == 0:
                ...
                if db_ver is None:
                    raise ObjNotFound("Object '{0:s}' not found "
                                      "in {1:s}".format(obj.Name, self.dbname))
                elif obj.Version < db_ver[0]:
                    msg = MSG_RELOAD.format(obj.Name, obj.Version, db_ver[0])
                    raise ObjDbError(msg)
                else:
                    raise ObjDbError("Deletion of {0:s} failed without "
                                     "obvious cause...".format(obj.Name))

            # --- cheanup ClassDefinitions
            curs.execute(QUERY_CLS_DELETE, (obj.Name,))
    ...
    def get_dated(self, name, date, strict=False):
        try:
            ...
            if strict:
                query = """SELECT
                           ObjName, ObjType, Date, ChangedBy, TimeStamp, Data
                           FROM Archive
                           WHERE ObjName = %s AND Date = %s;"""
                parms = (name, date)
            else:
                query = """SELECT
                           ObjName, ObjType, Date, ChangedBy, TimeStamp, Data
                           FROM Archive
                           WHERE ObjName = %s AND Date IN
            ...
            if row is None:
                ...
                setattr(obj, "ObjType", row[1])
                setattr(obj, "ChangedBy", row[3])
                setattr(obj, "LastUpdated", row[4])
    ...
    def update_dated(self, obj, date, override=False):
        ...
        query = """SELECT ObjName FROM Archive
                   WHERE ObjName=%s AND Date=%s LIMIT 1;"""
