...
##-----------------------------------------------------------------------------
def UpdateDatedObj(obj, date, override=False):
    # --- input validation
    if not isinstance(obj, UfoBase):
        raise ObjDbError("UpdateDatedObj must be fed "
                         "an object derived from UfoBase")

    return onyx_db.obj_clt.update_dated(obj, date, override)
...
def ObjNamesByType(obj_type=None):
   ...
   """
    ...
    if obj_type is None:
        curs.execute("SELECT ObjName FROM Objects;")
    else:
        curs.execute("SELECT ObjName "
