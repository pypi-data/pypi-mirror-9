...
from onyx.database.tsdb import TsDbClient
...
class UseTsDatabase(object):
    ...
    def __enter__(self):
        self.current = onyx_db.ts_clt
        onyx_db.ts_clt = TsDbClient(self.dbname, USER)

    def __exit__(self, exc_type, exc_value, traceback):
        onyx_db.ts_clt.close()
        onyx_db.ts_clt = self.current
...
##-----------------------------------------------------------------------------
def GetRowBy(table, name, date=None, strict=False):
    """
    Description:
...
##-----------------------------------------------------------------------------
def GetCurve(name, start=None, end=None, crv_type="CRV", field=None):
    """
    Description:
