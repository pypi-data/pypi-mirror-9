...
from onyx.depgraph.graph_api import GetVal, SetVal, InvalidateNode
...
class TestGraph(unittest.TestCase):
    def setUp(self):
        self.use_db = UseDatabase("TestDb")
    ...
    def test_caching(self):
        ...
        self.assertAlmostEqual(t.elapsed, 4.0, 2)

        # --- compute A a second time: result is cached
        with timer() as t:
            val = GetVal("obj1", "A")
        self.assertEqual(val, 1003.0)
        self.assertAlmostEqual(t.elapsed, 0.0, 2)
        ...
        with timer() as t:
            val = GetVal("obj1", "A")
        self.assertEqual(val, 1003.0)
        self.assertAlmostEqual(t.elapsed, 2.0, 2)
