...
def TableGroupBy(table, pivot, measures):
    ...
    # --- key-function for vertical pivot
    keyfunc = lambda r: r[pivot]
    ...
    pivot_table = [tags]
    for key, group in itertools.groupby(table.data, keyfunc):
        group = list(group)
...
def TableToCsv(table, file_name=None, delimiter=",", use_quotes=False):
    ...
    """
    if use_quotes:
        fstr = lambda x: '"{0:s}"'.format(str(x).encode("cp850", "replace"))
    else:
        fstr = lambda x: "{0:s}".format(str(x).encode("cp850", "replace"))
    ...
    try:
        ...
        for row in table.data:
            csvfile.writelines(delimiter.join([fstr(item)
