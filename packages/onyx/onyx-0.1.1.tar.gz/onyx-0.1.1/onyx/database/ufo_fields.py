...
pickle_loads = lambda val: pickle.loads(base64.b64decode(val))
pickle_dumps = lambda val: base64.b64encode(pickle.dumps(val)).decode("utf-8")
...
class StructureField(MutableField):
    ...
    def to_json(self, val):
        return val if val is None else zip(val.keys(), val.values())
