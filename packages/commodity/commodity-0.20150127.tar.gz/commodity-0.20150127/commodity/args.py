...
class ArgumentConfigParser(argparse.ArgumentParser):
    ...
    def _do_load_config(self, config):
        ...
        self._update(self, new_config['ui'])

    def _update(self, parser, new_values, command=None):
        debug('----')
        ...
        for key, sec in new_values.items():
            if key.startswith('cmd:'):
                ...
                if not isinstance(sec, collections.MutableMapping):
                    print "{} should be a config section".format(command)
                    sys.exit(1)
        ...
        for action in parser._actions:
            ...
            if isinstance(action, argparse._SubParsersAction) and value is not None:
