...
class Authentication():
    ...
    def check_session(self, _session_id):
        ...
        if len(_sessions) > 0:
            ...
            _user_condition = {
                "conditions": {"_id": mbe_object_id(_sessions[0]["user_id"]), "schemaId": schemaId_user},
                "collection": "node"
            }
            ...
            raise AuthenticationError("Authentication failed." + str(_session_id))
    ...
    def login(self, _credentials):
        ...
        if "usernamePassword" in _credentials:
            _credentials["usernamePassword"]["password"] = hashlib.md5(
                _credentials["usernamePassword"]["password"].encode('utf-8')
            ).hexdigest()

            _cred_condition = {
                "conditions": {"credentials": _credentials},
                "collection": "node"
            }
