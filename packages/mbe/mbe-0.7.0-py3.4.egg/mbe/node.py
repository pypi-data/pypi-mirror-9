...
class Node():
    ...
    def __init__(self, _database_access, _rights=None):
        ...
        if _rights is not None:
            node_rights = [object_id_right_admin_everything] + _rights
        else:
            node_rights = [object_id_right_admin_everything]
    ...
    def _string_to_object_ids(self, _data):
        ...
        elif type(_data) != ObjectId and _data[:9] == "ObjectId(":
    ...
    @aop_has_right(get_node_rights)
    def find(self, _conditions, _user):
    ...
    def history(self, _id, _user):
        ...
        return self.database_access.find({"conditions": {"node_id": object_id}, "collection": "event"})