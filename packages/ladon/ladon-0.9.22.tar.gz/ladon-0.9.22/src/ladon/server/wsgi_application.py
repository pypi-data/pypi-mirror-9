...
class LadonWSGIApplication(object):
	...
	def generate_catalog_html(self,services,client_path,catalog_name,catalog_desc,charset,skin):
	...
	def __call__(self,environ, start_response):
		...
		try:
			fault_hint = u'Processing request (further details)'
			...
			if sname in self.reserved_roots:
				fault_hint = u'Attempting to fetch server-side static skin-data at location: "%s"' % environ['PATH_INFO']
				if sname=='skins':
			...
			if sinst and ifclass:
				fault_hint = u'Instantiating dispatcher for "%s"' % sname
				dispatcher = Dispatcher(sinst,ifclass,charset,self.logging)
			elif not sname:
				fault_hint = u'Generating service catalog'
				...
				output = self.generate_catalog_html(
					...
					client_path,
					self.catalog_name,
					self.catalog_desc,charset,skin)
			elif sinst and not ifname:
				fault_hint = u'Generating service API HTML for "%s"' % sname
				content_type = 'text/html'
				skin = query.get('skin', [None])[0]
			...
			if dispatcher and dispatcher.iface:
				if action=='description':
					fault_hint = u'Generating "%s" description for "%s"' % (ifname,sname)
					...
					if environ['REQUEST_METHOD'] not in allowed_methods or not environ.get('CONTENT_LENGTH', ''):
						...
						fault_hint = u'Preparing "%s" call to "%s"' % (ifname,sname)
						content_type = dispatcher.iface.response_content_type()
						content_length = int(environ['CONTENT_LENGTH'])
						if multipart and boundary:
							fault_hint = u'Parsing incoming multipart request and attachments'
						...
						fault_hint = u'Passing request on to the dispatcher'
