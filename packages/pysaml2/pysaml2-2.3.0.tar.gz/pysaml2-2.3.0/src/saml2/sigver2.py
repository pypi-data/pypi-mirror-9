...
def key_from_key_value(key_info):
    res = []
    for _value in key_info.key_value:
        if _value.rsa_key_value:
            e = base64_to_long(_value.rsa_key_value.exponent)
            m = base64_to_long(_value.rsa_key_value.modulus)
            res.append(RSA.construct((m, e)))
    return res
...
def verify_redirect_signature(saml_msg, cert):
    ...
    try:
        ...
        if saml_msg["SigAlg"][0] == SIG_RSA_SHA1:
            ...
            string = "&".join(
                [urllib.urlencode({k: args[k][0]}) for k in _order if
                 k in args])
...
class CertHandler(object):
    def __init__(self, security_context, mtrl, generate_cert_info=None,
                 cert_handler_extra_class=None, tmp_file=None,
                 verify_cert=False):
        """
        ...
        Specific to signature/encryption

        :param security_context:
        :param mtrl: key material
        ...
        if mtrl["cert_type"] == "pem" and mtrl["key_type"] == "pem":
            self._verify_cert = verify_cert is True
            self._osw = OpenSSLWrapper()
            if mtrl["key_file"] and os.path.isfile(mtrl["key_file"]):
                self._key_str = self._osw.read_str_from_file(
                    mtrl["key_file"], mtrl["key_type"])
            else:
                self._key_str = ""

            if mtrl["cert_file"] and os.path.isfile(mtrl["cert_file"]):
                self._cert_str = self._osw.read_str_from_file(
                    mtrl["cert_file"], mtrl["cert_type"])
            else:
                self._cert_str = ""
            ...
            try:
                self._tmp_cert_file = tmp_file["cert"]
            except KeyError:
                self._tmp_cert_file = ""
            try:
                self._tmp_key_file = tmp_file["key"]
            except KeyError:
                self._tmp_key_file = ""
            ...
            if generate_cert_info is not None \
                    ...
                    and self._tmp_key_file is not "" \
                    and self._tmp_cert_file is not "":
...
class SecurityContext(object):
    ...
    def __init__(self, crypto, keys, metadata=None,
                 debug=False, template="", encrypt_key_type="des-192",
                 only_use_keys_in_metadata=False, cert_handler_extra_class=None,
                 generate_cert_info=None, tmp_file=None,
                 validate_certificate=None):
        """

        :param crypto:
        :param keys: list of dictionaries of the form
        {"usage": "", "file": "", "file_type": "", "key": "", "cert": ""}
        :param metadata:
        :param debug:
        :param template:
        :param encrypt_key_type:
        :param only_use_keys_in_metadata:
        :param cert_handler_extra_class:
        :param generate_cert_info:
        :param tmp_file: dictionary {"enc": {"key": "", "cert": ""}, ...}
        ...
        for typ in ["sig", "enc"]:
            self.cert_handler[typ] = CertHandler(
                self, keys[typ], generate_cert_info,
                cert_handler_extra_class, tmp_file["sig"],
                validate_certificate)

            self.cert_handler[typ].update_cert(True)
    ...
    def _add_key(self, typ, spec):
        if spec["file"]:
            assert os.path.isfile(spec["file"])

            try:
                ftype = spec["file_type"]
            except KeyError:
                ftype = "pem"

            try:
                _cert = read_cert_from_file(spec["file"], ftype)
                try:
                    self.my_crt[typ].append(_cert)
                except AttributeError:
                    self.my_crt[typ] = [_cert]
            except CertificateError:
                # raises exception if not proper
                _key = import_rsa_key_from_file(spec["file"])
                try:
                    self.my_key[typ].append(_key)
                except AttributeError:
                    self.my_key[typ] = [_key]
        elif "key" in spec:
            _key = RSA.importKey(spec["key"])
            try:
                self.my_key[typ].append(_key)
            except AttributeError:
                self.my_key[typ] = [_key]
        elif "cert" in spec:
            _crt = "".join(spec["cert"].split("\n"))
            try:
                self.my_crt[typ].append(_crt)
            except AttributeError:
                self.my_crt[typ] = [_crt]
    ...
    def decrypt(self, enctext, key_file=None):
        ...
        if key_file is not None and len(key_file.strip()) > 0:
            ...
            return self.crypto.decrypt(enctext, self.key["enc"]["key_file"])
    ...
    def verify_signature(self, signedtext, cert_file=None, cert_type="pem",
        ...
        if not cert_file:
            cert_file = self.key["sig"]["cert_file"]
            cert_type = self.key["sig"]["cert_type"]
    ...
    def _check_signature(self, decoded_xml, item, node_name=NODE_NAME,
                         origdoc=None, id_attr="", only_valid_cert=False):
        #print item
        try:
        ...
        if not certs and not self.only_use_keys_in_metadata:
            ...
            certs = [make_temp(pem_format(cert), suffix=".pem",
                               decode=False,
                               delete=self._xmlsec_delete_tmpfiles) for cert
                     in cert_from_instance(item)]
        else:
        ...
        if (not verified) and (not only_valid_cert):
            ...
            if not self.cert_handler["sig"].verify_cert(last_pem_file):
    ...
    def correctly_signed_response(self, decoded_xml, origdoc=None,
    ...
    def sign_statement(self, statement, node_name, key=None,
        ...
        if not key and not key_file:
            key_file = self.key["sig"]["key_file"]
    ...
    def multiple_signatures(self, statement, to_sign, key=None, key_file=None):
        ...
        for (item, sid, id_attr) in to_sign:
            ...
            if not item.signature:
                item.signature = pre_signature_part(sid,
                                                    self.key["sig"][
                                                        "cert_file"])
