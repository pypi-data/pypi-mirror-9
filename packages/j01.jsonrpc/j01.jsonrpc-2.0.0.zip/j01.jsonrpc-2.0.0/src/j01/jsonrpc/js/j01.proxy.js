 ...
 * }
 * var onError = function(xhr){// do something}
...
JSONRPC.prototype.addMethod = function(name, onSuccess, onError, onTimeout, isPushState, id) {
    ...
    var self = this;
    if(!self[name]){
        var method = new JSONRPCMethod(self, self._url, name, onSuccess, onError, onTimeout, isPushState, id);
...
var JSONRPCMethod = function (proxy, url, name, onSuccess, onError, onTimeout, isPushState, id) {
    ...
    this.id = id;
    // default values will get ovverriden by addMethod
    this.version = '2.0';
            ...
            var onCallback = function(xhr){
                ...
                try {
                    if(resp && xhr.status == 200){
                        ...
                        // handle response error. Note: error could be null
                        self.doHandleError(resp);
                    }
                } catch(e){
...
JSONRPCMethod.prototype.getResponseData = function(xhr){
    // returns result or error from response
    var source = "";
    try {
        // Firefox throws an exception when accessing
        // statusText for faulty cross-domain requests
    ...
    if (source !== null && source !== "") {
        ...
        try {
            ...
            // doHandleError will handle error or trigger an error event
            return null;
        }
        // apply is push state marker
        if (data && typeof isPushState !== 'undefined' && isPushState !== null) {
            data.isPushState = self.isPushState
        }
...
JSONRPCMethod.prototype.doHandleError = function(resp) {
    ...
    if (typeof(this.onError) !== 'undefined') {
        // custom error handler method. Note: response could be none on
        // connection error
        this.onError(resp);
    }
    else if (resp && resp.data && resp.data.nextURL) {
