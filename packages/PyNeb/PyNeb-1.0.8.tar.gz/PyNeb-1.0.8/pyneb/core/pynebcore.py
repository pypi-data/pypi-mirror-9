...
class Atom(object):
    ...
    @profile
    def __init__(self, elem=None, spec=None, atom=None, OmegaInterp='Cheb', noExtrapol = False):
        """
        Atom constructor
...
#        print 'maxError on ratio %f' % maxError
    ...
    def plotGrotrian(self, tem=1e4, den=1e2, thresh_int=1e-3, unit='eV', detailed=False):
        """
...
#            levelLabels = ['$^{{{0}}}${1}$_{{{2:.0f}}}$'.format(l['term'][0],l['term'][1],l['J']) for l in self.NIST]
        ...
        for i_multi in np.arange(len(multiplets)):
            ...
            for i in np.arange(len(multiplets[i_multi])):
                ...
                plt.plot([x_0, 1.5 * x_0, 1.6 * x_0, x_0 + x_span], 
                ...
                plt.text(x_span + x_0 + dx, y, '{0:7.4f}'.format(energies[multiplets[i_multi][i]]), size='small', 
                         horizontalalignment='left', verticalalignment='center')
                try:
                    if (detailed is False):
                        plt.text(dx, y, levelLabels[i_tot], size='medium', verticalalignment='center')
                    else:
                        plt.text(dx, y, str(i_tot+1) + ": " + levelLabels[i_tot] , size='medium', verticalalignment='center', color = color_list[np.mod(multiplets[i_multi][i], len(color_list))])
                    i_tot += 1
                except:
                    pass
                shift += 1
        ...
        for j in np.arange(self.NLevels - 1) + 2:
            for i in np.arange(j - 1) + 1:
                ...
                if emis > (emis_max * thresh_int):
                    ...
                    plt.text(x + 0.05 * delta_x, self.getEnergy(j, unit=unit)-blow/2., to_print, size='small', rotation=90, verticalalignment='top')
                    x = x + delta_x

        # issue warning on plot if level inversion 
        try:
            for i in warn_label:
                plt.text(0, self.getEnergy(multiplets[i][0]+1, unit=unit), 'Warning ', ha='right', color='red')
        except:
            pass
        plt.show()
