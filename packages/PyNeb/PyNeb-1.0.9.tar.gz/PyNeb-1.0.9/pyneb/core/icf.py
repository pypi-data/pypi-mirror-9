...
# Misleading comment clarified on 12 Dec 2014 
                         'KB94_A13.10': {'elem': 'C',
                                      ...
                                      'type': 'PNe',
#                                     'comment': 'Based on a grid of photoionization models. To be used if no C II lines detected'},
                                      'comment': 'Based on a grid of photoionization models. To be used if C5 is not present; only C3 lines are detected; O3 also detected; He2 and (maybe) He3 also detected'},
...
# Wrong comment. Corrected 19 Dec 2014
#                                      'comment': 'Based on a grid of photoionization models. Valid if N4 or N5 not seen'},
                                      'comment': 'Based on a grid of photoionization models. Valid if neither Ne4 nor Ne5 are seen, if C5 is not present; only C3 lines are detected; O3 also detected; He2 and (maybe) He3 also detected'},
...
#                                     'type': 'PNe',
#                                     'icf': '1./(1. - abun["N2"] / elem_abun["KB94_A1.6"])',
#                                      'comment': 'Based on a grid of photoionization models. To be used if both O4 and N5 detected'},
#                         'KB94_A30.8': {'elem': 'Ar',
#                                     'atom': 'abun["Ar3"] + abun["Ar4"] + abun["Ar5"]',
#                                     'icf': '1./(1. - abun["N2"] / elem_abun["KB94_A1.8"])',
#                                     'type': 'PNe',
#                                      'comment': 'Based on a grid of photoionization models. To be used if both O4 and N5 detected'},
#                         'KB94_A30.10': {'elem': 'Ar',
#                                     'atom': 'abun["Ar3"] + abun["Ar4"] + abun["Ar5"]',
...
#                                      'url': 'http://adsabs.harvard.edu/abs/2004ApJS..153..501G'},
    ...
    def getElemAbundance(self, atom_abun, icf_list=[], absentIon=np.nan, use_coll=True):
        """
                ...
                if self.all_icfs[icf_label]['elem'] is not None:
                    ...
                    try:
                        ...
                        if i_pass == self._max_pass:
                            pn.log_.warn('{0} cannot be evaluated'.format(self.all_icfs[icf_label]['icf']),
