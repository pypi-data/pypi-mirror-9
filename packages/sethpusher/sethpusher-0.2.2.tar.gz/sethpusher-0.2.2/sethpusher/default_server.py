...
from sethpusher import applications
...
class AppRunner(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._port = kwargs.get('port', self.port)
        self.options = kwargs

    def start(self, app):
        logging.info(u"Application Class: {0}".format(app.__name__))
        application = app(**self.options)
        server = HTTPServer(application)
        server.listen(self._port, self._host)
...
def main():
    ...
    runner.start(app=ApplicationClass)