...
class SWMM5Simulation(object):
    ...
    def _setvariables(self):
        with self._SWMM5Results_file_open(self.outFile):
            ...
            self._ids["SUBCATCH"  ]=[0,OrderedDict(sorted(list(d.items()), key=lambda t: t[1]))]
            d={swmm5.GetIDName():x for x in range(swmm5.cvar.SWMM_Nnodes)}
            self._ids["NODE"      ]=[1,OrderedDict(sorted(list(d.items()), key=lambda t: t[1]))]
            d={swmm5.GetIDName():x for x in range(swmm5.cvar.SWMM_Nlinks)}
            self._ids["LINK"      ]=[2,OrderedDict(sorted(list(d.items()), key=lambda t: t[1]))]
