...
class NBTMeta(type):
    ...
    def __setattr__(cls, name: str, value: object):
        ...
        if not isinstance(value, fields.AbstractField):
            super().__setattr__(name, value)
            return
        if name in cls.__fields:
            hidden = cls.__fields[name]
            hidden.detach(cls, name)
        super().__setattr__(name, value)
        value.attach(cls, name)
        cls.__fields[name] = value
    ...
    def __delattr__(cls, name: str):
        ...
        if not isinstance(field, fields.AbstractField):
            ...
            return
        field.detach(cls, name)
        super().__delattr__(name)
        for parent in cls.__mro__[1:]:
            if name in parent.__fields:
                field = parent.__fields[name]
