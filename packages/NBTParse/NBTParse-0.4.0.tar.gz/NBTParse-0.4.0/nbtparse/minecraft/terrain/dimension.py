...
class Dimension:
    ...
    def __repr__(self):
        return '<Dimension in {!r}>'.format(self.path)
    ...
    def _getregion(self, r_x: int, r_z: int) -> region.Region:
        ...
        """
        return self._region_manager.get_region(r_x, r_z, self.fill_value)
...
class _RegionManager:
    ...
    def __repr__(self):
        return '<_RegionManager: owner={!r}>'.format(self.owner)
    ...
    def _load_region(self, r_x: int, r_z: int,
                     fill_value: voxel.Block=None) -> region.Region:
        ...
        try:
            ...
            logger.info('Region %d, %d does not exist, filling in %r',
                        r_x, r_z, fill_value, exc_info=True)
            return self._fill_region(r_x, r_z, fill_value)
        with region_file:
            return region.Region.load(r_x, r_z, region_file)


    def _fill_region(self, r_x: int, r_z: int,
                     fill_value: voxel.Block) -> region.Region:
        """Create and populate the region at (r_x, r_z).

        Fill all the blocks with the given fill value.

        Result is not cached.

        """
        result = region.Region(r_x, r_z)
        for x, z in itertools.product(range(REGION_CHUNKS), repeat=2):
            cnk = chunk.Chunk()
            for y in range(SECTIONS_PER_CHUNK):
                sec = chunk.Section()
                sec.blocks[...] = fill_value
                cnk.sections[y] = sec
            result[x, z] = cnk
        return result
    ...
    def get_region(self, r_x: int, r_z: int,
        ...
        try:
            ...
            result = self._load_region(r_x, r_z, fill_value)
            self.regions[r_x, r_z] = result
            return result
...
class _EntitySliceable:
    ...
    def __getitem__(self, idx: (slice, slice, slice)) -> {entity.Entity}:
        ...
        for c_x, c_z in itertools.product(x_range, z_range):
            ...
            result.update(x for x in ck.get_entities(self.namespace)
                          if in_range(x))
        return result
    ...
    def __setitem__(self, idx: (slice, slice, slice), seq: {entity.Entity}):
        ...
        for c_x, c_z in itertools.product(x_range, z_range):
            ck = self.dim._getchunk(c_x, c_z)
            preexisting = (ent for ent in ck.get_entities(self.namespace)
                           if not in_range(ent))
            ...
            ck.set_entities(mapping[c_x, c_z].union(preexisting))
