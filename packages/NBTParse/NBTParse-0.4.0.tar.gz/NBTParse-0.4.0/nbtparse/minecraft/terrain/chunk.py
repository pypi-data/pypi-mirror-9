...
class Chunk(nbtobject.NBTObject):
    ...
    """
    def get_entities(self, namespace: str='minecraft') -> [entity.Entity]:
        """Return an iterable of entities in the chunk.

        Entities are converted from NBT based on the given namespace.

        """
        for nbt in self.data['Entities']:
            yield entityfactory.from_nbt(nbt, namespace=namespace)

    def set_entities(self, entities: [entity.Entity]):
        """Set the entities in this chunk to the given value."""
        nbt = tags.ListTag((entity.to_nbt() for entity in entities),
                           ids.TAG_Compound)
        self.data['Entities'] = nbt
