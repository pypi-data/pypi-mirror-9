...
def decode_named(input: io.BufferedIOBase, errors: str='strict'):
    """Decode a named tag from input and returns (name, tag).

    Reads from input; see :meth:`~TagMixin.decode_payload` for some caveats
    related to this.
    ...
    .. note::
        ...
        For reasons of API symmetry, this function is known as both
        :func:`decode_named` and as :meth:`TagMixin.decode_named`.  They are
...
class TagMixin(metaclass=abc.ABCMeta):
    ...
    decode_named = staticmethod(decode_named)
...
def _encode_payload_closure(formatstr):
    def encode_payload(self, output: io.BufferedIOBase, errors='strict'):
        ...
        try:
            ...
            raise  # in case someone passed -O to python
        output.write(raw)
