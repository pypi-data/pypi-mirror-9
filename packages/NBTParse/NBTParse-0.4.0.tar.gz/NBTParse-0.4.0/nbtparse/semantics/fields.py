...
class AbstractField(metaclass=abc.ABCMeta):
    ...
    def __init__(self, *args, default=None, **kwargs):
        ...
        """Default value used by :meth:`set_default`."""

        self.field_names = weakref.WeakKeyDictionary()
        """Mapping from owner classes to field names.

        Each class in which this field is visible is considered an owner
        class.  This includes classes which inherited the field.

        Keys are referenced weakly so that temporary classes will not be
        leaked.
    ...
    def attach(self, owner: NBT_META, name: str):
        """Hook called during owner class creation.

        Called once for each class in which this field is visible.

        Default behavior populates :attr:`field_names`.

        """
        self.field_names[owner] = name

    def detach(self, owner: NBT_META, name: str):
        """Hook called when the field is detached.

        Counterpart to :meth:`attach`.

        """
        del self.field_names[owner]
...
class MutableField(AbstractField):
    ...
    """
    def save(self, obj: NBT_OBJECT):
        field_name = self.field_names[type(obj)]
        try:
...
class ReadOnly:
    ...
    def __repr__(self):
        return 'ReadOnly({!r})'.format(self.wrapped)
...
class ByteArrayField(MutableField, SingleField):
    ...
    def __init__(self, nbt_name: str, *, default: bytearray=None):
        if default is None:
            default = bytearray()
        super().__init__(nbt_name, typename="string (8-bit)", default=default)
...
class NBTObjectField(MutableField, SingleField):
    ...
    def __init__(self, nbt_name: str, obj_class: NBT_META, *,
        ...
        super().__init__(nbt_name, typename="NBTObject", default=default)

    def __repr__(self) -> str:
        return ('NBTObjectField({!r}, {}, default={!r})'
                .format(self.nbt_name, self.obj_class.__name__, self.default))

    def to_python(self, nbt: tags.TagMixin) -> NBT_OBJECT:
...
class ListField(MutableField, SingleField):
    ...
    def __init__(self, nbt_name: str, item_to_python: types.FunctionType,
        ...
        super().__init__(nbt_name, typename="list", default=default)
...
class TupleListField(MutableField, SingleField):
    ...
    def __init__(self, nbt_name: str, item_to_python: types.FunctionType,
        ...
        self.item_from_python = item_from_python
        self.content_id = content_id
        super().__init__(nbt_name, typename="tuple", default=default)

    def __repr__(self):
        return ('TupleListField({!r}, {!r}, {!r}, {!r}, default={!r})'
                .format(self.nbt_name, self.item_to_python,
                        self.item_from_python, self.content_id, self.default))
...
class ObjectTupleField(TupleListField):
    ...
    def __repr__(self):
        return ('ObjectTupleField({!r}, {!r}, default={!r})'
                .format(self.nbt_name, self.obj_class, self.default))
...
class EnumField(SingleField):
    ...
    def __repr__(self):
        return ('EnumField({!r}, {!r}, tag_type={!r}, default={!r})'
                .format(self.nbt_name, self.enum_class, self.tag_type,
                        self.default))
...
class HeterogenousDictField(MutableField, SingleField):
    """Field for a dictionary of heterogenous values.

    Keys are strings, values are arbitrary objects.  The key ``'data'`` is
    reserved for storing the original value of the NBT.  None of the keys may
    begin with underscores.

    """
    def __init__(self, nbt_name: str, schema: {str: AbstractField}, *,
                 default: {str: object}=None):
        self.schema = schema
        """Template for the dictionaries this field should create.

        The keys are arbitrary strings, which will be duplicated verbatim.
        Those strings must not begin with underscores.  The values are fields,
        which will be invoked in the normal manner to create the values of
        this dictionary, and to parse them when converting back to NBT.

        The original NBT will be stored in the ``'data'`` key, which is
        reserved for this purpose.  If the key still exists when converting
        back to NBT, it will be used to supply any values not specified
        elsewhere in the dictionary.  This allows round-tripping of items not
        controlled by any field.

        """
        for key in schema:
            if key.startswith('_'):
                raise ValueError('Key {!r} begins with an underscore'
                                 .format(key))
        if 'data' in schema:
            raise ValueError('The name data is reserved')

        from . import nbtobject
        class ObjectProxy(nbtobject.NBTObject):
            pass
        for name, field in schema.items():
            setattr(ObjectProxy, name, field)

        self.object_proxy_class = ObjectProxy
        super().__init__(nbt_name, default=default)

    def to_python(self, nbt: tags.CompoundTag) -> {str: object}:
        object_proxy = self.object_proxy_class.from_nbt(nbt)
        result = {key: getattr(object_proxy, key) for key in self.schema
                  if getattr(object_proxy, key) is not None}
        result['data'] = nbt

    def from_python(self, mapping: {str: object}) -> tags.CompoundTag:
        result = mapping.pop('data', tags.CompoundTag())
        try:
            object_proxy = self.object_proxy_class(**mapping)
        except TypeError as exc:
            raise ValueError('Unrecognized key') from exc
        result.update(object_proxy.to_nbt())
        return result


class HomogenousDictField(MutableField, SingleField):
    """Field for a dictionary of homogenous values.

    Abstract.  Subclasses should override :meth:`item_to_python` and
    :meth:`item_from_python`."""
    def __init__(self, nbt_name: str, *, default: {str: object}=None):
        super().__init__(nbt_name, default=default)

    @abc.abstractmethod
    def item_to_python(self, key: tags.StringTag, value: tags.TagMixin
                       ) -> (str, object):
        raise NotImplementedError()

    @abc.abstractmethod
    def item_from_python(self, key: str, value: object) -> (tags.StringTag,
                                                            tags.TagMixin):
        raise NotImplementedError()

    def to_python(self, nbt: tags.CompoundTag) -> {str: object}:
        return dict(itertools.starmap(self.item_to_python, nbt.items()))

    def from_python(self, mapping: {str: object}) -> tags.CompoundTag:
        return tags.CompoundTag(itertools.starmap(self.item_from_python,
                                                  mapping.items()))
