...
def command_line_arguments(command_line_parameters):
  ...
  parser.add_argument('-f', '--far', action = 'store_true', help = "If given, FAR will be computed at FRR value precised in --frr option")
  parser.add_argument('--frr-value', default=0.01,  help='fixed FRR value')
...
def main(command_line_parameters=None):
  ...
  if args.criterion or args.roc or args.det or args.cllr or args.mindcf or args.far:
    ...
    if args.far:
      utils.info("Computing FAR on the development " + ("and on the evaluation set" if args.eval_files else "set"))
      for i in range(len(scores_dev)):
        # compute threshold on development set
        threshold = bob.measure.frr_threshold(scores_dev[i][0], scores_dev[i][1], float(args.frr_value))
        # apply threshold to development set
        far, frr = bob.measure.farfrr(scores_dev[i][0], scores_dev[i][1], threshold)
        print("The FAR of the development set of '%s' is %2.3f%%" % (args.legends[i] if args.legends else args.dev_files[i], far * 100. ))
        if args.eval_files:
          # apply threshold to evaluation set
          far, frr = bob.measure.farfrr(scores_eval[i][0], scores_eval[i][1], threshold)
          print("The FAR of the evaluation set of '%s' is %2.3f%%" % (args.legends[i] if args.legends else args.eval_files[i], far * 100. ))
