...
class Article(Page, RichText):
    ...
    show_content = models.BooleanField(
        _("show content"), default=True,
        help_text=_("Controls content on page."))
    show_children_list = models.BooleanField(
        _("show children"), default=False,
        help_text=_(
            "Controls children list on page."))
    ...
    def siblings(self):
        ...
        """
        if self.parent:
            return self.parent.children.exclude(pk=self.id).order_by("_order")
        return Article.objects.filter(parent__isnull=True).order_by("_order")
    ...
    def get_absolute_url(self):
        if self.slug.startswith('http://') or self.slug.startswith(
                'https://'):
            return mark_safe(self.slug)
        else:
