...
class SquirroClient(UserApiMixin, TopicApiMixin, BulkApiMixin):
    ...
    :param user_api_url: Endpiont URL for the user API service.
    :param topic_api_url: Endpoint URL for the topic API service.
    :param bulk_api_url: Endpoint URL fo the topic API service.
    ...
    :param retry_method_whitelist: Set of uppercased HTTP method verbs that we
        should retry on.
    ...
    :param timeout_secs: How many seconds to wait for data before giving up (default 55).
    ...
    def authenticate(self, tenant=None, access_token=None, refresh_token=None,
        ...
        :raises: :class:`squirro_client.exceptions.AuthenticationError` if authentication fails.
        ...
        base = {}
        if not self.client_id is None:
            base['client_id'] = self.client_id
        if not self.client_secret is None:
            base['client_secret'] = self.client_secret
        if not self.tenant is None:
            base['tenant'] = self.tenant
        if not user_information is None:
            base['user_information'] = json.dumps(user_information)

        # walk through all possible authentication scenearios
        # - access token
        # - refresh token
        # - external id
        # - username / password
    ...
    def _perform_authentication(self, data):
        ...
        # create endpoint url to fecth new tokens
        ...
        try:
            ...
            log.debug('unable to decode JSON: %r', ex)
    ...
    def _refresh_tokens(self, **kwargs):
        ...
        try:
            ...
            log.debug('unable to refresh access token: %r', ex)

        return ret

    def _process_response(self, res, exected_status_codes=None):
        """Return the data from the resonse if the status code matches to what
        is expected. If that is not the case the appropriate exception is
        being raised.
        """

        # work around mutable default arguments
        if exected_status_codes is None:
            exected_status_codes = [200]
        ...
        if res.status_code in exected_status_codes:
            return data
