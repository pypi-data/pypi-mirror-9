...
class HpsFluentCreditService(HpsSoapGatewayService):
    ...
    def cpc_edit(self):
        return HpsCreditServiceCpcEditBuilder(self)

    def edit(self):
        return HpsCreditServiceEditBuilder(self)
    ...
    def submit_transaction(self, transaction, client_transaction_id=None):
        ...
        self.process_charge_gateway_response(
            rsp, transaction.tag, amount, 'usd')

        self.process_charge_issuer_response(
            rsp, transaction.tag, amount, 'usd')
...
class HpsCreditServiceAuthorizeBuilder(HpsBuilderAbstract):
    ...
    def execute(self):
        ...
        if self._details is not None:
            block1.append(self._hydrate_additional_txn_fields(self._details))
...
class HpsCreditServiceVerifyBuilder(HpsBuilderAbstract):
    ...
    def only_one_payment_method(self):
        if self._card is not None and self._token is not None:
            return False
        elif self._card is None and self._token is None:
            return False
...
class HpsCreditServiceRecurringBuilder(HpsBuilderAbstract):
    ...
    def execute(self):
        ...
        Et.SubElement(recurring_data, 'ScheduleID').text = schedule_id
...
class HpsFluentCheckService(HpsSoapGatewayService):
    ...
    def build_transaction(self, action, check, amount, client_transaction_id=None):
...
class HpsFluentGiftCardService(HpsSoapGatewayService):
    ...
    def void(self):
        return HpsGiftCardServiceVoidBuilder(self)
...
class HpsGiftCardServiceAliasBuilder(HpsBuilderAbstract):
    ...
    def _setup_validations(self):
        ...
        self.add_validation(self.__action_not_none, HpsArgumentException, 'Alias needs an action.')
