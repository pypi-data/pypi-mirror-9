...
class HpsPayPlanPaymentMethod(HpsPayPlanResource):
    ...
    def get_json_data(self):
        ...
        if self.payment_method_type == HpsPayPlanPaymentMethodType.ACH:
            fields['telephoneIndicator'] = self.telephone_indicator
            fields['accountHolderYob'] = self.account_holder_yob
            fields['driversLicenseState'] = self.drivers_license_state
            fields['driversLicenseNumber'] = self.drivers_license_number
            fields['socialSecurityNumberLast4'] = self.social_security_number_last_4
        elif self.payment_method_type == HpsPayPlanPaymentMethodType.CREDIT_CARD:
            fields['expirationDate'] = self.expiration_date
