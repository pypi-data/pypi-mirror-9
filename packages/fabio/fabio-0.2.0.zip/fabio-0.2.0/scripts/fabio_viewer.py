...
import time
import h5py
from PyQt4.QtCore import QDir, Qt, SIGNAL, QObject, QRect, QString, QCoreApplication
    ...
    def save_as(self):
        ...
        if self.data.any():
            ...
            if str(info[0]) != '' and str(info[1]) != '':
                QFileDialog.warning(self, 'Warning', "Could not save image as file if no data have been loaded")
        ...
        if self.data_series or self.sequential_file_list:
            if str(info[0]) != '' and str(info[1]) != '':
                ...
                if format_ == '*.h5':
                    ...
                    QFileDialog.warning(self, 'Warning', "Unknown format: %s" % format_)
                    return
        else:
            if str(info[0]) != '' and str(info[1]) != '':
                QFileDialog.warning(self, 'Warning', "Could not save image as file if no data have been loaded")
        ...
        if self.sequential_file_mode:
            ...
            for imgkey in self.sequential_file_list:
                ...
                try:
                    ...
                    QFileDialog.warning(self, 'Message', 'Automatic format recognition procedure failed or pehaps you are trying to open a binary data block...\n\n     ...Switch to manual procedure ')
                    Dial = binDialog(self)
                    dim1, dim2, offset, bytecode, endian = Dial.exec_()
                    if dim1 is not None and dim2 is not None:
                        if endian == 'Short':
    ...
    def horizontal_mirror(self):
        if self.transform_data_series:
            if self.sequential_file_mode:
                ...
                total = len(self.data_series)
                if not total:
                    QFileDialog.warning(self, 'Warning', "Could not transform image if no data have been loaded")
                    return
                for i in range(len(self.data_series)):
                    ...
                    if self.sequential_file_mode:
                        ...
                        for i in range(len(self.data_series)):
                            if self.data_series[i].shape != self.mask.shape:
                                QFileDialog.warning(self, 'Warning', "Mask and image have different shapes, skipping image %d" % i)
...
#--------------------------------------------------------------------------------------------------------
    ...
    def batch_to_view(self):
        ...
        for xtens in hdfxtens:
            if xtens in item:
                QFileDialog.warning(self, 'Message', "Can't display hdf archive from batch mode ")
                return
        if self.sequential_file_mode:
            self.data_series = []
            self.header_series = []
    ...
    def sequential_option(self, state):
        if not self.h5_loaded:
            ...
            self.sequential_file_mode = False
            self.filecheckBox.stateChanged.connect(self.sequential_option)
            QFileDialog.warning(self, 'Message', 'Sequential file mode is not compatible with hdf5 input file: option removed')
...
#--------------------------------------------------------------------------------------------------------
    ...
    def on_about(self):
        ...
        QFileDialog.about(self, "About FabIO Viewer", os.linesep.join(msg))
    ...
    def exec_(self):
        if QDialog.exec_(self) == QDialog.Accepted:
            if str(self.lineEdit.text()) != '':
                ...
                QFileDialog.warning(self, 'Warning', "All informations are mandatory, please fill the blanks")
        else:
