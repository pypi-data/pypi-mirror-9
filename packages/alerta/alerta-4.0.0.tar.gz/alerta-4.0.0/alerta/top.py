...
class AlertTop(threading.Thread):

    def __init__(self, url, key=None, filters=None, interval=2):
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.from_date = self._get_alerts(self.filters, self.from_date)
            try:
                time.sleep(self.interval)
            except (KeyboardInterrupt, SystemExit):
                sys.exit(0)

    def _get_alerts(self, filters, from_date=None):
...
class Screen(object):
    ...
    def run(self):
        ...
        self.w = AlertTop(url=self.endpoint, key=self.key)
        self.w.start()
