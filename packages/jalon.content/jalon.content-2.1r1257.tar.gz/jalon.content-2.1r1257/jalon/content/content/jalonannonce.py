...
class JalonAnnonce(ATDocumentBase):
    ...
    def envoyerAnnonce(self):
        ...
        #if not de:
        #    de = authMember.getProperty("mail")
        ...
        #diffusion = jalon_conf.getListeDiffusion()
        ...
        for public in self.getPublics():
            ...
            if diffusion["activer_diffusion"] and typeELP in diffusion["type_diffusion"]:
                # spécifique à versailles pour 2013
                #codeAp = codeELP.rsplit("-", 1)
                #dico["a"] = "%s%s" % (codeAp[0], diffusion["mailListeDiffusion"])
