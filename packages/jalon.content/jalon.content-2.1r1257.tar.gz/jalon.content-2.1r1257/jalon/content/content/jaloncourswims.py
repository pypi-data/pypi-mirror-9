...
class JalonCoursWims(ATDocument):
    ...
    # afficherRessource() modifie les dates de la ressource pour
    # activer/desactiver son affichage
    ####################
    ...
    # ajouterElement() permet d'ajouter un element (sujet ou exercice) à une autoevaluation ou un examen.
    ##################
    ...
    # authUser() : appelle la fonction authUser de jalon_utils
    ...
    #autoriser_Affichage : définit si l'affichage de l'objet 'idElement' est autorisé ou pas.
    ...
    # detacherRessource() détache un sujet (la ressource) de l'activité courante
    ...
    # getAllNotes() renvoie toutes les notes des étudiants sur l'activité courante
    # TODO
    ...
    # getClasse() retourne l'identifiant de la classe du cours associée à l'auteur de cette activite
    def getClasse(self):
        auteur = self.getCreateur()
    ...
    # getCreateur() retourne l'auteur de l'activite courante
    ...
    # setClasse() crée une classe associée au cours dans le groupe de classes de l'auteur de cette activite
    def setClasse(self):
        auteur = self.getCreateur()
        listeClasses = list(self.aq_parent.getListeClasses())
        idClasse = None
    ...
    # getDisplayLang() retourne la langue de l'exercice, dans un format affichable, ainsi qu'un code d'icone de drapeau
    ...
    # getDicoProperties() Renvoie le dico des propriétés de l'objet jalonCoursWims courant (Autoevaluation ou Examen)
    ...
    # getElementCours() renvoie soit infos_element, soit infos_element[key] (utilisé pour detacher un exercice)
    def getElementCours(self, key=None):
        if key:
            return self.getInfosElement().get(key, None)
        return self.getInfosElement()

    # getGroupement() renvoit le groupement de classe associé a l'auteur en parametre
    ...
    # getInfosListeAttribut() : renvoit la liste des elements d'une activité WIMS
    ...
    def getNotes(self, quser, actif, isProf, detailed=False, site_lang="", request=None):
                        ...
                        if rep["status"] == "OK":
                            ...
                            for user in rep["data_scores"]:
                                if user["id"] in dico_ETU:
                                    ...
                                    user["nom"] = individu["nom"]
                                    user["prenom"] = individu["prenom"]
                                    user["num_etu"] = individu["num_etu"]
                                else:
                                    fullname = jalon_utils.getDisplayName(user["id"], portal=portal)
                                    user["prenom"], user["nom"] = fullname.split(" ", 1)
        ...
        rep = json.loads(self.wims("callJob", {"job": "getsheet", "code": quser, "qclass": param["qclass"], "qsheet": param["qsheet"]}))
        listeNotes = []
        if rep["status"] == "OK":
    ...
    # getNotesTableur()  renvoit les notes de l'activite courante, dans un format tableur (csv ou tsv)
            ...
            if isProf:
                ...
                for etudiant in listeEtudiant["data_scores"]:
                    if self.idExam:
                        ...
                        #if sep == ",":
                        #    score = score.replace(",", ".")
                        ligne = [etudiant["nom"], etudiant["prenom"], str(etudiant["num_etu"]), etudiant["id"], str(etudiant["attempts"]), score]
                    else:
                        note = "%s %%" % etudiant["user_percent"]
                        qualite = str(etudiant["user_quality"])
                        #if sep == ",":
                        #    qualite = qualite.replace(",", ".")
                        ligne = [etudiant["nom"], etudiant["prenom"], str(etudiant["num_etu"]), etudiant["id"], note, qualite]
    ...
    # getUserLog()
    # Renvoit les logs de connexion à wims de l'utilisateur quser
    ...
    def getUserLog(self, quser, authUser, isProf, option="score"):
        ...
        rep_wims["fullname"] = jalon_utils.getDisplayName(quser)
    ...
    # getWimsLang() renvoie le code de langue de l'interface des exercices.
    ...
    # retirerElement() détache un élément de l'activité
    ...
    # setAttributActivite() modifie les attribut de l'objet jaloncourswims
    ...
    #Obtentiopn (et eventuellement Création) d'un identifiant Wims pour la feuille
    ...
    # setJalonProperties() Modifie les propriétés de l'objet jalonCoursWims courant (Autoevaluation ou Examen), coté Jalon uniquement.
    ...
    # setProperties() modifie les propriétés de l'objet jalonCoursWims courant (Autoevaluation ou Examen)
    ...
    # wims() : permet d'appeler la methode definie en parametre depuis le connecteur wims, en lui envoyant le dico "param".
