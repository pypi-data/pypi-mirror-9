...
class Foxtrot(object):
  ...
  def __fetch_methods(self):
    resp = requests.get("{0}://{1}{2}".format(self._proto(), self.host, self.api_root))
    return resp.json()['endpoints'][self.version]

  def __create_method(self, endpoint, endpoint_name, endpoint_method_name, http_method):
    def f(self, *args, **kwargs):
      if len(args) == 1 and len(kwargs) == 0:
        kwargs = args[0]
      for param in endpoint['params'][http_method]:
  ...
  def __set_method(self, endpoint):
    for http_method in endpoint['methods']:
      endpoint_name = (endpoint['path']
                      ...
                      .replace('/', '_'))
      endpoint_name = re.sub(r'_<(.*)>', '', endpoint_name)
      endpoint_method_name = http_method.lower() + '_' + endpoint_name
