...
__all__ = ['STT', 'SQTT', 'LINEAR_INTERPOLATION', 'LAGRANGE_INTERPOLATION', 'PROJECTION', 'AVAIL_SURROGATE_TYPES']
...
from TensorToolbox.core import TensorWrapper, TTvec, QTTvec, storable_object
...
class STT(storable_object):
    ...
    def __init__(self, f, grids, params, range_dim=0, marshal_f=True,
        ...
        self.polys = None                # List of polynomials (Spectral1D.Poly1D)
        #
    ...
    def build(self,maxprocs=None,force_redo=False):
        ...
        if self.generic_approx == None or force_redo:
        ...
        for point, val in np.ndenumerate(self.generic_approx):
            if val == None or not val.init:
                ...
                if val == None or self.generic_approx[point].Jinit == None:
...
class SQTT(STT):
    ...
    def __init__(self, f, grids, params, range_dim=0, marshal_f=True,
        ...
        super(SQTT,self).__init__(f, grids, params, range_dim=range_dim, marshal_f=marshal_f,
                                  surrogateONOFF=surrogateONOFF, surrogate_type=surrogate_type, orders=orders, orderAdapt=orderAdapt, 
                                  eps=eps, method=method,
                                  rs=rs, fix_rank=fix_rank, Jinit=Jinit, delta=delta, maxit=maxit, 
                                  mv_eps=mv_eps, mv_maxit=mv_maxit,
                                  kickrank=kickrank,
                                  store_location=store_location,store_overwrite=store_overwrite, store_freq=store_freq)
