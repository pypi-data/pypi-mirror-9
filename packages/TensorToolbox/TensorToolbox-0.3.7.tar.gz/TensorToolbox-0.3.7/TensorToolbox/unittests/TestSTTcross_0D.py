...
import os
import os.path
from auxiliary import print_ok, print_fail, print_summary
...
def run(maxprocs, loglev=logging.WARNING):
    ...
    if os.path.isfile(store_file + '.pkl'): # If file already exists, then restart the construction from
        ...
        STTapprox = TT.STT(f, X, params, eps=eps, range_dim=0, orders=orders, method='ttcross', surrogateONOFF=True, surrogate_type=surr_type, store_location=store_file, store_overwrite=True, store_freq=store_freq) # See documentation of STT
        STTapprox.build(maxprocs)
    ...
    passed = passed and err_mean < targetErr
    if passed:
        print_ok("Spectral Tensor Train Cross - 0D")
        nsucc += 1
    else:
        print_fail("Spectral Tensor Train Cross - 0D")
        nfail += 1
