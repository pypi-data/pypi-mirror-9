...
from TensorToolbox.core import TTvec, mat_to_tt_idxs
...
class TTmat(TTvec):
    ...
    def build(self, eps=1e-10, method='svd', rs=None, fix_rank=False, Jinit=None, delta=1e-4, maxit=100, mv_eps=1e-6, mv_maxit=100, kickrank=2):
        ...
        if isinstance(self.A,list) and np.all( [ isinstance(self.A[i],scsp.csr_matrix) for i in range(len(self.A)) ] ):
        ...
        elif isinstance(self.A,list) and np.any( [ isinstance(self.A[i],scsp.csr_matrix) for i in range(len(self.A)) ] ):
            raise TypeError("Mixed sparse/full initialization not implemented yet")
        else:
            self.sparse_only = False
            super(TTmat,self).build( eps, method, rs, fix_rank, Jinit, delta, maxit, mv_eps, mv_maxit, kickrank )
            if isinstance(nrows,int) and isinstance(ncols,int):
                d = len(self.TT)
                self.nrows = [nrows for i in range(d)]
                self.ncols = [ncols for i in range(d)]
