...
class TTvec(storable_object):
    """ Constructor of multidimensional tensor in Tensor Train format [1]_
    ...
    .. [2] Oseledets, I., and Tyrtyshnikov, E. (2010). TT-cross approximation for multidimensional arrays. Linear Algebra and Its Applications, 432(1), 70-88. doi:10.1016/j.laa.2009.07.024
    .. [3] Savostyanov, D., and Oseledets, I. (2011). Fast adaptive interpolation of multi-dimensional arrays in tensor train format. The 2011 International Workshop on Multidimensional (nD) Systems, 1–8. doi:10.1109/nDS.2011.6076873
    .. [4] Savostyanov, D. (2013). Quasioptimality of maximum-volume cross interpolation of tensors. arXiv Preprint arXiv:1305.1818, c, 1–23. Retrieved from http://arxiv.org/abs/1305.1818
    ...
    def __init__(self,A, 
        ...
        super(TTvec,self).__init__(store_location, store_freq, store_overwrite, store_object)
        ...
        self.serialize_list.extend( ['TT', 'init', 'method', 'eps', 'multidim_point', 'rs', 'Js', 'Is', 'Js_last', 'Jinit', 'ltor_fiber_lists', 'rtol_fiber_lists', 'dmrg_not_converged', 'dmrg_sweep_it', 'dmrg_sweep_direction', 'dmrg_Pinv'] )
    ...
    def svd(self, eps):
        ...
        d = self.A.ndim
        n = self.A.shape
        delta = (eps/max(np.sqrt(d-1),1e-300)) * npla.norm(self.A.flatten(),2) # Truncation parameter
        ...
        for k in range(d-1):
            ...
            CF = npla.norm(S,2)
            rk = 1
            while npla.norm(S[rk:],2) > delta: rk += 1
    ...
    def ttdmrgcross(self, eps, Jinit, maxit, mv_eps, mv_maxit, kickrank=None, store_init=True):
    ...
    def ttdmrg(self, eps, Jinit, maxit, mv_eps, mv_maxit, kickrank=None, cross=False, store_init=True):
        """ Construct a TT representation of A using TT-Density matrix renormalization group
        ...
        while it < maxit and self.dmrg_not_converged:
            ...
            if it == 1 and self.dmrg_sweep_direction == 'lr':
                drange = xrange(self.dmrg_sweep_it,d-1)
            ...
            if it == 1 and self.dmrg_sweep_direction == 'rl':
                drange = xrange(self.dmrg_sweep_it,0,-1)
            else:
                drange = xrange(d-1,0,-1)
                self.Js = [None] * d
                self.Js[-1] = [()]
        ...
        if cross:
            ...
            for ii,jj in itertools.product(self.Is[k],self.Js[k+1]):
                if isinstance(self.A, TensorWrapper):
                    fix_idxs = list( ii + jj )
                    fix_dims = range(len(ii)) + range(len(ii)+2,d)
            ...
            if k == 0:
                idx = (slice(None,None,None), slice(None,None,None)) + tuple( itertools.izip(*self.Js[k+1]) )
            else:
    ...
    def __right_left_ttdmrg_step(self, it, k, not_converged, Pinv, kickrank, mv_eps, mv_maxit, 
        ...
        if cross:
            ...
            # Create index list to get the values at once
            if k == d-1:
                idx = tuple( itertools.izip(*self.Is[k-1]) ) + (slice(None,None,None), slice(None,None,None))
            else:
    ...
    def __check_create_Jinit( self, rs, Jinit):
        ...
        for k_Js in range(len(Jinit)-1):
            if Jinit[k_Js] == None:
                if rs[k_Js+1] > max( np.prod(n[k_Js+1:]), sys.maxint ):
