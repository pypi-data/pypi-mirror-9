...
DAT_FOLDER = "./Data-ttdmrg"
FIG_FOLDER = "./Fig-tmp-ttdmrg"

def test(FNUM, GenzNormalized=False, SpectralType='Projection', GenzPatrick=False):

    logging.basicConfig(filename='TTdmrgGenz-%d-%s-%s-%s.log' % (FNUM, str(GenzNormalized), SpectralType, str(GenzPatrick)),
                        level=logging.DEBUG)
    ...
    for i_d, d in enumerate(ds):
        for n_exp in range(N_EXP):
            ...
            for i_size, size in enumerate(sizes):
                ...
                TTapprox.build(method='ttdmrg',delta=delta,mv_eps=maxvoleps,maxit=tt_maxit)
