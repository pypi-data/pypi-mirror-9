...
"""Computes Kollreider features for all or a given subset of videos in the
Replay Attack database. Kollreider features are defined at the paper entitled
"Non-intrusive liveness detection by face images", in Image and Vision
Computing, Elsevier, 2009.
"""
...
def main():
  ...
  parser.add_argument('-t', '--tau', metavar='FLOAT>=0', type=float, dest='tau', default=1.0, help='Threshold "tau" to be applied to the flow field comparisons (defaults to \'%(default)s\').')
  ...
  for obj in process:
    ...
    data = kollreider.computeFeature(input, locations, args.tau)
