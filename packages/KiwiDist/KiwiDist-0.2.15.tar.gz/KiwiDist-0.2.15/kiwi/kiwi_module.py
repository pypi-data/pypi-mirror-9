...
def plot(gsn, gss, gsc='', gls='', pc=0.01, adj=True, prm=10000, gsz=10**6,
    ...
    ## Retrieve minimum p-value in the GSS statistics dataset - This was meant to normalize the directionalityScore
    ...
    p_min_nonzero = p_all[p_all!=0].min()
    ...
    pzero      = min(pzero, pcutoff, p_min_nonzero)
    ...
    color_code = (p_stable[:,5] + # <- This is almost zero unless a non-piano file is supplied 
                    ((p_stable[:,1]*(p_stable[:,0]+p_stable[:,2]) - 
    ...
    sm = plt.cm.ScalarMappable(cmap=plt.cm.RdBu_r, 
         norm=plt.Normalize(vmin=-abs(color_code).max(), vmax=abs(color_code).max()))
    sm._A = []
    plt.colorbar(sm)
    if len(nwPlotFile)>0: 
