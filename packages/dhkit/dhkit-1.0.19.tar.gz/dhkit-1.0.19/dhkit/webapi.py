...
from oauthlib.oauth2 import LegacyApplicationClient, BackendApplicationClient, OAuth2Error
...
class ShouYiApi(Api):
    ...
    __access_token = None
    __session_access_token = None

    __expire_in = None
    __session_expire_in = None

    __last_access_time = None
    __session_last_access_time = None
    ...
    def access_token(self):
        ...
        print "self.__access_token is None ", self.__access_token is None
    ...
    def set_access_token(self, headers):
        if not self.api_authorized:
            return
        if self.__session_access_token and not self._is_session_expired():
            headers['Authorization'] = 'Bearer %s' % self.__session_access_token
        else:
    ...
    def authorize(self, username, password):
        ...
        try:
            token_dict = oauth_client.parse_request_body_response(body)
            ShouYiApi.__session_access_token = token_dict.get('access_token')
            ShouYiApi.__session_expire_in = token_dict.get('expires_in')
            ShouYiApi.__session_last_access_time = datetime.datetime.now()
            return token_dict
        except OAuth2Error, e:
            raise ShouYiApiError(msg=e.message)

    def _is_session_expired(self):
        expire_at = self.__session_last_access_time + datetime.timedelta(seconds=self.__session_expire_in)
        if expire_at <= datetime.datetime.now():
            ShouYiApi.__session_access_token = None
            return True
        else:
            return False
