...
class ConnectionFactory(object):
    ...
    @classmethod
    def get_db(cls):
        return AsyncDatabaseTemplate(cls._connection_pool, cls._thread_pool, cls._io_loop)

    @classmethod
    def get_transaction_db(cls):
        current_thd_name = threading.currentThread().getName()
        db = cls.__transactional_db_instances.get(current_thd_name)
        if not db:
            db = TransactionalDatabaseTemplate(cls._connection_pool)
            cls.__transactional_db_instances[current_thd_name] = db
        return db
...
class AsyncDatabaseTemplate(DatabaseTemplate):

    def __init__(self, connection_pool, threadpool, ioloop=None):
        super(AsyncDatabaseTemplate, self).__init__(connection_pool)
...
class TransactionalDatabaseTemplate(DatabaseTemplate):

    def __init__(self, connection_pool):
        super(TransactionalDatabaseTemplate, self).__init__(connection_pool)
        self.conn_local = threading.local()

    def begin(self):
        self.conn_local.conn = self.connection_pool.connection()

    def commit(self):
        if hasattr(self, 'conn_local'):
            self.conn_local.conn.commit()
            self.conn_local.conn.close()

    def rollback(self):
        if hasattr(self, 'conn_local'):
            self.conn_local.conn.rollback()
            self.conn_local.conn.close()

    def execute(self, sql_statement, params):
        return self._execute(sql_statement, params, self.conn_local.conn)

    def insert(self, sql_statement, params=None):
        return self._execute(sql_statement, params, self.conn_local.conn)[1]

    def delete(self, sql_statement, params=None):
        return self._execute(sql_statement, params, self.conn_local.conn)[0]

    def update(self, sql_statement, params=None):
        return self._execute(sql_statement, params, self.conn_local.conn)[0]

    def query(self, sql_statement, params=None, return_metadata=False, row_mapper=None):
        return self._query(sql_statement, params, self.conn_local.conn, return_metadata, row_mapper)

    def get(self, sql_statement, params=None, return_metadata=False, row_mapper=None):
        rs = self._query(sql_statement, params, self.conn_local.conn, return_metadata, row_mapper)
...
def transactional(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        def transaction_func(*args_, **kwargs_):
            db_ = ConnectionFactory.get_transaction_db()
            db_.begin()
            log.debug("db: [%s] transaction begin..." % db_)
            try:
                rv = func(*args_, **kwargs_)
                db_.commit()
                log.debug("db: [%s] transaction commit ok." % db_)
                return rv
            except:
                db_.rollback()
                log.debug("db: [%s] transaction rollback ok." % db_)
                raise
        ...
        def handle_result(result):
            ...
            if error:
                ...
                future.set_result(result.get_result())
        ConnectionFactory._thread_pool.add_task(
            partial(transaction_func, *args, **kwargs), callback=handle_result)
        return future
    return wrapper